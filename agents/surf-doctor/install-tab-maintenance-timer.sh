...
Installs user-level systemd unit files for guarded Surf tab maintenance.
The installer does not enable or start the timer unless explicitly requested.
EOF
}
...
if command -v systemctl >/dev/null 2>&1; then
  ...
  [[ "$ENABLE" -eq 0 ]] || systemctl --user enable surf-tab-maintenance.timer
  [[ "$START" -eq 0 ]] || systemctl --user start surf-tab-maintenance.timer
fi
