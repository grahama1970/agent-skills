...
def test_health_fix_reports_per_dimension_results():
    ...
    assert per_dim["qra_coverage_per_control"]["skip_reason"] == "no_records_changed"
    assert per_dim["sparta_explorer_page_purpose"]["skip_reason"] == "not_repairable_by_monitor_sparta"


def test_qra_contract_flags_legacy_unbounded_worker_launch():
    mod = _load_module()
    receipt = _fixture("repair_cycle_noop_input.json")
    enriched = mod.enrich_repair_cycle_receipt(receipt)
    ...
    assert qra_step["skip_reason"] == "legacy_unbounded_create_qras_worker_not_allowed_use_bounded_create_qras_repair_lane"
    assert enriched["r3_diagnostics"]["contract"] == "QRA coverage is Dewey-repairable through bounded create-qras review/dry-run/canary"
    assert "qra_coverage_per_control" in enriched["r3_diagnostics"]["repairable_by_repair_cycle"]
    assert "qra_coverage_per_control" not in enriched["r3_diagnostics"]["remaining_unfixable_dimensions"]
...
def test_operator_manifest_entry_is_jsonl(tmp_path):
    ...
    assert decoded["contract"] == "bounded_create_qras_repair_lane"
