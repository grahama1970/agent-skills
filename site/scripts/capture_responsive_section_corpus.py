...
def section_plan(cdp: CDPController, section_id: str) -> dict:
...
(() => {{
  ...
  const availableHeight = Math.max(220, innerHeight - Math.max(0, navRect.bottom || 0) - 24);
  const step = Math.max(180, Math.floor(availableHeight * 0.86));
  const height = Math.max(1, rect.height);
  const segments = [];
  for (let offset = 0, index = 0; offset < height && index < 12; offset += step, index += 1) {{
    segments.push({{
      index,
      offset,
      height: Math.min(availableHeight, height - offset),
    }});
  }}
  if (segments.length > 1 && segments[segments.length - 1].height < 220) {{
    segments.pop();
  }}
...
(async () => {{
  ...
  const targetY = Math.max(0, absoluteTop + segment.offset - navBottom - 8);
  ...
  const rect = el.getBoundingClientRect();
  const segmentTop = rect.top + segment.offset;
  const visibleTop = Math.max(0, segmentTop, navBottom > 0 ? Math.min(navBottom + 8, innerHeight - 1) : 0);
  const desiredBottom = Math.min(rect.bottom, segmentTop + segment.height);
  const visibleBottom = Math.min(innerHeight, Math.max(visibleTop + 1, desiredBottom));
  ...
  return JSON.stringify({{
    ...
    return {
        ...
        "intended_proof": (
            ...
            "not a full-page or whole-site screenshot. Tall sections are split "
            "into ordered sub-crops."
        ),
    }
...
def draw_contact_sheet(
    ...
    *,
    ...
    for index, item in enumerate(images):
        ...
        segment = ""
        if item.get("segment_count", 1) > 1:
            segment = f' / s{item["segment_index"] + 1}of{item["segment_count"]}'
        label = f'{item["viewport_id"]} / #{item["id"]}{segment}'
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for viewport in VIEWPORTS:
            ...
            for index, section_id in enumerate(SECTIONS):
                plan = section_plan(cdp, section_id)
                if plan.get("error"):
                    plan.setdefault("viewport_id", viewport["id"])
                    plan.setdefault("viewport", viewport)
                    failures.append({"status": "FAIL", **plan})
                    continue
                segments = plan["segments"]
                for segment in segments:
                    filename = (
                        f'{viewport["id"]}-{index:02d}-{section_id}-'
                        f's{segment["index"] + 1:02d}of{len(segments):02d}.png'
                    )
                    record = capture_section_segment(
                        cdp,
                        out_dir / filename,
                        viewport,
                        section_id,
    ...
    manifest = {
        ...
        "viewport_contact_sheets": [
            ...
            for viewport in VIEWPORTS
            if (out_dir / f'{viewport["id"]}-contact-sheet.png').is_file()
        ],
