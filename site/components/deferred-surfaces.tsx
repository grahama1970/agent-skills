...
export function DeferredSkillMosaic() {
  const ref = useRef<HTMLDivElement>(null);
  const near = useNearViewport(ref);
  const [Surface, setSurface] = useState<ComponentType | null>(null);
  ...
  useEffect(() => {
    if (!near || Surface) return;
    import('./skill-mosaic').then((mod) => {
      setSurface(() => mod.SkillMosaic);
    });
  }, [near, Surface]);

  return (
    <div ref={ref} className="deferred-surface deferred-surface--ledger">
      {Surface ? <Surface /> : null}
    </div>
  );
}
...
export function DeferredKeyboardNav() {
  ...
  useEffect(() => {
    ...
    if (idle.requestIdleCallback) {
      const id = idle.requestIdleCallback(load, { timeout: 1800 });
