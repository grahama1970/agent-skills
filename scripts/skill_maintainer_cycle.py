...
ROUTES: list[dict[str, Any]] = [
    {
        "route": "prompt_contract",
        "repair_agent": "coder",
        "verifier_agent": "project-or-harness-verifier",
        "review_agent": "code-reviewer",
        "labels": {"prompt", "prompt-contract", "prompt-review", "review-prompt"},
        "terms": {"prompt", "prompt contract", "prompt-template", "expected response", "validator"},
    },
    {
        "route": "design_or_ux",
...
def target_paths_section(issue: dict[str, Any]) -> str | None:
    for heading in ("Target paths", "Target path"):
        text = issue_section(issue, heading)
        if text:
            return text
    return None
...
def target_skills(issue: dict[str, Any]) -> list[str]:
    text = target_paths_section(issue)
    if not text:
        return []
    found: list[str] = []
...
def receipt_summary(receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "commands_run": receipt.get("commands_run") or receipt.get("commands") or [],
        "changed_files": receipt.get("changed_files") or receipt.get("files_changed") or receipt.get("changed_paths") or [],
...
def scoped_diff_summary(paths: list[str]) -> dict[str, Any]:
    ...
    if not scoped:
        return {"paths": [], "status_short": "", "name_status": "", "stat": ""}
    status_short = git_run(["status", "--short", "--", *scoped])
    name_status = git_run(["diff", "--name-status", "--", *scoped])
    stat = git_run(["diff", "--stat", "--", *scoped])
    return {
        "paths": scoped,
        "status_short": tail(status_short.stdout, 6000),
...
def build_bundle(run_dir: Path, issue: dict[str, Any], route: dict[str, str], skills: list[str]) -> Path:
    ...
    publication_path_list = scoped_publication_paths(result, repair)
    diff_summary = scoped_diff_summary(publication_path_list or target_path_list)
    issue_labels = [
        label.get("name")
    ...
    bundle.write_text(
        "\n".join(
            [
                ...
                "",
                "## Publication Candidate Paths",
                json.dumps(publication_path_list, indent=2),
                "",
                "## Issue Body",
...
def diagnosis_review_lane(issue: dict[str, Any], route: dict[str, str]) -> dict[str, str]:
    labels = {label.get("name", "").lower() for label in issue.get("labels", []) if isinstance(label, dict)}
    text = f"{issue.get('title', '')}\n{issue.get('body', '')}".lower()
    if route.get("route") == "design_or_ux":
        lane = "review-design"
    elif route.get("route") == "prompt_contract" or labels & {"prompt", "prompt-contract", "prompt-review", "review-prompt"}:
        lane = "review-prompt"
    else:
        lane = "review-code"
    if "review-design" in text:
        lane = "review-design"
    elif "review-prompt" in text:
        lane = "review-prompt"
    elif "review-code" in text:
        lane = "review-code"
    return {
        "lane": lane,
        "webgpt_project": "agent-skills-review-code",
        "browser_oracle_from": str(REPO_ROOT),
        "selection_reason": f"route:{route.get('route')}",
    }


def build_diagnosis_bundle(run_dir: Path, issue: dict[str, Any], route: dict[str, str], skills: list[str]) -> Path:
    bundle = run_dir / "diagnosis-bundle.md"
    target_path_list = target_paths(issue)
    issue_labels = [
        label.get("name")
        for label in issue.get("labels", [])
        if isinstance(label, dict) and isinstance(label.get("name"), str)
    ]
    lane = diagnosis_review_lane(issue, route)
    bundle.write_text(
        "\n".join(
            [
                "# Skill Maintainer Pre-Repair WebGPT Diagnosis Bundle",
                "",
                f"Issue: #{issue.get('number')} {issue.get('title')}",
                f"URL: {issue.get('url')}",
                f"State: `{issue.get('state')}`",
                f"Labels: `{', '.join(issue_labels) or 'none'}`",
                f"Selected maintainer route: `{route['route']}`",
                f"Diagnosis review lane: `${lane['lane']}`",
                f"Repair agent: `{route['repair_agent']}`",
                f"Verifier agent: `{route['verifier_agent']}`",
                f"Review agent: `{route['review_agent']}`",
                "",
                ...
                "",
                "## Issue Body",
                tail(str(issue.get("body") or ""), 12000),
                "",
                "## Required Diagnosis",
                "Before any repair worker starts, diagnose the likely root cause, the safest scoped repair path, tests that should fail before the patch, and risks that must fail closed.",
                "",
                "## Output Contract",
                "Start with exactly one line: `WEBGPT_DIAGNOSIS_BLOCKED: YES` or `WEBGPT_DIAGNOSIS_BLOCKED: NO`.",
                "Then provide:",
                "- suspected root cause",
                "- scoped repair plan",
                "- deterministic tests/checks to add or run",
                "- evidence that would be insufficient",
                "- constraints for the repair worker",
                "",
                "Treat this as advisory diagnosis. Local repository evidence and deterministic proof remain authoritative.",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def receipt_gate(receipt: dict[str, Any], *, phase: str) -> tuple[bool, str]:
    ...
    if phase == "repair" and status == "repaired":
        ...
        files_changed = receipt.get("files_changed") or receipt.get("changed_files") or receipt.get("changed_paths")
    ...
    if phase in {"deterministic_verification", "independent_review"} and not status and decision:
        status = "completed"
    if status != "completed":
        return False, str(status or "missing")
    commands_run = receipt.get("commands_run") or receipt.get("commands")
...
def dispatch_review_repair_retry(
    issue_dir: Path,
    result: dict[str, Any],
    issue: dict[str, Any],
    route: dict[str, str],
    ...
    *,
    ...
    attempt = int(result.get("review_repair_attempts") or 0) + 1
    max_attempts = int(result.get("max_review_repair_attempts") or 2)
    if attempt > max_attempts:
        blocked_status = terminal_block_status("review_repair_attempts_exhausted", review_reason)
        ...
        result["required_next_evidence"] = [
            "manual repair or explicit maintainer retry after review repair attempts exhausted"
        ]
        return terminal_dispose(issue_dir, result, issue, dry_run=dry_run, github_dry_run=github_dry_run)

    repair_packet = load_json(issue_dir / "repair-packet.json", {})
    if not isinstance(repair_packet, dict):
        repair_packet = {}
    repair_packet.update({
        "issue": issue,
        "route": route,
        "target_skills": skills,
        "repair_agent": route["repair_agent"],
        "instruction": (
            "Repair exactly the independent review blocking findings. Preserve unrelated "
            "worktree changes. Return patch and targeted proof receipts."
        ),
        "review_repair_attempt": attempt,
        "review_feedback": {
            "reason": review_reason,
            "receipt": review,
        },
    })
    write_json(issue_dir / "repair-packet.json", repair_packet)

    if dry_run:
        return dry_run_simulation(
            result,
            action="dispatch_repair_after_review_findings",
            would_update={
                "status": "repair_subagent_started",
                "closure_decision": "waiting_for_repair_subagent_receipt",
                "review_repair_attempts": attempt,
                "spec": result["subagent_specs"]["repair_spec"],
            },
        )

    dispatch = maybe_dispatch_subagent(result["subagent_specs"]["repair_spec"], dry_run=dry_run)
    result.setdefault("subagent_dispatch", []).append({"phase": "repair_after_review", **dispatch})
    if int(dispatch.get("returncode", 1)) != 0:
        result["continuation_status"] = "blocked_review_repair_subagent_start_failed"
        result["closure_decision"] = "blocked_review_repair_subagent_start_failed"
        result["status"] = "blocked_review_repair_subagent_start_failed"
        return terminal_dispose(issue_dir, result, issue, dry_run=dry_run, github_dry_run=github_dry_run)

    result["review_repair_attempts"] = attempt
    result["status"] = "repair_subagent_started"
    result["continuation_status"] = "repair_subagent_started_after_review_findings"
    result["closure_decision"] = "waiting_for_repair_subagent_receipt"
    result["terminal_disposition_status"] = "superseded_by_review_repair_retry"
    result["required_next_evidence"] = [
        "repair-response.json from review repair retry",
        "verifier-response.json from independent verifier after retry",
        "review-response.json from independent reviewer after retry",
    ...
    result["repair_session"] = write_repair_started_receipt(
        issue_dir / "repair-response.json",
        issue=issue,
        route=route,
        skills=skills,
        issue_dir=issue_dir,
...
def bundle_needs_evidence_refresh(bundle: Path) -> bool:
    ...
    if any(section not in text for section in required_sections):
        return True
    stale_receipt_fragments = [
        '"verifier": {\n    "status": null',
        '"commands_run": []',
    ]
    return any(fragment in text for fragment in stale_receipt_fragments)


def _repair_receipt_paths(repair: dict[str, Any]) -> list[str]:
    paths: list[str] = []
    for key in ("changed_files", "files_changed", "changed_paths"):
...
def ask_webgpt_diagnosis_command(bundle: Path, lane: dict[str, str]) -> list[str]:
    webgpt_bundle = bundle.with_suffix(".webgpt.md")
    webgpt_bundle.write_text(
        sanitize_webgpt_bundle_text(bundle.read_text(encoding="utf-8", errors="replace")),
        encoding="utf-8",
    ...
    question = "\n\n".join(
        [
            (
                f"Diagnose this skill-maintainer GitHub issue before repair. "
                f"Use the `${lane['lane']}` review lens. Return root cause, scoped repair strategy, "
                "tests to run, and fail-closed risks. Do not claim deterministic proof."
            ),
            webgpt_bundle.read_text(encoding="utf-8", errors="replace"),
        ]
    )
    return [
        ...
        lane["webgpt_project"],
        "--browser-oracle-from",
        lane["browser_oracle_from"],
        "--once",
        "--json",
    ]


def maybe_run_webgpt_diagnosis(bundle: Path, lane: dict[str, str], *, dry_run: bool = False) -> dict[str, Any]:
    cmd = ask_webgpt_diagnosis_command(bundle, lane)
    if dry_run:
        return {"cmd": cmd, "dry_run": True, "returncode": 0, "stdout": "", "stderr": ""}
    cp = run(cmd, check=False)
    return {
...
def continue_pending_run(
    ...
    *,
    ...
    if str(status or "").startswith("blocked_invalid_review_receipt"):
        review = load_json(issue_dir / "review-response.json", {})
        review_ok, review_reason = receipt_gate(review, phase="independent_review")
        if review_ok:
            result["status"] = "review_subagent_started"
            result["continuation_status"] = "resuming_after_repaired_review_gate"
            result["closure_decision"] = "blocked_waiting_for_webgpt_and_closure_wiring"
            result["receipt_gate_reason"] = "pass"
            result["terminal_disposition_status"] = "superseded_by_valid_review_receipt"
            status = result["status"]
        elif terminal_receipt_status(review.get("status")):
            result = dispatch_review_repair_retry(
                issue_dir,
                result,
                issue,
                route,
            ...
            result["continuation_status"] = "waiting_for_review_receipt"
...
def main() -> int:
    ...
    parser.add_argument(
        "--run-diagnosis-webgpt",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Before repair dispatch, run the real $ask WebGPT diagnosis route through browser-oracle binding.",
    )
    parser.add_argument(
    ...
    diagnosis_lane = diagnosis_review_lane(issue, route)
    write_json(issue_dir / "issue-route.json", {
        "issue": issue.get("number"),
        "route": route,
        "diagnosis_review_lane": diagnosis_lane,
    })
    diagnosis_bundle = build_diagnosis_bundle(issue_dir, issue, route, skills)
    diagnosis_result = {
        "phase": "webgpt_diagnosis",
        "status": "blocked_waiting_for_webgpt_diagnosis",
        "review_lane": diagnosis_lane,
        "diagnosis_bundle": str(diagnosis_bundle),
        "command": ask_webgpt_diagnosis_command(diagnosis_bundle, diagnosis_lane),
        "required_next_evidence": [
            "diagnosis-webgpt-result.json from the real $ask WebGPT route",
            "advisory diagnosis reconciled into repair-packet.json",
        ],
    }
    if args.controlled_live_mutation_fixture:
        diagnosis_result["status"] = "not_run"
        diagnosis_result["reason"] = "controlled_live_mutation_fixture_stops_before_webgpt"
    elif github_dry_run and args.run_diagnosis_webgpt:
        diagnosis_run = maybe_run_webgpt_diagnosis(diagnosis_bundle, diagnosis_lane, dry_run=True)
        diagnosis_result = {
            **diagnosis_result,
            **diagnosis_run,
            "status": "dry_run",
        }
    write_json(issue_dir / "diagnosis-webgpt-result.json", diagnosis_result)
    write_json(issue_dir / "repair-packet.json", {
        ...
        "diagnosis": {
            "review_lane": diagnosis_lane,
            "bundle": str(diagnosis_bundle),
            "receipt": str(issue_dir / "diagnosis-webgpt-result.json"),
            "status": diagnosis_result["status"],
            "advisory_only": True,
        },
        "instruction": (
            "Repair exactly this leased issue. Preserve unrelated worktree changes. "
            "Use the WebGPT diagnosis as advisory context only; local repository evidence "
            "and deterministic proof remain authoritative. Return patch and targeted proof receipts."
        ),
    })
    ...
    result = {
        ...
        "require_webgpt": args.require_webgpt,
        "run_diagnosis_webgpt": args.run_diagnosis_webgpt,
        "diagnosis_status": diagnosis_result["status"],
        "diagnosis_bundle": str(diagnosis_bundle),
        "diagnosis_webgpt_artifacts": [str(issue_dir / "diagnosis-webgpt-result.json")],
        ...
        "required_next_evidence": [
            "diagnosis-webgpt-result.json from pre-repair WebGPT diagnosis",
            "repair-response.json from selected repair agent",
    ...
    if diagnosis_result["status"] == "failed":
        result["status"] = "blocked_webgpt_diagnosis_failed"
        result["closure_decision"] = "blocked_webgpt_diagnosis_failed"
        result["required_next_evidence"] = [
            "successful pre-repair WebGPT diagnosis before repair dispatch",
            "diagnosis-webgpt-result.json with returncode 0",
        ]
        write_json(issue_dir / "cycle-result.json", result)
        print(json.dumps(result, indent=2))
        return 2
    ...
    if args.dispatch_subagents:
        diagnosis_result = load_json(issue_dir / "diagnosis-webgpt-result.json", {})
        if args.run_diagnosis_webgpt and diagnosis_result.get("status") == "blocked_waiting_for_webgpt_diagnosis":
            if not github_dry_run:
                ensure_ask_runtime()
            diagnosis_run = maybe_run_webgpt_diagnosis(diagnosis_bundle, diagnosis_lane, dry_run=github_dry_run)
            diagnosis_result = {
                **diagnosis_result,
                **diagnosis_run,
                "status": "dry_run" if diagnosis_run.get("dry_run") else (
                    "completed" if int(diagnosis_run.get("returncode", 1)) == 0 else "failed"
                ),
            }
            write_json(issue_dir / "diagnosis-webgpt-result.json", diagnosis_result)
            result["diagnosis_status"] = diagnosis_result["status"]
            repair_packet = load_json(issue_dir / "repair-packet.json", {})
            if isinstance(repair_packet, dict):
                repair_packet.setdefault("diagnosis", {})["status"] = diagnosis_result["status"]
                write_json(issue_dir / "repair-packet.json", repair_packet)
        if diagnosis_result.get("status") == "failed":
            result["status"] = "blocked_webgpt_diagnosis_failed"
            result["closure_decision"] = "blocked_webgpt_diagnosis_failed"
            result["required_next_evidence"] = [
                "successful pre-repair WebGPT diagnosis before repair dispatch",
