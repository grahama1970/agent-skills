...
def _record_duration(name: str, command: str, result: dict[str, Any]) -> None:
    """Record job execution duration to /memory via estimate_timeout.py.

    Runs in a best-effort subprocess so it never blocks or crashes the scheduler.
    """
    try:
        duration = result.get("duration", 0)
        outcome = result.get("status", "failed")
        script = _COMMON_DIR / "estimate_timeout.py"
        if not script.exists():
            return
        subprocess.Popen(
            [
                "python3", str(script), "record",
                "--skill", name,
        ...
        pass  # never let recording break the scheduler


def _clean_env() -> dict[str, str]:
    """Build a clean environment for child processes.

    Removes VIRTUAL_ENV (which pollutes child venv activation) and ensures
    ~/.local/bin is in PATH (where uv, uvx, and other user tools live).
    """
    env = os.environ.copy()
    ...
    if local_bin not in path.split(os.pathsep):
        env["PATH"] = f"{local_bin}{os.pathsep}{path}"
    return env
...
def run_job(job: dict[str, Any], show_progress: bool = True) -> dict[str, Any]:
    ...
    # Auto-record execution duration to /memory for timeout estimation
    _record_duration(name, command, result)
...
def _rotate_log_if_needed(log_file: Path) -> None:
    """Truncate log file to last N lines if it exceeds size limit."""
    try:
        if log_file.exists() and log_file.stat().st_size > _MAX_LOG_BYTES:
            lines = log_file.read_text().splitlines()
            log_file.write_text("\n".join(lines[-_TAIL_LINES:]) + "\n")
    except OSError:
        pass  # Non-fatal — proceed with append
...
def _write_log(
    ...
    This is best-effort — IO errors (ENOSPC, permission) are caught and
    suppressed so they never propagate up to job_wrapper/save_jobs.
    """
    try:
        ...
        # ENOSPC, permission denied, etc. — never let log IO
        # cascade into save_jobs() corruption (2026-03-04 incident)
        pass


# In-memory cache of last successful load_jobs() — fallback when file is
# corrupt/empty so APScheduler-triggered jobs still execute.
_JOBS_CACHE: dict[str, Any] = {}
_JOBS_CACHE_LOCK = threading.Lock()


def _load_jobs_cached(job_name: str) -> dict[str, Any] | None:
    """Load jobs with in-memory fallback.

    Returns the job dict for job_name, or None if not found.
    Updates the cache on every successful disk read.
    """
    global _JOBS_CACHE
    jobs = load_jobs()

    if jobs:
        # Successful read — update cache
        with _JOBS_CACHE_LOCK:
            _JOBS_CACHE = jobs
        return jobs.get(job_name)

    # Disk read returned empty (corrupt/missing) — try cache
    with _JOBS_CACHE_LOCK:
        if _JOBS_CACHE:
            rprint(
                f"[yellow][scheduler][/yellow] jobs.json unavailable, "
                f"using cached definition for {job_name}"
            )
            return _JOBS_CACHE.get(job_name)
...
def job_wrapper(job_name: str) -> None:
    ...
    """
    job = _load_jobs_cached(job_name)
    if job is None:
    ...
    # Update job metadata — best-effort, don't let it crash the wrapper
    try:
        update_job_run_status(job_name, result["status"], result.get("duration"))
    except Exception:
        rprint(f"[yellow][scheduler][/yellow] Failed to update run status for {job_name}")
