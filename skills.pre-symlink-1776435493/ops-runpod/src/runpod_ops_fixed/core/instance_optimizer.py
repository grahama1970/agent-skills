...
Example Usage:
>>> from runpod_ops_fixed.instance_optimizer import InstanceOptimizer
>>> optimizer = InstanceOptimizer()
...
class InstanceOptimizer:
    ...
    def get_optimal_config(
        ...
        Args:
            ...
            tokens: Total tokens to process (optional, defaults to 10M)
            priority: Optimization priority
            max_budget: Maximum budget in USD
            hours: Alternative to tokens - estimated training hours
        ...
        if tokens is None:
            if hours is not None:
                # Estimate ~1M tokens per hour as rough baseline
                tokens = int(hours * 1_000_000)
            else:
    ...
    def optimize_for_inference(
        ...
        for gpu in self.gpu_configs.values():
            ...
            # Calculate max batch size for KV cache (handle decimal sizes like "1.5B")
            kv_cache_gb_per_token = 0.0001 * float(model_size.rstrip("B"))  # Rough estimate
    ...
    def _get_optimal_batch_size(self, gpu: GPUConfig, model_size: str) -> int:
        ...
        # Rough estimate: 0.5GB per batch for 7B model (handle decimal sizes like "1.5B")
        gb_per_batch = 0.5 * (float(model_size.rstrip("B")) / 7)
