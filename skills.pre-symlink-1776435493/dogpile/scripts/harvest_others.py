...
BRAVE_SCRIPT = Path(__file__).resolve().parent.parent.parent / "brave-search" / "brave_search.py"
...
def search_brave(query: str, monitor: TaskMonitor, count: int = 50, offset: int = 0) -> List[str]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not urls:
                # Debug logging
                # logger.warning(f"Brave returned 0 URLs. Raw stdout: {result.stdout[:200]}...")
                pass
...
if __name__ == "__main__":
    # Adversarial Queries
    adversarial_queries = [
        "filetype:pdf site:archive.org scanned document",
        "filetype:pdf site:loc.gov historical",
        "filetype:pdf site:dtic.mil technical report",
        "filetype:pdf site:ntrs.nasa.gov scanned",
        "filetype:pdf site:haithitrust.org"
    ]
    harvest_pdfs(adversarial_queries, "adversarial_pdfs.txt", "corpus-expansion-adversarial", target_count=500) 
    
    # Finance/Legal Queries
    finance_queries = [
        "filetype:pdf site:sec.gov 10-k",
        "filetype:pdf site:courtlistener.com opinion",
        "filetype:pdf site:law.cornell.edu",
        "filetype:pdf site:eur-lex.europa.eu"
    ]
    harvest_pdfs(finance_queries, "finance_pdfs.txt", "corpus-expansion-finance", target_count=500)
