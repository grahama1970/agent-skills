"""Task Monitor Integration for monitor-taxonomy.

Thin wrapper around shared TaskClient. Preserves TaxonomyMonitorTracker interface.
"""
...
STATE_DIR = Path.home() / ".pi" / "monitor-taxonomy"


class TaxonomyMonitorTracker:
    """Task-monitor compatible tracker for taxonomy quality probes."""

    def __init__(
        self,
        job_type: str = "check",
        name: str = "taxonomy-monitor",
        register: bool = True,
    ):
        ...
        self._client = TaskClient(
            ...
            total=1,
            description=f"Taxonomy Monitor: {job_type}",
...
_trackers: Dict[str, TaxonomyMonitorTracker] = {}


def start_job(job_type: str) -> TaxonomyMonitorTracker:
    tracker = TaxonomyMonitorTracker(job_type=job_type)
    _trackers[job_type] = tracker
    return tracker


def get_tracker(job_type: str) -> Optional[TaxonomyMonitorTracker]:
