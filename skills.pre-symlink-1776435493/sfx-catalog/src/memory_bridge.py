...
class SFXMemoryBridge:
    ...
    def ingest_catalog(self, manifest: dict) -> bool:
        ...
        Args:
            manifest: Catalog data with 'items' list
    ...
    def search_sfx(
        ...
        Args:
            ...
            categories: Optional category filters (post-filter)
            duration_range: Optional (min, max) duration filter (post-filter)
        ...
        try:
            data = _memory_cmd([
                "recall", "--q", query,
                "--scope", "sfx_library",
                "--k", str(k * 3),  # over-fetch for post-filtering
            ])
            items = data.get("items", [])

            # Post-filter by category and duration
            results = []
            for item in items:
                # Try to parse solution JSON (stored as JSON string)
                doc = item
