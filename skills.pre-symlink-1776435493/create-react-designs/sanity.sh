...
echo "=== [create-react-designs] Sanity Check ==="
...
for script in scripts/create.mjs scripts/iterate.mjs scripts/loop.sh; do
...
# --- Python syntax check on council.py ---
if [[ -f "$SCRIPT_DIR/scripts/council.py" ]]; then
  if python3 -c "import ast; ast.parse(open('$SCRIPT_DIR/scripts/council.py').read())" 2>/dev/null; then
    echo "OK: scripts/council.py parses"
  else
    echo "FAIL: scripts/council.py has syntax errors"
    ERRORS=$((ERRORS + 1))
  fi
else
  echo "WARN: scripts/council.py not found"
  WARNINGS=$((WARNINGS + 1))
fi
