"""judge_deepseek - train_convo_steering.
...
# ---------------------------------------------------------------------------
_use_gateway = os.environ.get("STEERING_USE_GATEWAY", "1") == "1"
...
def judge_enabled() -> bool:
    return os.getenv("DEEPSEEK_JUDGE_ENABLED", "0").lower() in ("1", "true", "yes")
...
async def judge_best_preset(
    ...
    if _gateway_available:
        try:
            preset_list = [{"id": p.preset_id, "desc": p.description} for p in presets]
            gw_result = _gw_validate(
                input_data={"user_text": user_text, "presets": preset_list},
                task="steering-preset-ranker",
            )
            if gw_result and gw_result.result:
                obj = gw_result.result
                if validate_judge_response(obj, presets):
                    return obj
        except Exception as e:
            logger.debug("gateway judge call failed, falling through to scillm: {}", e)
    ...
    api_base = os.getenv("SCILLM_API_BASE", "http://localhost:4001").rstrip("/")
    api_key = os.getenv("SCILLM_PROXY_KEY", "sk-dev-proxy-123")
    model = os.getenv("DEEPSEEK_MODEL", os.getenv("CHUTES_TEXT_MODEL", "deepseek-v3"))

    if not api_key:
        logger.warning("DeepSeek judge enabled but missing SCILLM_PROXY_KEY")
        return None
