...
for module in "${MODULES[@]}"; do
    if [[ -f "$SKILL_DIR/$module" ]]; then
        # Check line count is under 800 (project max, except config which has data)
        ...
        if [[ "$module" == "config.py" ]]; then
            ...
            if [[ $lines -lt 800 ]]; then
                ...
                fail "$module exceeds 800 lines ($lines lines)"
            fi
        fi
    else
        fail "$module missing"
...
# Check legacy monolith backup exists
if [[ -f "$SKILL_DIR/paper_writer_monolith.py.bak" ]]; then
    pass "paper_writer_monolith.py.bak backup exists"
else
    warn "paper_writer_monolith.py.bak backup missing"
fi
...
UVX_CMD="uvx --with typer --with loguru --with httpx"
