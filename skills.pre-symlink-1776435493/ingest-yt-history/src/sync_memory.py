...
def format_memory_entry(
    ...
    if "domain" in collection_tags:
        domain_val = collection_tags["domain"]
        domain_str = ", ".join(domain_val) if isinstance(domain_val, list) else str(domain_val)
        solution_parts.append(f"Domain: {domain_str}.")

    # Add thematic weight
    if "thematic_weight" in collection_tags:
        thematic_val = collection_tags["thematic_weight"]
        thematic_str = ", ".join(thematic_val) if isinstance(thematic_val, list) else str(thematic_val)
        solution_parts.append(f"Thematic: {thematic_str}.")
    ...
    if "thematic_weight" in collection_tags:
        thematic_val = collection_tags["thematic_weight"]
        if isinstance(thematic_val, list):
            for t in thematic_val:
                memory_tags.append(t.lower())
        else:
            memory_tags.append(str(thematic_val).lower())
...
def main(
    input_path: str = typer.Argument(..., help="Path to parsed YouTube history JSONL file"),
    output: str = typer.Option(None, "-o", "--output", help="Output JSONL file path (default: stdout)"),
    include_all: bool = typer.Option(False, "--all", help="Include entries without taxonomy data"),
    stats: bool = typer.Option(False, "--stats", help="Print statistics after sync"),
) -> None:
    """Sync YouTube music history to memory-compatible JSONL."""
    try:
        if output:
