...
from typing import Any, Iterator, Optional, TextIO, TypedDict
...
def main(
    command: str = typer.Argument("build", help="Command to run: build"),
    input_path: Optional[str] = typer.Argument(None, help="Path to parsed YouTube history JSONL file"),
    output: Optional[str] = typer.Option(None, "-o", "--output", help="Output JSON file path (default: ~/.pi/ingest-yt-history/profile.json)"),
    include_all: bool = typer.Option(False, "--all", help="Include non-music entries"),
    top: int = typer.Option(10, "-n", "--top", help="Number of top items per category (default: 10)"),
    as_json: bool = typer.Option(False, "--json", help="Output profile to stdout as JSON"),
) -> None:
    """Build taste profile from YouTube music history."""
    if command == "build":
        if not input_path:
            print("Error: input_path is required for build command", file=sys.stderr)
        ...
        try:
            ...
            if as_json:
                ...
                output_loc = output or str(Path.home() / ".pi" / "ingest-yt-history" / "profile.json")
