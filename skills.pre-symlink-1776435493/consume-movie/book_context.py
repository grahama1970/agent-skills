...
def main(
    ...
    scene: Optional[str] = typer.Option(None, help="Scene text to find relevant notes for"),
):
    """CLI for testing book context."""

    context = get_book_notes(movie)

    if scene:
        scene_context = get_book_context_for_scene(movie, scene, context)
        if json:
            print(json.dumps(scene_context, indent=2, default=str))
        else:
            console.print(f"\n[bold]Scene:[/bold] {scene[:100]}...")
