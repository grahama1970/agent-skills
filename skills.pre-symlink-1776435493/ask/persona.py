...
def store_persona_profile(
    ...
    # Use memory-agent CLI directly for speed
    ...
    if MEMORY_AGENT_CLI.exists():
        ...
        try:
            ...
            log.error("Failed to store persona profile: %s", e)
            return False
...
def run_memory_recall(query: str, scope: str, k: int = 5, timeout: int = 15) -> dict:
    """Run memory recall using memory-agent CLI directly (faster than run.sh)."""
    MEMORY_AGENT_CLI = Path(os.path.expanduser("~/.local/bin/memory-agent"))
    ...
    if MEMORY_AGENT_CLI.exists():
        try:
            ...
            return {"returncode": -2, "stdout": "", "stderr": f"memory-agent timed out ({timeout}s)", "skipped": False}
        except Exception as e:
            log.error("memory-agent recall failed: %s", e)
