...
def store_qra_to_memory(
    ...
    if not MEMORY_AGENT_CLI.exists():
        # Fall back to memory skill's run.sh
        log.info("Using memory run.sh (memory-agent CLI not found)")
        for qra in qras:
    ...
    for qra in qras:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                stats["errors"] += 1
                log.debug("Memory store failed for %r: %s", problem[:60], result.stderr[:100])
        except (subprocess.TimeoutExpired, OSError) as e:
            stats["errors"] += 1
            log.debug("Memory store error: %s", e)
