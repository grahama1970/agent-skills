...
def generate_sankey_diagram(
    ...
    """
    if check_plotly():
        return generate_plotly_sankey(flows, output_path, title, format, canvas=canvas)
...
def generate_plotly_sankey(
    ...
    if canvas:
        from canvas import wrap_plotly_canvas
        html = wrap_plotly_canvas(fig, title=title, source="/create-figure sankey")
...
def generate_sunburst(
    ...
    if canvas:
        from canvas import wrap_plotly_canvas
        html = wrap_plotly_canvas(fig, title=title, source="/create-figure sunburst")
        output_path.with_suffix(".html").write_text(html)
    elif format == "html":
        fig.write_html(str(output_path))
    else:
...
def generate_treemap(
    ...
    """
    if check_plotly():
        return generate_plotly_treemap(data, output_path, title, format, canvas=canvas)
    elif check_squarify():
...
def generate_plotly_treemap(
    ...
    if canvas:
        from canvas import wrap_plotly_canvas
        html = wrap_plotly_canvas(fig, title=title, source="/create-figure treemap")
        output_path.with_suffix(".html").write_text(html)
    elif format == "html":
        fig.write_html(str(output_path))
    else:
...
def generate_parallel_coordinates(
    ...
    if check_plotly():
        ...
        if canvas:
            from canvas import wrap_plotly_canvas
            html = wrap_plotly_canvas(fig, title=title, source="/create-figure parallel-coords")
            output_path.with_suffix(".html").write_text(html)
        elif format == "html":
            fig.write_html(str(output_path))
        else:
