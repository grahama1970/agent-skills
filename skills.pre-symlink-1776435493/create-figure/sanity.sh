#!/usr/bin/env bash
# Sanity check for create-figure skill (modularized version)
...
echo "=== Create-Figure Sanity Check (Modular) ==="
echo ""
...
if [[ -d "tests" ]]; then
    if command -v pytest &>/dev/null; then
        echo "Running pytest..."
        pytest tests/ -v --tb=short 2>/dev/null && echo "Tests: OK" || { echo "Tests: FAILED"; exit 1; }
    else
