...
TOP_BLOCKER: No package command exercises the real /api/projects/watch/yolo-labels disk receipt through accept, stop, reload readback, and reassignment.
NEXT_ACTION: Add one deterministic smoke that launches the existing server with isolated environment paths using Node’s child-process API, then expose it through npm scripts; npm package scripts make local binaries such as tsx available on PATH. 
Node.js
+1

LIVE_STOP_CONDITION: npm --prefix skills/watch/ui run test:yolo-label-receipt, npm --prefix skills/watch/ui test, and npm --prefix skills/watch/ui run typecheck all exit 0.
...
-    "test": "tsx scripts/watchAnnotationSession.smoke.ts && tsx scripts/watchYoloSequenceProjection.smoke.ts",
+    "test": "tsx scripts/watchAnnotationSession.smoke.ts && tsx scripts/watchYoloSequenceProjection.smoke.ts && tsx scripts/watchYoloLabelReceiptReplay.smoke.ts",
+    "test:yolo-label-receipt": "tsx scripts/watchYoloLabelReceiptReplay.smoke.ts",
     "typecheck": "tsc --noEmit"
   },
   "dependencies": {
diff --git a/skills/watch/ui/scripts/watchYoloLabelReceiptReplay.smoke.ts b/skills/watch/ui/scripts/watchYoloLabelReceiptReplay.smoke.ts
new file mode 100644
--- /dev/null
+++ b/skills/watch/ui/scripts/watchYoloLabelReceiptReplay.smoke.ts
@@ -0,0 +1,252 @@
...
<<<WEBGPT_DONE:20260715T171858Z:fa691598>>>_
