...
def search_literature(scope: PaperScope, analysis: ProjectAnalysis) -> LiteratureReview:
    ...
    if not scope.contributions:
        logger.error("No contributions defined - cannot generate context")
        raise typer.Exit(1)

    if not ARXIV_SCRIPT.exists():
        logger.error(f"Skill script not found: {ARXIV_SCRIPT}")
        raise typer.Exit(1)
...
"""
    ...
    try:
        ...
        logger.error(f"Failed to parse arxiv output as JSON: {e}")
    except (subprocess.CalledProcessError, subprocess.TimeoutExpired) as e:
        if isinstance(e, subprocess.CalledProcessError):
            logger.error(f"/arxiv search failed with exit code {e.returncode}")
            if e.stderr:
                logger.error(f"stderr: {e.stderr}")
        else:
            logger.error("/arxiv search timed out after 90s")
...
def learn_from_papers(review: LiteratureReview, scope: PaperScope) -> LiteratureReview:
    ...
    if not ARXIV_SCRIPT.exists():
        logger.error(f"Skill script not found: {ARXIV_SCRIPT}")
        logger.error("Cannot proceed without arxiv skill")
        return review
    ...
    for i, paper_id in enumerate(review.papers_selected, 1):
        ...
        try:
            ...
            logger.error("Timed out after 5 min")
    ...
    if success_count == 0:
        logger.warning("No papers successfully extracted. Draft will use stub content.")
