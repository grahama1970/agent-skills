...
def analyze_project(
    ...
    if not ASSESS_SCRIPT.exists():
        logger.error(f"Skill script not found: {ASSESS_SCRIPT}")
        raise typer.Exit(1)
...
def _run_assess(project_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"/assess failed with exit code {e.returncode}")
        if e.stderr:
        ...
        logger.error(f"Failed to parse assess output as JSON: {e}")
        return {}
...
def _run_dogpile_research(contributions: list) -> str:
    ...
    for contrib in contributions[:2]:  # Limit to 2 to avoid rate limits
        ...
        try:
            ...
            logger.warning(f"Dogpile failed for '{contrib}' with exit code {e.returncode}")
            if e.stderr:
                logger.warning(f"stderr: {e.stderr}")
            research_parts.append(f"## {contrib}\n(Research failed)")
        except subprocess.TimeoutExpired:
            logger.warning(f"Dogpile timed out for '{contrib}' after 120s")
            research_parts.append(f"## {contrib}\n(Research failed)")
...
def _run_code_review_alignment(
    ...
    if not CODE_REVIEW_SCRIPT.exists():
        logger.warning("Code-review skill not found, skipping alignment check")
...
"""
        ...
        if result.returncode == 0:
            ...
            logger.warning(f"Code-review returned non-zero: {result.returncode}")
            return f"Code-review failed: {result.stderr}"
    except subprocess.TimeoutExpired:
        logger.warning("Code-review timed out after 5 min")
