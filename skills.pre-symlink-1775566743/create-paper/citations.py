...
def verify_arxiv_id(arxiv_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        response = httpx.get(url, headers={"User-Agent": "create-paper/1.0"}, timeout=10)
        ...
        logger.warning(f"arXiv HTTP error for {clean_id}: {e.response.status_code}")
        return {
            "status": "Unsupported",
            "source": "arXiv",
            "arxiv_id": clean_id,
        ...
        logger.error(f"arXiv verification failed for {clean_id}: {e}")
        return {
            "status": "Uncertain",
            "source": "arXiv",
            "arxiv_id": clean_id,
...
def verify_doi(doi: str) -> Dict[str, Any]:
    ...
    try:
        ...
        response = httpx.get(url, headers={
            "User-Agent": "create-paper/1.0 (mailto:contact@example.com)",
        }, timeout=10)
        response.raise_for_status()
        data = response.json()
        ...
        logger.warning(f"CrossRef HTTP error for {clean_doi}: {e.response.status_code}")
        return {
            "status": "Uncertain",
            "source": "CrossRef",
            "doi": clean_doi,
        ...
        logger.error(f"CrossRef verification failed for {clean_doi}: {e}")
        return {
            "status": "Uncertain",
            "source": "CrossRef",
            "doi": clean_doi,
...
def verify_semantic_scholar(title: str, authors: List[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 429:
            logger.warning(f"Semantic Scholar rate limited for: {title[:50]}")
            return {
                "status": "Uncertain",
                "source": "Semantic Scholar",
                "searched_title": title,
        ...
        logger.error(f"Semantic Scholar verification failed for: {title[:50]}: {e}")
        return {
            "status": "Uncertain",
            "source": "Semantic Scholar",
            "searched_title": title,
