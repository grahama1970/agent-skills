...
Example Usage:
>>> from runpod_ops_fixed.instance_monitor import InstanceMonitor
>>> monitor = InstanceMonitor()
...
class InstanceMonitor:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # SSH manager for real metrics
        self.ssh_manager = SSHManager(api_key)
        # Monitoring data
    ...
    async def check_instance_health(self, instance_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Try to get real GPU metrics via SSH
            try:
                ssh_metrics = await self.ssh_manager.monitor_training(instance_id)
                if ssh_metrics.get("gpus"):
                    gpu = ssh_metrics["gpus"][0]  # First GPU
                    health["gpu_utilization"] = gpu.get("utilization", 0)
                    health["gpu_memory_used_gb"] = gpu.get("memory_used", 0) / 1024
                    health["gpu_memory_total_gb"] = gpu.get("memory_total", 0) / 1024
                    health["training_running"] = ssh_metrics.get("training_running", False)
                    health["disk_usage_percent"] = ssh_metrics.get("disk_usage_percent", 0)
                    health["latest_checkpoint"] = ssh_metrics.get("latest_checkpoint")
                else:
                    # Fallback to pod info if SSH not available
                ...
                logger.debug(f"Could not get SSH metrics: {e}")
                # Use basic pod info
                gpu_metrics = pod_info.get("gpus", [])
                if gpu_metrics:
                    gpu = gpu_metrics[0]
    ...
    async def _get_instance_logs(self, instance_id: str, lines: int = 100) -> Optional[str]:
        ...
        try:
            ...
            result = await self.ssh_manager.execute_on_pod(
                instance_id,
                f"tail -n {lines} /workspace/training.log 2>/dev/null || tail -n {lines} /var/log/jupyter.log 2>/dev/null"
            )
            ...
            result = await self.ssh_manager.execute_on_pod(
                instance_id,
                "find /workspace -name '*.log' -type f -exec tail -n 50 {} \\; 2>/dev/null | head -n 200"
            )
    ...
    def _parse_training_progress(self, logs: str, pattern: str = "loss:") -> Dict[str, Any]:
        """Parse training progress from logs."""
        progress = {}
        ...
        # Multiple loss patterns for different frameworks
        loss_patterns = [
            rf"{pattern}\s*([\d.]+)",  # User specified pattern
            r"loss[:\s]+([\d.]+)",      # Generic loss
            r"train_loss[:\s]+([\d.]+)", # Training loss
            r"eval_loss[:\s]+([\d.]+)",  # Evaluation loss
            r"'loss':\s*([\d.]+)",      # JSON format
        ]
        
        for loss_pattern in loss_patterns:
            matches = re.findall(loss_pattern, logs, re.IGNORECASE)
            if matches:
                progress["loss"] = float(matches[-1])
                break
                
        # Look for learning rate
        lr_pattern = r"learning_rate[:\s]+([\d.e-]+)"
        lr_matches = re.findall(lr_pattern, logs, re.IGNORECASE)
        if lr_matches:
            progress["learning_rate"] = float(lr_matches[-1])
        ...
        eval_patterns = {
            "accuracy": r"(?:eval_)?accuracy[:\s]+([\d.]+)",
            "perplexity": r"(?:eval_)?perplexity[:\s]+([\d.]+)",
            "bleu": r"(?:eval_)?bleu[:\s]+([\d.]+)",
            "rouge": r"(?:eval_)?rouge[:\s]+([\d.]+)",
        }
        
        for metric, pattern in eval_patterns.items():
            matches = re.findall(pattern, logs, re.IGNORECASE)
            if matches:
                progress[f"eval_{metric}"] = float(matches[-1])
                
        # Look for checkpoint saves
        checkpoint_pattern = r"Saving checkpoint.*?(\d+)"
        checkpoint_matches = re.findall(checkpoint_pattern, logs, re.IGNORECASE)
        if checkpoint_matches:
            progress["last_checkpoint_step"] = int(checkpoint_matches[-1])
            
        # Look for memory usage
        memory_pattern = r"allocated[:\s]+([\d.]+)\s*GB"
        memory_matches = re.findall(memory_pattern, logs, re.IGNORECASE)
        if memory_matches:
            progress["gpu_memory_allocated_gb"] = float(memory_matches[-1])
    ...
    async def monitor_fine_tuning_job(
        self,
        instance_id: str,
        ...
        """
        Monitor a fine-tuning job with detailed progress tracking.
        ...
        Args:
            ...
            check_interval: Seconds between checks
        """
        logger.info(f"Starting fine-tuning monitor for {instance_id}")
        ...
        while True:
            try:
                ...
                # Get training logs
                logs = await self._get_instance_logs(instance_id, lines=500)
                
                # Parse progress
                progress = self._parse_training_progress(logs) if logs else {}
                
                # Check for new checkpoints
                checkpoint_info = await self._check_checkpoints(instance_id, output_dir)
                if checkpoint_info.get("latest_checkpoint") != last_checkpoint:
                    last_checkpoint = checkpoint_info["latest_checkpoint"]
                    progress["new_checkpoint"] = last_checkpoint
                    
                # Track best model
                if progress.get("eval_loss") and progress["eval_loss"] < best_metric:
                    best_metric = progress["eval_loss"]
                    progress["is_best_model"] = True
                    
                # Check TensorBoard logs
                tb_metrics = await self._check_tensorboard_logs(instance_id)
                if tb_metrics:
                    progress.update(tb_metrics)
                    
                # Calculate training speed and ETA
                if progress.get("step") and progress.get("total_steps"):
                    elapsed_hours = (time.time() - start_time) / 3600
                    steps_per_hour = progress["step"] / elapsed_hours
                    remaining_steps = progress["total_steps"] - progress["step"]
                    eta_hours = remaining_steps / steps_per_hour if steps_per_hour > 0 else 0
                    
                    progress["steps_per_hour"] = int(steps_per_hour)
                    progress["eta_hours"] = round(eta_hours, 2)
                    progress["progress_percent"] = round(progress["step"] / progress["total_steps"] * 100, 1)
                    
                # Get model size if available
                model_size = await self._get_model_size(instance_id, output_dir)
                if model_size:
                    progress["model_size_gb"] = model_size
                ...
                update = {
                    ...
                    "gpu_utilization": health.get("gpu_utilization", 0),
                    "gpu_memory_used_gb": health.get("gpu_memory_used_gb", 0),
                    "best_eval_loss": best_metric if best_metric != float('inf') else None,
                    "total_cost": health.get("cost_so_far", 0)
                }
                ...
                if await self._check_training_complete(instance_id, logs):
                    ...
                    final_model = await self._get_final_model_info(instance_id, output_dir)
                    update["final_model"] = final_model
    ...
    async def _check_checkpoints(self, instance_id: str, output_dir: str) -> Dict[str, Any]:
        """Check for saved checkpoints."""
        try:
            result = await self.ssh_manager.execute_on_pod(
                instance_id,
                f"find {output_dir} -name 'checkpoint-*' -type d | sort -V"
            )
            ...
            if result["success"] and result["stdout"]:
                ...
                if checkpoints:
                    size_result = await self.ssh_manager.execute_on_pod(
                        instance_id,
                        f"du -sh {checkpoints[-1]} | cut -f1"
                    )
                    if size_result["success"]:
                        checkpoint_info["latest_checkpoint_size"] = size_result["stdout"].strip()
            ...
            logger.debug(f"Could not check checkpoints: {e}")
            return {"total_checkpoints": 0, "checkpoints": []}
            
    async def _check_tensorboard_logs(self, instance_id: str) -> Dict[str, Any]:
        """Check TensorBoard logs for metrics."""
        try:
            # Look for TensorBoard event files
            result = await self.ssh_manager.execute_on_pod(
                instance_id,
                "find /workspace/logs -name 'events.out.tfevents.*' -type f | head -1"
            )
            
            if result["success"] and result["stdout"]:
                # TensorBoard logs exist
                # In production, would parse these properly
                return {"tensorboard_available": True}
                
            return {}
            
        except Exception as e:
            logger.debug(f"Could not check TensorBoard logs: {e}")
            return {}
            
    async def _get_model_size(self, instance_id: str, output_dir: str) -> Optional[float]:
        """Get size of saved model in GB."""
        try:
            result = await self.ssh_manager.execute_on_pod(
                instance_id,
                f"find {output_dir} -name '*.bin' -o -name '*.safetensors' | xargs du -c | tail -1 | awk '{{print $1/1024/1024}}'"
            )
            ...
            logger.debug(f"Could not get model size: {e}")
            return None
            
    async def _get_final_model_info(self, instance_id: str, output_dir: str) -> Dict[str, Any]:
        """Get information about the final trained model."""
        try:
            model_info = {}
            ...
            result = await self.ssh_manager.execute_on_pod(
                instance_id,
                f"ls -la {output_dir}/*/pytorch_model.bin {output_dir}/*/model.safetensors 2>/dev/null | tail -5"
            )
            
            if result["success"] and result["stdout"]:
                model_info["model_files"] = result["stdout"].strip()
                
            # Check for config
            config_result = await self.ssh_manager.execute_on_pod(
                instance_id,
                f"cat {output_dir}/*/config.json 2>/dev/null | jq -r '.model_type' | head -1"
            )
            
            if config_result["success"] and config_result["stdout"]:
                model_info["model_type"] = config_result["stdout"].strip()
                
            # Check for tokenizer
            tokenizer_result = await self.ssh_manager.execute_on_pod(
                instance_id,
                f"ls {output_dir}/*/tokenizer*.json 2>/dev/null | wc -l"
            )
            
            if tokenizer_result["success"] and tokenizer_result["stdout"]:
                model_info["has_tokenizer"] = int(tokenizer_result["stdout"].strip()) > 0
                
            # Get total size
            size_result = await self.ssh_manager.execute_on_pod(
                instance_id,
                f"du -sh {output_dir} | cut -f1"
            )
            
            if size_result["success"]:
                model_info["total_size"] = size_result["stdout"].strip()
