...
@app.command()
def run(
    continue_on_error: bool = typer.Option(False, help="Continue on transcription errors"),
    ...
    monitor = TaskClient("ingest-audiobook", total=len(pending_books)) if TaskClient else None
...
@app.command()
def status(
    as_json: bool = typer.Option(False, "--json", help="Output JSON for agent parsing"),
    verbose: bool = typer.Option(False, "-v", "--verbose", help="Show all books"),
):
    """Show current status."""
    state = PipelineState.load()
...
@app.command()
def reset(
    logs: bool = typer.Option(False, help="Also remove log file"),
):
    """Reset state file."""
    if STATE_FILE.exists():
        STATE_FILE.unlink()
        print(f"Removed {STATE_FILE}")
    if logs and LOG_FILE.exists():
        LOG_FILE.unlink()
...
@app.command()
def revalidate():
    """Check all transcriptions and re-queue incomplete ones."""
    Console, *_ = get_rich()
    console = Console()
    ...
    for book_name, book in state.books.items():
        ...
        if book.status == "completed":
            if not transcript.exists():
                ...
                console.print(f"[green]{book_name[:50]}[/green]")
    ...
    if requeued:
        ...
        for name, reason in requeued:
            console.print(f"  [yellow]{name[:50]} ({reason})[/yellow]")
