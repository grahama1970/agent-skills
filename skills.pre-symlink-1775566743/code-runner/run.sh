...
case "${1:-help}" in
    run)
        ...
        exec uv run --project "$SCRIPT_DIR" python "$SCRIPT_DIR/code_runner.py" run "$spec_file" "$@"
        ;;
    dry-run)
        shift
        spec_file="${1:-}"
        [[ -n "$spec_file" && -f "$spec_file" ]] || { echo "Usage: ./run.sh dry-run <task-spec.json>" >&2; exit 1; }
        exec uv run --project "$SCRIPT_DIR" python "$SCRIPT_DIR/code_runner.py" dry-run "$spec_file"
        ;;
    review)
        shift
        review_file="${1:-}"
...
"
        ;;
    help|--help|-h)
        echo "Usage: ./run.sh {run|dry-run|review|result|help}"
        echo ""
        echo "Commands:"
        echo "  run <spec.json>       Run task with self-improvement loop"
        echo "  dry-run <spec.json>   Show what would execute"
