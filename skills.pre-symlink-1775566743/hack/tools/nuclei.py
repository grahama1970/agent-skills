...
def nuclei_command(
    ...
    add_task_accomplishment(f"Starting Nuclei scan on {target} ({profile})")
    ...
    if severity is None:
        severity = ",".join(profile_cfg["nuclei_severity"])
    if rate_limit is None:
        # Scale rate limit by profile level (higher level = slower/stealthier)
        # Levels: 1 (noisy/fast) to 4 (stealthy/slow)
        rate_limit = profile_cfg.get("rate_limit", 150 - (profile_cfg["level"] - 1) * 30)
...
def create_nuclei_typer_command() -> Callable[..., None]:
    ...
    def nuclei(
        ...
        rate_limit: int = typer.Option(
            None, help="Maximum requests per second (default varies by profile)"
        ),
        ...
        start_task_session(project=f"nuclei-{target}")
        try:
            nuclei_command(target, templates, severity, profile, rate_limit, output, recall)
            end_task_session(notes=f"Nuclei scan complete for {target}")
        except Exception as e:
            end_task_session(notes=f"Nuclei scan failed: {e}")
            raise e
