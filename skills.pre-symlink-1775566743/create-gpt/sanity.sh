...
if [[ -d "$scripts_dir" ]]; then
    ...
    for f in "$scripts_dir"/rewards/*.py; do
        [[ ! -f "$f" ]] && continue
        if ! python3 -c "import py_compile; py_compile.compile('$f', doraise=True)" 2>/dev/null; then
            echo ""
            echo "  WARN: syntax issue in rewards/$(basename "$f")"
            ((fail_count++)) || true
        fi
    done
...
if echo "$output" | grep -qi "train\|gpt\|task"; then
    echo "OK"
else
...
# Check 6: Symlinks
echo -n "Check 6: Symlinks... "
for link in data models logs; do
    ...
    echo "WARN - $link symlink missing or broken"
done
echo "OK"
