/**
 * D3 Manipulation Runtime — injected into every D3 visualization iframe.
 *
 * Listens for postMessage({type: "d3-manipulate", commands: [...]}) and
 * dispatches to per-action handlers. Supports all 16 ManipulationActions
 * across all VizFamily types (NETWORK, HIERARCHY, FLOW, BAR, PIE, etc.).
 *
 * Actions: zoom_in, zoom_out, zoom_to, pan, highlight, highlight_related,
 *          highlight_path, focus, expand, collapse, drill_down, drill_up,
 *          filter, explode, brush, reset
 */
(function() {
  'use strict';
  ...
  var initRetries = 0;
  var MAX_INIT_RETRIES = 50; // 10 seconds max
  var trustedOrigin = null; // Set on first valid message
  ...
  function init() {
    ...
    if (!svg) {
      if (++initRetries < MAX_INIT_RETRIES) {
        ...
        window.parent.postMessage({type: 'd3-manipulate-error', message: 'No SVG found after init timeout'}, '*');
      }
      return;
    }
      ...
      zoomBehavior = d3.zoom()
        ...
        .on('zoom', function(event) {
          ...
          if (!g) {
            ...
            // Move visual children only (skip defs, style, clipPath)
            var skip = {'defs':1, 'style':1, 'clipPath':1, 'linearGradient':1, 'radialGradient':1, 'filter':1, 'marker':1, 'pattern':1};
            var children = Array.from(svg.children);
            for (var i = 0; i < children.length; i++) {
              if (children[i] !== g && !skip[children[i].tagName.toLowerCase()]) {
  ...
  var handlers = {
    ...
    highlight_path: function(cmd) {
      ...
      allElements.forEach(function(el) { el.classList.add('d3m-dimmed'); el.style.opacity = '0.1'; });
    ...
    filter: function(cmd) {
      if (!svg || !cmd.target) return;
      // Hide elements NOT matching the filter — track via CSS class for reliable reset
    ...
    reset: function() {
      ...
      // Show all filtered/hidden elements
      svg.querySelectorAll('.d3m-filtered').forEach(function(el) {
        el.classList.remove('d3m-filtered');
        el.style.removeProperty('display');
        el.style.removeProperty('opacity');
      });
    }
  ...
  function findElement(target) {
    ...
    // By text content in <text> elements — prefer exact match over substring
    ...
    for (var i = 0; i < texts.length; i++) {
      ...
      if (!substringMatch && textContent.includes(t)) {
        substringMatch = texts[i].closest('g.node, g.bar, g.slice, g.arc, g.link, g') || texts[i];
      }
    }
  ...
  function findRelated(targetEl, targetName) {
    ...
    svg.querySelectorAll('line, path.link, g.link path, g.edge path').forEach(function(el) {
      ...
      var tgt = (el.getAttribute('data-target') || '').toLowerCase();
      if (source === t || tgt === t || source.includes(t) || tgt.includes(t)) {
        related.push(el);
        // Also find the other endpoint node
        var otherName = source === t || source.includes(t) ? tgt : source;
  ...
  window.addEventListener('message', function(event) {
    ...
    // Trust the first sender's origin, reject others
    if (!trustedOrigin) {
      trustedOrigin = event.origin;
    } else if (event.origin !== trustedOrigin) {
      console.warn('[d3-manipulate] Rejected message from untrusted origin:', event.origin);
      return;
    }
    ...
    // ACK back to parent — use sender's origin, not '*'
    if (event.source) {
      event.source.postMessage({
        type: 'd3-manipulate-ack',
        executed: executed,
        success: executed.length === (msg.commands || []).length
      }, event.origin);
    }
  });
