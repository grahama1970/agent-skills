...
_MEMORY_PERSONA_PATH = Path(__file__).resolve().parent.parent.parent.parent.parent / "memory" / "persona"
...
# Merge AUDIO_BRIDGE_THRESHOLDS into BRIDGE_INDICATORS for calculate_bridge_score
# Split thresholds into indicators (rhythm/harmony/dynamics) and audio_patterns (timbre)
_INDICATOR_KEYS = {
    "tempo_variance", "time_signature", "harmonic_complexity",
    "beat_strength", "mode", "dynamic_range",
}
_AUDIO_PATTERN_KEYS = {
    "brightness", "texture", "spectral_flatness",
    "spectral_centroid", "zero_crossing_rate", "spectral_contrast",
}

# Default descriptions for bridges that lack them
_BRIDGE_DESCRIPTIONS = {
    "Precision": "Technical complexity, polyrhythmic mastery",
    "Resilience": "Triumphant building intensity, enduring strength",
    "Fragility": "Delicate vulnerability, sparse tenderness",
    "Corruption": "Distorted harshness, industrial decay",
    "Loyalty": "Ceremonial solemnity, ritual devotion",
    "Stealth": "Ambient subtlety, minimal presence",
}

# Always merge audio thresholds (both canonical and fallback need structured data)
for _bridge, _thresholds in AUDIO_BRIDGE_THRESHOLDS.items():
    if _bridge in BRIDGE_INDICATORS:
        _ind = {}
        _pat = {}
        for _k, _v in _thresholds.items():
            if _k in _INDICATOR_KEYS:
                _ind[_k] = _v
            elif _k in _AUDIO_PATTERN_KEYS:
                _pat[_k] = _v
        # Store text indicators separately, replace with structured dict
        if "text_indicators" not in BRIDGE_INDICATORS[_bridge]:
            _orig = BRIDGE_INDICATORS[_bridge].get("indicators")
            if isinstance(_orig, list):
                BRIDGE_INDICATORS[_bridge]["text_indicators"] = _orig
        BRIDGE_INDICATORS[_bridge]["indicators"] = _ind
        BRIDGE_INDICATORS[_bridge]["audio_patterns"] = _pat
        if "description" not in BRIDGE_INDICATORS[_bridge]:
            BRIDGE_INDICATORS[_bridge]["description"] = _BRIDGE_DESCRIPTIONS.get(
                _bridge, f"Audio bridge: {_bridge}"
            )
