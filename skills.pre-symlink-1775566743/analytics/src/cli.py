...
def chart(
    ...
    d3: bool = typer.Option(False, "--d3", help="Render interactive D3 HTML via create-figure d3_backend"),
    discord: bool = typer.Option(False, "--discord", help="Optimize D3 output for Discord phone (500px)"),
    canvas: bool = typer.Option(False, "--canvas", help="Wrap D3 output in 5ft answer canvas"),
):
    ...
    D3 interactive output:
        ...
        ./run.sh chart data.jsonl --name distribution_channel --d3 --canvas -o chart.html
    """
    try:
        df = load_data(path)
        ...
        # D3 interactive rendering path
        if d3:
            _render_d3_chart(spec, chart_type, name, output, canvas=canvas, discord=discord)
            return
...
def _render_d3_chart(
    ...
    """Render chart spec as interactive D3 HTML via create-figure d3_backend.

    Uses the viz-type-selector classifier to pick optimal D3 viz type,
    then renders via d3_backend.render_d3().
    """
    import importlib
    import subprocess

    # Resolve the D3 viz type — try classifier first, fall back to chart_type
    viz_name = _classify_viz_type(chart_type, name)

    # Build data in D3-friendly format (list of dicts)
    data = spec.get("data", {})
    d3_data = _spec_data_to_d3(data, chart_type)

    if not d3_data:
        console.print("[red]Error: Could not convert chart data to D3 format[/red]")
        return

    # Resolve output path
    out = output or Path(f"/tmp/{name}_d3.html")
    out = out.with_suffix(".html")

    # Try to import d3_backend directly
    create_figure_dir = Path(__file__).resolve().parents[2] / "create-figure"
    sys.path.insert(0, str(create_figure_dir))
    try:
        from d3_backend import render_d3
        success = render_d3(
            viz_name, d3_data, out,
            title=name.replace("_", " ").title(),
            canvas=canvas, discord=discord,
        )
        if success:
            console.print(f"[green]D3 HTML: {out}[/green]")
            console.print(f"[dim]Viz type: {viz_name} | Data points: {len(d3_data)}[/dim]")
        else:
            console.print(f"[red]D3 render failed for viz type '{viz_name}'[/red]")
    except ImportError:
        # Fallback: shell out to create-figure
        cmd = [
            str(create_figure_dir / "run.sh"), viz_name,
            "--data", json.dumps(d3_data),
            "-o", str(out),
        ]
        if canvas:
            cmd.append("--canvas")
        ...
        if result.returncode == 0:
            ...
            console.print(f"[red]D3 render failed: {result.stderr}[/red]")
    finally:
        sys.path.pop(0)


def _classify_viz_type(chart_type: str, name: str) -> str:
    """Use viz-type-selector classifier to pick optimal D3 viz type.

    Falls back to chart_type if classifier unavailable.
    """
    # Direct mapping for common types
    type_map = {
        "bar": "bar", "hbar": "hbar", "line": "line",
        "pie": "pie", "heatmap": "heatmap", "area": "area",
        "scatter": "scatter",
    }
    if chart_type in type_map:
        return type_map[chart_type]

    # Try SetFit classifier
    try:
        from setfit import SetFitModel
        model_path = Path.home() / ".pi/models/classifiers/viz_type_setfit"
        if model_path.exists():
            model = SetFitModel.from_pretrained(str(model_path))
            query = f"{name} {chart_type}"
            prediction = model.predict([query])
            viz_name = str(prediction[0])
            return viz_name
    except Exception:
        pass
    ...
    try:
        import joblib
        model_path = Path.home() / ".pi/models/classifiers/viz_type_selector_classifier.joblib"
        if model_path.exists():
...
def _spec_data_to_d3(data: dict, chart_type: str) -> list:
    """Convert analytics chart spec data to D3-friendly list of dicts."""
    if not data:
        return []

    # Metrics format: {"metrics": {"label": value}}
    if "metrics" in data:
        metrics = data["metrics"]
        return [{"label": k, "value": v} for k, v in metrics.items()]

    # Line/area format: {name: {"x": [...], "y": [...]}}
    ...
    # Heatmap format: {row: {col: value}}
    if all(isinstance(v, dict) for v in data.values()):
        result = []
        for row, cols in data.items():
            for col, val in cols.items():
                result.append({"row": row, "col": col, "value": val})
        return result
