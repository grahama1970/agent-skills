...
app = typer.Typer(help="Arxiv paper to verified memory knowledge")


@app.command()
def main(
    arxiv_id: str = typer.Argument(None, help=""),
    search: str = typer.Option(None, help="Search query to find paper"),
    file: str = typer.Option(None, help="Local PDF file"),
    scope: str = typer.Option(..., help="Memory scope for storage"),
    context: str = typer.Option(None, help="Domain context for relevance filtering"),
    mode: str = typer.Option("auto", help="Interview mode"),
    dry_run: bool = typer.Option(False, help="Preview without storing"),
    skip_interview: bool = typer.Option(False, help=""),
    max_edges: int = typer.Option(20, help=""),
    accurate: bool = typer.Option(False, help=""),
    json: bool = typer.Option(False, help="Output as JSON"),
):
    """CLI entry point."""

    if not any([arxiv_id, search, file]):
