...
# Default timezone for human-readable display
DEFAULT_TZ = ZoneInfo("America/New_York")


def format_human_timestamp(dt: datetime | None = None, tz: ZoneInfo = DEFAULT_TZ) -> str:
    """Format datetime as human-readable timestamp like 'Mon, 6:10pm EST'.

    Args:
        dt: Datetime to format (default: now)
        tz: Timezone for display (default: EST/EDT)

    Returns:
        Formatted string like "Mon, 6:10pm EST"
    """
    if dt is None:
        dt = datetime.now(tz)
    ...
    # Format: "Mon, 6:10pm EST"
    day = dt.strftime("%a")
    hour = dt.hour % 12 or 12  # Convert to 12-hour, with 0 -> 12
    ...
    return f"{day}, {hour}:{minute}{ampm} {tz_abbr}"
