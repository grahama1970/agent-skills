...
def test_win_credit_for_6_gate_system():
    assert win_credit(6) == 1.0
    assert win_credit(5) == 0.5
    assert win_credit(4) == 0.5
    assert win_credit(2) == 0.5
    assert win_credit(1) == 0.0


def test_gates_to_verdict_6_gate_thresholds():
    assert gates_to_verdict(6) == "satisfied"
    assert gates_to_verdict(5) == "inconclusive"
    assert gates_to_verdict(4) == "inconclusive"
    assert gates_to_verdict(2) == "inconclusive"
    assert gates_to_verdict(1) == "not_satisfied"


def test_gates_to_grade_6_gate_thresholds():
    assert gates_to_grade(6) == "A+"
    assert gates_to_grade(5) == "A"
    assert gates_to_grade(4) == "B"   # 4/6 = 0.667 < 0.75
    assert gates_to_grade(3) == "B"   # 3/6 = 0.50
    assert gates_to_grade(2) == "C"   # 2/6 = 0.333 >= 0.25
    assert gates_to_grade(1) == "F"   # 1/6 = 0.167 < 0.25
...
def test_constants_still_6_gate():
    assert GATES_TOTAL == 6
