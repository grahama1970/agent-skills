...
./run.sh compose --description "ridgeline plot of scores by domain" --data sample.json

# From a d3_catalog type name
./run.sh compose --type ridgeline --data sample.json

# Output to specific file
./run.sh compose --description "bar chart of costs" --output costs.html
```
...
./run.sh evaluate --html gallery/ridgeline_v1.html
```

### 4. Iterate on a Composition

```bash
# Improve a low-scoring composition (max 3 rounds)
./run.sh iterate --html gallery/ridgeline_v1.html --max-rounds 3

# Set a custom quality threshold
./run.sh iterate --html gallery/ridgeline_v1.html --threshold 0.85
```

### 5. Promote to /create-figure

```bash
# Promote a tested composition to a /create-figure command
./run.sh promote --input gallery/ridgeline_v1.html
```

### 6. Gallery and Catalog

```bash
# List all compositions in the gallery
./run.sh gallery

# Show d3_catalog coverage (implemented vs NOT_YET)
./run.sh catalog-status

# List NOT_YET types ranked by demand
./run.sh backlog
```
...
| `promote` | Move successful composition to /create-figure (requires score >= 0.75) |
...
**Threshold**: >= 0.75 to promote. < 0.50 triggers re-composition from scratch.
