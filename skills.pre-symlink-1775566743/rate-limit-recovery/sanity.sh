...
# Test basic CLI invocation (no subcommands -- this is a flat Typer app)
echo -n "Testing CLI with invalid platform... "
RECOVER_OUTPUT=$(python3 "$PYTHON_SCRIPT" --platform _sanity_test_ 2>&1 || true)
if [[ "$RECOVER_OUTPUT" == *"Unknown platform"* ]]; then
    echo "✓ CLI responds correctly to invalid platform"
else
    echo "✗ CLI did not reject invalid platform"
