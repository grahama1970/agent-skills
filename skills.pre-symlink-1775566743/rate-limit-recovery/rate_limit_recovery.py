...
app = typer.Typer(help="Rate Limit Recovery Skill")


@app.command()
def recover(
    platform: str = typer.Option(None, "--platform", help="Specific platform to recover from (codex, claude, pi, antigravity)"),
    session_id: str = typer.Option(None, "--session-id", help="Session ID for recovery"),
    workspace: str = typer.Option(None, "--workspace", help="Workspace path for Claude recovery"),
    task_id: str = typer.Option(None, "--task-id", help="Task ID for Antigravity recovery"),
    format: str = typer.Option("markdown", "--format", help="Output format (json, markdown, txt)"),
    output: str = typer.Option(None, "--output", help="Output file path (default: ~/.pi/rate-limit-recovery/)"),
) -> None:
    """Recover from rate limiting on agent platforms."""
    recovery = RateLimitRecovery()

    # Auto-detect platform if not specified
    detected_platform = platform or recovery.detect_platform()
    if not detected_platform:
        print("Could not auto-detect platform. Please specify --platform")
        raise typer.Exit(1)

    print(f"Recovering from {detected_platform} rate limiting...")

    # Perform platform-specific recovery
    if detected_platform == "codex":
        recovery_data = recovery.recover_codex(session_id)
    elif detected_platform == "claude":
        recovery_data = recovery.recover_claude(workspace)
    elif detected_platform == "pi":
        recovery_data = recovery.recover_pi(session_id)
    elif detected_platform == "antigravity":
        recovery_data = recovery.recover_antigravity(task_id)
    else:
    ...
    # Export data
    output_file = recovery.export_recovery_data(recovery_data, format, output)
    ...
    try:
        ...
        if memory_script.exists():
            memory_cmd = [
                ...
                str(memory_script),
                "learn",
                "--problem", f"Rate limited on {detected_platform} platform",
                "--solution", f"Recovered session data from {detected_platform} rate limit. See {output_file}"
            ]
