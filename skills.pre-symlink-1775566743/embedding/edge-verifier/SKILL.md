...
```json
{
  ...
  "text": "Run .pi/skills/edge-verifier/run.sh on recent data",
  "schedule": "0 2 * * *",
...
1.  **Link Skill**: Ensure `mom` can see this skill.
    ```bash
    ln -s $(pwd)/.pi/skills/edge-verifier ~/.pi/mom/data/skills/edge-verifier
    ```
2.  **Schedule**: Copy the template.
    ```bash
    cp .pi/skills/edge-verifier/schedule_template.json ~/.pi/mom/data/events/verify-daily.json
