...
def check_cache(query: str) -> Optional[IntentResult]:
    ...
    if client is not None:
        try:
            ...
            data = search(q=cache_query, scope="os", k=1)
    ...
    try:
        result = subprocess.run(
            [str(MEMORY_AGENT_CLI), "recall", "-q", cache_query,
             "--scope", "os", "--k", "1"],
    ...
    except (json.JSONDecodeError, subprocess.TimeoutExpired, KeyError, OSError):
        pass
...
def store_cache(query: str, result: IntentResult) -> None:
    ...
    if client is not None:
        try:
            client.learn(problem=problem, solution=solution, scope="os", tags=["intent-cache"])
            return
        except Exception:
            pass
