...
MEMORY_DIR = Path(os.path.expanduser("~/workspace/experiments/memory"))
...
def recall_memory(query: str, scope: str, k: int = 5, timeout: int = 15) -> list:
    """Recall from memory via embry-memory daemon Unix socket."""
    import httpx
    ...
    try:
        ...
        log.debug(f"memory httpx recall failed: {e}")
...
def discover_persona(name: str) -> Optional[dict]:
    ...
    if personas_yaml.exists():
        try:
            ...
            log.debug(f"personas.yaml parse failed: {e}")
