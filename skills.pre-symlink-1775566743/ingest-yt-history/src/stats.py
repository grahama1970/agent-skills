...
def main(
    ...
    by_service: bool = typer.Option(False, "--by-service", help="Show breakdown by service (YouTube vs YouTube Music)"),
    as_json: bool = typer.Option(False, "--json", help="Output as JSON instead of human-readable format"),
) -> None:
    """Show statistics for YouTube watch history."""
    try:
        entries = load_history(input_path)
        stats = compute_stats(entries, by_service=by_service)
