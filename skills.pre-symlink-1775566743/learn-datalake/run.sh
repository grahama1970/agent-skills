...
_start_supervised() {
    ...
    setsid -f uv run --project "$SCRIPT_DIR" python supervise_learn_datalake.py \
        "$root" \
        --label "$label" \
        "${extra[@]}" \
...
_status_supervised() {
    ...
    local state_timeout_hint_count="0"
    local state_extraction_attempts="0"
    local state_extraction_success_count="0"
    local state_extraction_cached_profile_count="0"
    local state_extraction_failed_count="0"
    local state_fail_rate_pct="0.00"
    local state_missing_structural_count="0"
    local state_missing_structural_rate_pct="0.00"
    local state_throughput_per_hour="0.00"
    local state_timeout_model_decisions="0"
    local state_timeout_model_used_count="0"
    local state_timeout_model_used_rate_pct="0.00"
    local state_timeout_model_high_risk_count="0"
    local state_timeout_model_last_risk="0.00"
    ...
    local state_memory_retry_queue_count="0"
    local state_memory_retry_dead_letter_count="0"
    local state_workers="1"
    local corpus_pdf_count="0"
    local corpus_profile_count="0"
    local cached_profile_count="0"
    ...
    if [[ -f "$state_file" ]]; then
        ...
        state_timeout_hint_count="$(jq -r '.run_metrics.extraction_timeout_hint_count // .extraction_timeout_hint_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_extraction_attempts="$(jq -r '.run_metrics.extraction_attempts // .extraction_attempts // 0' "$state_file" 2>/dev/null || echo 0)"
        state_extraction_success_count="$(jq -r '.run_metrics.extraction_success_count // .extraction_success_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_extraction_cached_profile_count="$(jq -r '.run_metrics.extraction_cached_profile_count // .extraction_cached_profile_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_extraction_failed_count="$(jq -r '.run_metrics.extraction_failed_count // .extraction_failed_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_fail_rate_pct="$(jq -r '.run_metrics.extraction_fail_rate_pct // .extraction_fail_rate_pct // 0' "$state_file" 2>/dev/null || echo 0)"
        state_missing_structural_count="$(jq -r '.run_metrics.extraction_missing_structural_count // .extraction_missing_structural_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_missing_structural_rate_pct="$(jq -r '.run_metrics.extraction_missing_structural_rate_pct // .extraction_missing_structural_rate_pct // 0' "$state_file" 2>/dev/null || echo 0)"
        ...
        state_timeout_model_decisions="$(jq -r '.run_metrics.timeout_model_decisions // .timeout_model_decisions // 0' "$state_file" 2>/dev/null || echo 0)"
        state_timeout_model_used_count="$(jq -r '.run_metrics.timeout_model_used_count // .timeout_model_used_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_timeout_model_used_rate_pct="$(jq -r '.run_metrics.timeout_model_used_rate_pct // .timeout_model_used_rate_pct // 0' "$state_file" 2>/dev/null || echo 0)"
        state_timeout_model_high_risk_count="$(jq -r '.run_metrics.timeout_model_high_risk_count // .timeout_model_high_risk_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_timeout_model_last_risk="$(jq -r '.run_metrics.timeout_model_last_risk // .timeout_model_last_risk // 0' "$state_file" 2>/dev/null || echo 0)"
        ...
        state_memory_retry_queue_count="$(jq -r '.run_metrics.memory_retry_queue_count // .memory_retry_queue_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_memory_retry_dead_letter_count="$(jq -r '.run_metrics.memory_retry_dead_letter_count // .memory_retry_dead_letter_count // 0' "$state_file" 2>/dev/null || echo 0)"
        state_workers="$(jq -r '.run_metrics.workers // 1' "$state_file" 2>/dev/null || echo 1)"
    ...
    if [[ -n "$root" ]] && [[ -d "$root" ]]; then
        corpus_pdf_count="$(fd -e pdf -e html -e htm -e docx -e md -e xml -e epub -e txt -e json -e jsonl -e rst -e csv -e tsv -e pptx -e xlsx . "$root" 2>/dev/null | wc -l | tr -d ' ')"
        corpus_profile_count="$(fd -p '00_profile_detector/profile.json' "$root" 2>/dev/null | wc -l | tr -d ' ')"
        if [[ -d "$REVIEW_PDF_EXTRACTED_RUNS" ]]; then
            cached_profile_count="$(
                fd -p '00_profile_detector/profile.json' "$REVIEW_PDF_EXTRACTED_RUNS" 2>/dev/null | \
                    wc -l | \
                    tr -d ' '
            )"
        fi
        extracted_pdf_count="$(
            {
                ...
                if [[ -d "$REVIEW_PDF_EXTRACTED_RUNS" ]]; then
                    fd -0 -p '00_profile_detector/profile.json' "$REVIEW_PDF_EXTRACTED_RUNS" 2>/dev/null
                fi
            } | \
                xargs -0 -r jq -r '.file // empty' 2>/dev/null | \
                rg -i '\.(pdf|html?|docx?|md|xml|epub|txt|json|jsonl|rst|csv|tsv|pptx?|xlsx?)$' | \
                sort -u | \
                wc -l | \
                tr -d ' '
        )"
        ...
        echo "corpus_doc_count=$corpus_pdf_count"
        echo "corpus_profile_count=$corpus_profile_count"
        echo "cached_profile_count=$cached_profile_count"
        echo "extracted_doc_count=$extracted_pdf_count"
        echo "missing_doc_count=$missing_pdf_count"
        echo "extracted_doc_coverage_pct=$extracted_pdf_coverage_pct"
    fi
    ...
    if [[ "$show_table" == "yes" ]]; then
        ...
        _row "extraction_attempts" "$state_extraction_attempts"
        _row "extraction_success_count" "$state_extraction_success_count"
        _row "extraction_cached_profile_count" "$state_extraction_cached_profile_count"
        _row "extraction_failed_count" "$state_extraction_failed_count"
        _row "timeout_rate_pct" "$state_timeout_rate_pct"
        _row "timeout_hint_count" "$state_timeout_hint_count"
        _row "extract_fail_rate_pct" "$state_fail_rate_pct"
        _row "missing_structural_count" "$state_missing_structural_count"
        _row "missing_structural_rate_pct" "$state_missing_structural_rate_pct"
        _row "throughput_per_hour" "$state_throughput_per_hour"
        _row "timeout_model_decisions" "$state_timeout_model_decisions"
        _row "timeout_model_used_count" "$state_timeout_model_used_count"
        _row "timeout_model_used_rate_pct" "$state_timeout_model_used_rate_pct"
        _row "timeout_model_high_risk_count" "$state_timeout_model_high_risk_count"
        _row "timeout_model_last_risk" "$state_timeout_model_last_risk"
        ...
        _row "memory_retry_queue_count" "$state_memory_retry_queue_count"
        _row "memory_retry_dead_letter_count" "$state_memory_retry_dead_letter_count"
        _row "corpus_pdf_count" "$corpus_pdf_count"
        _row "corpus_profile_count" "$corpus_profile_count"
        _row "cached_profile_count" "$cached_profile_count"
...
viewer)
    shift
    port="${1:-8004}"
    echo "Starting learn-datalake viewer on http://localhost:$port"
    echo "  API: http://localhost:$port/api/supervisors"
    echo "  UI:  http://localhost:$port"
    exec uv run --project "$SCRIPT_DIR" \
...
quarantine-ui)
    ...
    exec uv run --project "$SCRIPT_DIR" \
        uvicorn datalake_api:app --host 0.0.0.0 --port "$port"
    ;;
review-quarantine)
    shift
    exec uv run --project "$SCRIPT_DIR" python learn_datalake.py review-quarantine "$@"
    ;;
heal)
    shift
    exec uv run --project "$SCRIPT_DIR" python heal_failures.py triage "$@"
    ;;
heal-retry)
    shift
    exec uv run --project "$SCRIPT_DIR" python heal_failures.py retry "$@"
    ;;
heal-fixture)
    shift
    exec uv run --project "$SCRIPT_DIR" python heal_failures.py fixture "$@"
    ;;
heal-analyze)
    shift
    exec uv run --project "$SCRIPT_DIR" python heal_failures.py analyze "$@"
    ;;
once-heal)
    # Run one learning cycle, then auto-heal any quality failures
    shift
    uv run --project "$SCRIPT_DIR" python learn_datalake.py once "$@" || once_rc=$?
    once_rc="${once_rc:-0}"
    if [[ $once_rc -ne 0 ]]; then
        echo "learn-datalake once exited with rc=$once_rc — running heal triage"
        uv run --project "$SCRIPT_DIR" python heal_failures.py triage
    else
        echo "learn-datalake once succeeded — skipping heal"
    fi
    exit $once_rc
    ;;
*)
    exec uv run --project "$SCRIPT_DIR" python learn_datalake.py "$@"
    ;;
esac
