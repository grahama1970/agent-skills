...
# Collect OOM killer events (kernel-level)
get_oom_events() {
  jctl_cmd "" | grep -iE "(out of memory|oom-kill|killed process|oom_reaper|invoked oom)" | head -50 || true
}

# Collect V8/Electron heap OOM events (application-level, e.g. VS Code)
get_v8_oom_events() {
  jctl_cmd "" | grep -iE "(OOM error in V8|JavaScript heap out of memory|heap out of memory)" | head -20 || true
}
...
output_markdown() {
  ...
  # V8/Electron Heap OOM (application-level, e.g. VS Code)
  echo "### V8/Electron Heap OOM Events"
  echo ""
  local v8_ooms
  v8_ooms=$(get_v8_oom_events)
  if [[ -n "$v8_ooms" ]]; then
    echo "**Application-level V8 heap exhaustion detected** (causes UI freezes in Electron apps like VS Code):"
    echo ""
    echo "\`\`\`"
    echo "$v8_ooms"
    echo "\`\`\`"
    echo ""
    echo "**Fix:** Run \`./run.sh vscode --fix\` to kill runaway workers, or set heap limit in \`~/.config/Code/User/argv.json\`."
  else
    echo "No V8 heap OOM events found."
  fi
  echo ""
