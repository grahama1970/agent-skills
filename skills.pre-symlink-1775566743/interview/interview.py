...
QuestionType = Literal[
    "yes_no", "yes_no_refine", "select", "multi", "text", "confirm",
    "image_compare", "bbox_annotation",
]
...
class Question:
    ...
    # V2.2 fields for bbox annotation
    bbox_labels: list[dict] | None = None  # [{"name": "Table", "color": "#22c55e"}]
    pre_annotations: list[dict] | None = None  # [{"type": "Table", "bbox": [x0,y0,x1,y1], "page": 0}]
    render_dpi: int = 150  # DPI used for page rendering
    page_num: int | None = None  # PDF page number for this question
    # V2.3 fields for code blocks and recommendation strength
    code_block: str | None = None  # Code/diff to display with the question
    code_lang: str | None = None  # Language hint for syntax highlighting (e.g. "python", "diff")
...
ConvictionLevel = Literal["strong", "slight", "neutral"]
WeightLevel = Literal["critical", "important", "minor"]
...
class Response:
    ...
    # V2.3 fields for richer decision signal
    conviction: ConvictionLevel | None = None  # How strongly the user feels
    weight: WeightLevel | None = None  # How important this decision is
...
app = typer.Typer(help="Structured human-agent Q&A")


@app.command()
def main(
    file: str = typer.Option(None, "-f", "--file", help="Questions JSON file"),
    mode: str = typer.Option("auto", "-m", "--mode", help="Mode: auto, html, or tui"),
    timeout: int = typer.Option(600, "-t", "--timeout", help="Timeout in seconds"),
    resume: str = typer.Option(None, "-r", "--resume", help="Resume session (use 'latest' for most recent)"),
    json_output: bool = typer.Option(False, "--json", help="Output as JSON"),
) -> None:
    """Structured human-agent Q&A."""
    if resume:
        if resume == "latest":
        ...
        title, context, questions = load_questions_file(file)
        interview = Interview(title=title, context=context)
        result = interview.run(questions, mode=mode, timeout=timeout)
    else:
        print("Must provide --file or --resume", file=sys.stderr)
        raise typer.Exit(1)
