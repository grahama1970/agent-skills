...
# Logging is handled by loguru (imported above)
...
JSON response:"""
    ...
    # Use scillm HTTP proxy instead of claude -p CLI
    try:
        import httpx
...
if __name__ == "__main__":
    ...
    app = typer.Typer(help="Agent-Inbox Triage Module")

    @app.command()
    def classify(
        message: str = typer.Argument(..., help="Bug message to classify"),
        no_llm: bool = typer.Option(False, "--no-llm", help="Use heuristics only"),
    ):
        result = classify_severity_llm(message) if not no_llm else None
        if not result:
            severity, indicators = classify_severity_heuristic(message)
    ...
    @app.command()
    def route(
        message: str = typer.Argument(..., help="Bug message with file paths"),
    ):
        project = auto_route(message)
        if project:
    ...
    @app.command()
    def triage(
        message: str = typer.Argument(..., help="Bug message"),
        no_llm: bool = typer.Option(False, "--no-llm", help="Use heuristics only"),
    ):
        result = triage_message(message, use_llm=not no_llm)
        print(json.dumps(result, indent=2))
    ...
    def webhook(
        ...
        events: str = typer.Option(None, help="Comma-separated events"),
        project: str = typer.Option(None, help="Project filter"),
    ):
        if action == "add":
            if not url:
            ...
            if register_webhook(url, events_list, project):
    ...
    def log(
        ...
        log_data = get_triage_log(msg_id)
        if log_data:
            print(json.dumps(log_data, indent=2))
        else:
            print(f"No triage log for: {msg_id}")
