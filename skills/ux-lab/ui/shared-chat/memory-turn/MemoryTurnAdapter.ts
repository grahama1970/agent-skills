...
export type ChatRole = "user" | "assistant" | "system" | "tool" | "agent" | "error";
...
export interface ChatMessage {
	...
	timestamp?: number;
	/**
	 * Legacy diagnostic-only assistant turn tombstoned on session load; rendered
	 * collapsed and clearly invalidated, never as a current substantive answer
	 * (grahama1970/sparta#36). Mirrors the field on the shared-chat
	 * `ChatMessage` so both message shapes stay assignable.
	 */
...
export function liveStatusLabelFromSteps(steps: StreamingStep[], fallback = "Thinking…"): string {
	const latestById = new Map<string, StreamingStep>();
	const anonymousSteps: StreamingStep[] = [];
	for (const step of steps) {
		const key = step.id;
		if (key) latestById.set(key, step);
		else anonymousSteps.push(step);
	}
	const latestSteps = [...latestById.values(), ...anonymousSteps];
	for (let index = latestSteps.length - 1; index >= 0; index -= 1) {
		const step = latestSteps[index];
		if (step.status === "running") return step.liveStatusLabel ?? step.label ?? step.summary ?? fallback;
	}
	const last = latestSteps[latestSteps.length - 1];
