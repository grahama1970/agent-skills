...
import { EMBRY } from './_support/EmbryStyle'
import {
...
export function InlineEvidenceCase({ data, onViewDetails, onOpenInWorkspace, onOpenInSources, loading, receiptMode = false, unifiedFooter = false, defaultExpanded = false }: InlineEvidenceCaseProps) {
  ...
  const provenanceStripeColor = canApproveOrExport
    ? EMBRY.green
    : evidenceState === 'Artifact not bound' || approval === 'Rejected'
      ? EMBRY.red
      : EMBRY.amber
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
          ...
          >
            ...
            <span data-qid={`evidence-case:tier-badge:${id}`} style={{ display: 'inline-flex', alignItems: 'center', padding: '2px 8px', borderRadius: 0, fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: tierTone.color, background: tierTone.bg, border: `1px solid ${tierTone.border}` }}>{tierBadge}</span>
  ...
  if (loading) {
    return (
      ...
      >
        Building deterministic receipt...
      </section>
    )
  }
  ...
  return (
    <section
      ...
      title={`${id}: ${evidenceState}; ${traceState}; ${approval}`}
      aria-label={`${id}: ${evidenceState}; ${traceState}; ${approval}`}
      style={{
        backgroundColor: EMBRY.bgDeep,
        border: `1px solid ${EMBRY.border}`,
        borderLeft: `1px solid ${provenanceStripeColor}`,
        borderRadius: 0,
        padding: '10px 12px',
        marginTop: 8,
        maxWidth: 760,
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', gap: 12, alignItems: 'start' }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <span data-qid={`evidence-case:id:${id}`} title={`Case ID: ${id}`} style={{ color: EMBRY.dim, fontSize: 10, fontFamily: 'var(--font-mono)', overflowWrap: 'anywhere' }}>
              {id}
            </span>
          </div>
          {expanded ? (
            <>
              <div data-qid={`evidence-case:artifact:${id}`} title={`Bound artifact: ${boundArtifact}`} style={{ color: EMBRY.white, fontSize: 13, lineHeight: 1.5, overflowWrap: 'anywhere' }}>
                <span style={{ fontFamily: 'var(--font-mono)' }}>{boundArtifact}</span>
              </div>
              <div data-qid={`evidence-case:provenance-distinction:${id}`} title="Artifact binding is not citation provenance" style={{ color: EMBRY.dim, fontSize: 11, lineHeight: 1.45, marginTop: 4 }}>
                Artifact binding only confirms the file is attached; source-page citation provenance is still blocked.
              </div>
              {artifactHash && (
                <div data-qid={`evidence-case:artifact-hash:${id}`} title={auditInvalidHash ? `Mock artifact hash: ${artifactHash}` : `Artifact SHA256: ${artifactHash}`} style={{ color: auditInvalidHash ? '#d0a862' : EMBRY.dim, fontSize: 11, fontFamily: 'var(--font-mono)', overflowWrap: 'anywhere', marginTop: 2, fontWeight: auditInvalidHash ? 750 : 400 }}>
              ...
              <div data-qid={`evidence-case:citation-preview:${id}`} title={citations.length > 0 ? `Citation anchors pending source-page excerpts: ${citations.join(', ')}` : 'Citations pending'} style={{ color: EMBRY.dim, fontSize: 11, lineHeight: 1.45, marginTop: 8, overflowWrap: 'anywhere' }}>
      ...
      {expanded ? (
        <div data-qid={`evidence-case:body:${id}`}>
          {gateCount > 0 && (
            <div data-qid={`evidence-case:gates:${id}`} title={`${gatePassed}/${gateCount} gates passed`} style={{ borderTop: `1px solid ${EMBRY.border}`, marginTop: 10, paddingTop: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}>
                <span style={{ color: gatePassed >= gateCount ? EMBRY.green : EMBRY.amber, border: `1px solid ${gatePassed >= gateCount ? EMBRY.green : EMBRY.amber}55`, borderRadius: 0, padding: '2px 8px', fontSize: 10, fontWeight: 800, fontFamily: 'var(--font-mono)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                  {gatePassed}/{gateCount} gates passed
                </span>
                <span style={{ color: EMBRY.dim, fontSize: 10, fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap' }}>{traceState}</span>
              </div>
              ...
              {gatePassed < gateCount && (
                <div data-qid={`evidence-case:next-step:${id}`} title="Next step to resolve blocked evidence gates" style={{ color: EMBRY.dim, fontSize: 11, lineHeight: 1.45, marginTop: 8 }}>
                  Request more evidence or rerun after source-page provenance is bound.
                </div>
              )}
            </div>
          )}
          ...
          <div
            data-qid={`evidence-case:reviewer-actions:${id}`}
            data-qs-action="EVIDENCE_CASE_REVIEWER_ACTIONS"
            title="Evidence case reviewer actions"
          ...
          {!canApproveOrExport && (
            <div id={`evidence-case:blocked-actions:${id}`} style={{ color: EMBRY.dim, fontSize: 11, lineHeight: 1.45, marginTop: 8 }}>
              {blockedActionReason}
            </div>
          )}
          ...
          <div data-qid={`evidence-case:expanded:${id}`} title="Expanded evidence case audit trail" style={{ borderTop: `1px solid ${EMBRY.border}`, marginTop: CHAT_READABLE.blockGap, paddingTop: 12 }}>
            {entities.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                {entities.map(entity => (
                  <span key={entity} tabIndex={0} title={`${entity}: bound entity in this evidence case`} style={{ color: EMBRY.blue, border: `1px solid ${EMBRY.blue}55`, backgroundColor: `${EMBRY.blue}14`, borderRadius: 0, padding: '3px 8px', fontSize: 11, fontFamily: 'var(--font-mono)' }}>
                    {entity}
                  </span>
                ))}
              </div>
            ...
            {citations.length > 0 && (
              <>
                ...
                <div style={{ color: EMBRY.dim, fontSize: 11, lineHeight: 1.45, marginBottom: 6 }}>
          ...
          {approval !== 'Approved' && (
            <div data-qid={`evidence-case:draft-warning:${id}`} title="Fail-closed draft warning" style={{ color: '#d0a862', fontSize: 12, fontWeight: 750, lineHeight: 1.45, marginTop: 10, padding: '8px 10px', border: `1px solid ${EMBRY.amber}55`, borderRadius: 0, backgroundColor: `${EMBRY.amber}0a` }}>
