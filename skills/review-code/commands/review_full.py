...
try:
    from memory_integration import recall_prior_reviews, learn_review
    _HAS_MEMORY_INTEGRATION = True
except ImportError:
    try:
        # Try relative import for module mode
        from ..memory_integration import recall_prior_reviews, learn_review
        _HAS_MEMORY_INTEGRATION = True
    except ImportError:
        pass
...
def review_full(
    ...
    # Pre-hook: Recall prior reviews for this project
    project_name = file.stem  # Use request filename as project identifier
    if _HAS_MEMORY_INTEGRATION:
        try:
            prior = recall_prior_reviews(project_name)
            if prior:
                typer.echo(f"Found prior review findings in memory for '{project_name}'", err=True)
                previous_context += f"\n\n## Prior Review Findings (from memory)\n{prior}"
        except Exception as e:
            typer.echo(f"Memory recall skipped: {e}", err=True)
    ...
    if _HAS_MEMORY_INTEGRATION:
        try:
            learn_review(
                ...
                files_reviewed=[str(file)],
                findings=result.get("final_output", "")[:4000] if result else None,
                severity_counts=None,  # Could be parsed from output in future
                provider=provider,
                model=actual_model,
                rounds_completed=len(result.get("rounds", [])),
                diff_generated=bool(result.get("final_diff")),
            )
            typer.echo("Review findings learned to memory.", err=True)
        except Exception as e:
            typer.echo(f"Memory learn skipped: {e}", err=True)
