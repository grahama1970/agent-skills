...
class MemoryScope(str, Enum):
    ...
    BRANDON_BAILEY = "brandon_bailey"     # Brandon Bailey (cybersecurity/SPARTA)
    EMBRY = "embry"                       # Embry persona knowledge
    MARGARET_CHEN = "margaret_chen"       # Margaret Chen persona knowledge
    JENNIFER_CHEUNG = "jennifer_cheung"   # Jennifer Cheung persona knowledge
...
def _is_direct_control_lookup(query: str) -> bool:
    ...
    Direct lookups (e.g. "SV-123", "AC-2(1)", "NIST 800-53 AC-2") should NOT
    trigger the evidence-case fallback — they just need a corpus search.
    """
    import re
    q = query.strip()
    # STIG/CCI/SRG IDs: SV-12345, V-12345, CCI-001234, SRG-OS-000001
    if re.match(r'^(SV|V|CCI|SRG)-\d', q, re.IGNORECASE):
        return True
    # NIST control IDs: AC-2, AC-2(1), SI-4.a
...
# Default evidence-case fallback chain for non-trivial queries with no results.
# Skills: /assess → /create-evidence-case → /plan → /orchestrate
EVIDENCE_FALLBACK_CHAIN = [
    {"skill": "/assess", "purpose": "understand the problem space"},
    {"skill": "/create-evidence-case", "purpose": "build structured CAE proof before answering"},
    {"skill": "/plan", "purpose": "plan remediation if evidence reveals issues"},
    {"skill": "/orchestrate", "purpose": "execute the plan"},
]
...
class RecallResult:
    ...
    def fallback_chain(self) -> List[Dict[str, str]]:
        """Recommended skill chain when found=false and query is non-trivial.

        Returns the evidence-case fallback chain for analytical/reasoning
        questions. Returns empty list for direct control lookups or when
        results were found.

        The chain is: /assess → /create-evidence-case → /plan → /orchestrate
        This ensures structured verification before answering.
        """
        if self.found:
            return []
        if _is_direct_control_lookup(self.query):
            return []
        return EVIDENCE_FALLBACK_CHAIN

    @property
    def should_build_evidence(self) -> bool:
        """Whether the caller should build an evidence case before answering.

        True when: no results found AND query is analytical (not a direct
        control lookup). This is the global default for all /ask-related tasks.
        """
...
class MemoryClient:
    ...
    def _get_run_script(self) -> Path:
        ...
        # Check multiple locations — .pi/skills/memory/run.sh FIRST
        # (memory_root/run.sh may be a different skill's run.sh)
        candidates = [
            self.memory_root / ".pi" / "skills" / "memory" / "run.sh",
    ...
    def _get_python_client(self) -> Any:
        ...
        NOTE: use_python_api=True is deprecated. All callers should use
        the CLI subprocess path (use_python_api=False, the default).
        Raising RuntimeError to enforce the /memory subprocess contract.
        """
        raise RuntimeError(
            "Direct Python API access to graph_memory is no longer supported. "
            "Use use_python_api=False (the default) to go through /memory subprocess."
        )
    ...
    def _learn_cli(
        ...
        try:
            ...
            if result.returncode == 0:
                try:
                    # Strip non-JSON prefix (e.g. [quality-gate] lines) from stdout
                    stdout = result.stdout.strip()
                    json_start = stdout.find("{")
                    if json_start > 0:
                        stdout = stdout[json_start:]
                    data = json.loads(stdout)
                    # Response may be nested: {"meta": {"ok": true}, "items": [{"_key": "..."}]}
                    items = data.get("items", [])
                    lesson_id = (
                        items[0].get("_key", "") if items
                        else data.get("_key", data.get("id", ""))
                    )
                    return LearnResult(
                        success=True,
                        lesson_id=lesson_id,
    ...
    def _learn_python(
        ...
        meta = result.get("meta", {})
        items = result.get("items", [])
        ok = meta.get("ok", result.get("success", True))
        lesson_id = (
            items[0].get("_key", "") if items
            else result.get("_key", result.get("id", ""))
        )
        return LearnResult(
            success=ok,
            lesson_id=lesson_id,
