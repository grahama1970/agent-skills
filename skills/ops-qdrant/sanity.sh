...
echo "[1/5] uv sync + CLI import smoke"
...
echo "[2/5] SAFETY: source performs NO writes (forbidden mutation verbs absent)"
...
# No Qdrant point/collection mutation endpoints.
if grep -nE '/points|/collections/[^"]*(", *json=|/index)|create_collection|upsert' "$SRC"; then
    echo "FAIL: Qdrant mutation endpoint referenced"; fail=1
fi
[ "$fail" -eq 0 ] && note "ok — no writes"

echo "[3/5] POSITIVE-CONTROL: check --json emits ops_qdrant.health.v1"
...
echo "[4/5] NEGATIVE-CONTROL: unreachable Qdrant -> status=down, exit 2"
...
echo "[5/5] SCHEMA: named-vector parsing shape is present when collections exist"
