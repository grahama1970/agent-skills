...
`/paper-lab` accepts a comprehensive plan file that drives the entire convergence loop.
The plan specifies venue, sections, code files to ground against, reviewer personas,
figure requirements, and model routing preferences.

### Three Phases

**Phase 1: Planning (`/plan` + `/interview`)** — Human in the loop.
- `/interview` collects: core claims, target venue, honest gaps, code grounding files
- `/project-state` provides ground truth of the running system
- `/dogpile` + `/arxiv` research related work and citation targets
- Output: `paper_plan.md` (comprehensive, human-validated)

**Phase 2: Convergence (`/paper-lab tune --plan`)** — Headless, no human.
- `/create-paper` generates draft from plan + project state
- `/create-figure` generates publication figures from analytics
- `/review-paper` multi-model review (headless — flags ambiguities, never asks questions)
- `/create-peer-review` academic scoring + arxiv benchmark comparison
- Fix pass: apply HIGH → MEDIUM → LOW severity
- Loop until: `score >= threshold` OR `|delta| < epsilon` OR `round >= max_rounds`

**Phase 3: Human Review (`/interview`)** — Human back in the loop.
- Present unresolvable findings (contradictions, missing data, ambiguous claims)
- Collect human decisions needed to unblock next convergence round
- Feed revision notes back into Phase 2

### Goal

Papers pass `/create-peer-review` at **8+/10** average across 4 reviewer personas.
This is the convergence target — everything else is a means to that end.
...
`/paper-lab` operates in three phases with clear human/automation boundaries:

### Phase 1: Planning (Human + Agent)
- `/plan` creates the paper plan (scope, claims, code to reference, figures needed)
- `/interview` fills gaps — author answers questions about intent, audience, constraints
- Output: a structured plan file consumed by Phase 2

### Phase 2: Headless Convergence (Automated)
```bash
/paper-lab tune paper.tex --plan paper_plan.json --rounds 3 --threshold 8.5
```
- Runs `/review-paper --headless` each round
- Computes score deltas
- Applies fixes (HIGH → MEDIUM → LOW priority)
- **Never uses /interview** — ambiguities become AMBIGUITY findings
- Stops on convergence criteria (score >= threshold, delta < epsilon, max rounds)

### Phase 3: Human Resolution (Human + Agent)
- `/interview` resolves stalled findings (type=AMBIGUITY that survived Phase 2)
- Author decides: fix the code, fix the paper, or acknowledge as limitation
- Then re-enters Phase 2 if needed

### The `--plan` Flag
When `--plan` is provided, paper-lab:
1. Validates all claims in the plan against `/project-state` output
2. Checks that all referenced code files exist and match described behavior
3. Verifies figure data artifacts exist before generating figures
4. Routes review dimensions to optimal providers (see `/review-paper` model routing)
...
> **Key invariant**: `/interview` is NEVER called inside the convergence loop.
> Ambiguities are flagged as findings and resolved in Phase 3.
> This is what makes `/paper-lab` the paper equivalent of `/orchestrate` —
> a headless convergence engine that composes skills with quality gates.
...
|----------|-----------|--------------|
| 0 | **Pen name disclosure** | All fictitious author names must have `*` marker. Add footnote: "Fictitious names representing domain-expert AI personas." Human author must NOT have `*`. Severity: HIGH if missing. |
...
Options:
  ...
  --plan PATH          Paper plan file (venue, sections, code files, personas, models)
...
| `/create-peer-review` | Final quality gate — 4-persona peer review with arxiv benchmark comparison |
| `/create-figure` | Regenerate figures when figure-text alignment issues found |
| `/project-state` | Ground truth check (Step 0) — prevents doc-code drift at source |
| `/interview` | Phase 1 and Phase 3 only — never inside Phase 2 automated loop |
| `/dogpile` | ArXiv research for related work gaps identified during review |
| `/arxiv` | Paper search and citation management |
