...
def is_sfx_review_candidate(row: dict) -> bool:
    ...
    if str(row.get("source_type") or "") != "sfx":
        return False
    if str(row.get("review_status") or "") == "rejected":
        return False
...
<head>
  ...
  <title>Voice Segment Review</title>
  <style>
    :root {{
...
<body>
<main>
  <h1>SRT Ear Review</h1>
  <section class="summary">
    <div><strong>Job:</strong> <code>{escape(str(job_dir))}</code></div>
    <div><strong>{len(candidates)}</strong> English movie SRT slices with tagged transcripts only</div>
  </section>
  ...
  <section class="clip-list" aria-label="Voice clip candidates">
    ...
    <aside class="queue" aria-label="Selected clone reference queue">
      ...
      <div class="queue-actions">
        ...
        <label>Speaker/persona <input id="speaker-id" type="text" placeholder="embry"></label>
