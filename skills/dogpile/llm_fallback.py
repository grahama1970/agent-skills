...
class OpenAIProvider(LLMProvider):
    """OpenAI-compatible provider via scillm HTTP proxy."""
    ...
    def call(self, prompt: str, schema: Optional[Path] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            log_status(f"{self.name} HTTP error: {e.response.status_code}", provider=self.name, status="ERROR")
...
class ClaudeProvider(LLMProvider):
    """Claude provider via scillm HTTP proxy (replaces claude -p CLI)."""

    name = "claude"
    SCILLM_URL = "http://localhost:4001"

    # Map friendly names to scillm model IDs
    MODEL_MAP = {
...
if __name__ == "__main__":
    ...
    @_app.command()
    def _main(
        prompt: str = typer.Argument("What is 2+2? Reply with just the number.",
                                     help="Test prompt"),
        list_providers: bool = typer.Option(False, "--list", help="List available providers"),
    ):
        if list_providers:
        ...
        print(f"Testing fallback chain with prompt: {prompt[:50]}...")
        print()

        provider_name, result = call_with_fallback(prompt)
