...
def _run_skill(skill_path: Path, args: list[str], label: str, timeout: int = 1800) -> tuple[bool, str]:
...
def ingest(
    ...
    monitor = TaskClient("ingest-compliance-doc", total=len(stages)) if TaskClient else None

    # Stage 1: Extract
    typer.echo(f"[1/{len(stages)}] Extracting document content...")
    extract_args = [path, "--fast", "--no-interactive"]
    if preset:
...
@app.command()
def batch(
    directory: Path = typer.Argument(..., help="Directory of PDFs to process"),
    preset: Optional[str] = typer.Option(None, help="Force extractor preset"),
    ...
    if not pdfs:
        typer.echo(f"No PDFs found in {directory} (recursive)")
        raise typer.Exit(1)

    typer.echo(f"Found {len(pdfs)} PDFs in {directory}\n")
    batch_monitor = TaskClient("ingest-compliance-doc-batch", total=len(pdfs)) if TaskClient else None
    for i, pdf in enumerate(pdfs, 1):
        typer.echo(f"\n{'='*60}")
        typer.echo(f"[{i}/{len(pdfs)}] {pdf.name}")
        typer.echo(f"{'='*60}")
        try:
            ingest(str(pdf), preset=preset, dry_run=False, skip_cui=skip_cui, scope=scope)
        except SystemExit:
            typer.echo(f"  FAILED — continuing with next document")
        if batch_monitor:
            batch_monitor.update(item=pdf.name)

    if batch_monitor:
        batch_monitor.finish()
    typer.echo(f"\nBatch complete: {len(pdfs)} documents processed")
