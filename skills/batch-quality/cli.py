...
def _apply_model_override(model: Optional[str]) -> None:
    """Set CHUTES_TEXT_MODEL env var if --model provided."""
    if model:
...
STAGE_12_PROMPT = """Generate a Question-Reasoning-Answer (QRA) pair for this control-technique relationship.

The question should ask HOW the control defends against or relates to the technique.
The reasoning should explain the connection step by step.
The answer should use VERBATIM phrases from the source text as evidence.

Return JSON:
{
    "question": "How does [control] defend against [technique]?",
    "reasoning": "Step-by-step explanation of the relationship...",
    "answer": "The answer using exact quotes from source...",
    "citations": ["exact quote 1", "exact quote 2"]
}

CRITICAL: Citations must be EXACT phrases from the source text."""

STAGE_10B_PROMPT = """You are Brandon Bailey, a space cybersecurity expert.
Validate which CWE weaknesses are relevant to this security control.

For each potential CWE, determine if it ACTUALLY applies to this control.
Be STRICT - reject CWEs that don't clearly apply.

Return JSON array:
[{"cwe_id": "CWE-XXX", "relevant": true/false, "confidence": 0.0-1.0, "reason": "brief"}]"""
...
def get_sample_content(run_id: str, stage: str, n_samples: int) -> list[dict]:
    ...
    try:
        ...
        elif stage in ["12", "12_qra"]:
            # Stage 12: Sample from relationships without QRAs
            # Join path: relationships → controls → control_urls → url_knowledge
            query = """
                SELECT r.relationship_id,
                       c.name as control_name,
                       c.description as control_desc,
                       t.name as technique_name,
                       uk.text as source_text
                FROM relationships r
                JOIN controls c ON r.source_control_id = c.control_id
                LEFT JOIN s01_raw__sparta_techniques t ON r.technique_id = t.technique_id
                LEFT JOIN control_urls cu ON c.control_id = cu.control_id
                LEFT JOIN url_knowledge uk ON cu.url_id = uk.url_id
                LEFT JOIN qra q ON r.relationship_id = q.relationship_id
                WHERE q.qra_id IS NULL
                ORDER BY RANDOM() LIMIT ?
            """
            result = conn.execute(query, [n_samples]).fetchall()
            for row in result:
                rel_id, ctrl_name, ctrl_desc, tech_name, source_text = row
                # Build content from available fields
                content_parts = []
                if ctrl_name:
                    content_parts.append(f"Control: {ctrl_name}")
                if ctrl_desc:
                    content_parts.append(f"Description: {ctrl_desc[:500]}")
                if tech_name:
                    content_parts.append(f"Technique: {tech_name}")
                if source_text:
                    content_parts.append(f"Source: {source_text[:1500]}")
                samples.append({
                    "id": rel_id,
                    "content": "\n".join(content_parts) if content_parts else "N/A"
                })

        elif stage in ["10b", "10b_cwe_mapping"]:
            # Stage 10b: Sample controls for CWE mapping
            query = """
                SELECT c.control_id, c.name, c.description
                FROM controls c
...
def check_knowledge_coverage(run_id: str, results: dict) -> bool:
    """Check knowledge coverage in SPARTA database."""
    conn = get_duckdb_connection(run_id)
    if not conn:
    ...
    typer.echo("\n  [check] Knowledge Coverage Analysis:")
    
    try:
        # Get coverage by control type
        query = """
            SELECT 
                c.control_type,
                COUNT(DISTINCT c.control_id) as total,
                COUNT(DISTINCT CASE WHEN uk.text IS NOT NULL AND uk.text != '' THEN c.control_id END) as with_knowledge
            FROM controls c
            LEFT JOIN control_urls cu ON c.control_id = cu.control_id
            LEFT JOIN url_knowledge uk ON cu.url_id = uk.url_id
        ...
        total_controls = 0
        total_with_knowledge = 0
        failed_types = []
        
        typer.echo(f"    {'Control Type':<25} | {'Knw':<5} | {'Tot':<5} | {'Cov %':<6}")
        typer.echo(f"    {'-'*25}-|-{'-'*5}-|-{'-'*5}-|-{'-'*6}")
        
        for row in rows:
            ctype, tot, knw = row
            cov = (knw / tot) * 100 if tot > 0 else 0
            
            status = "✅" if cov >= 10 else "❌"
            if cov < 1:
                failed_types.append(ctype)
            
            typer.echo(f"    {ctype:<25} | {knw:<5} | {tot:<5} | {cov:>5.1f}% {status}")
            
            total_controls += tot
            total_with_knowledge += knw
            
        overall_cov = (total_with_knowledge / total_controls) * 100 if total_controls > 0 else 0
        typer.echo(f"    {'-'*25}-|-{'-'*5}-|-{'-'*5}-|-{'-'*6}")
        typer.echo(f"    {'TOTAL':<25} | {total_with_knowledge:<5} | {total_controls:<5} | {overall_cov:>5.1f}%")
        
        results["checks"].append({
            "name": "knowledge_coverage", 
            "status": "pass" if not failed_types else "fail",
            "coverage": overall_cov,
            "failed_types": failed_types
        })
        
        if failed_types:
            typer.echo(f"\n  [FAIL] Zero coverage for: {', '.join(failed_types)}")
            typer.echo(f"  [Action] Run /fetcher for these control types")
            return False
            
        return True

    except Exception as e:
        typer.echo(f"  [warn] Coverage check failed: {e}")
        results["checks"].append({"name": "knowledge_coverage", "status": "warn", "error": str(e)})
        return True
    finally:
        conn.close()
...
def preflight(
    ...
    check_knowledge_coverage_flag: bool = typer.Option(False, "--check-knowledge-coverage", help="Fail if knowledge coverage is low"),
    ...
    if api_key:
        typer.echo(f"  [check] SCILLM_PROXY_KEY set")
        env_checks.append({"name": "api_key", "status": "pass"})
    else:
        typer.echo(f"  [FAIL] SCILLM_PROXY_KEY not set")
    ...
    # Check X: Knowledge Coverage (if requested)
    if check_knowledge_coverage_flag and run_id:
        if not check_knowledge_coverage(run_id, results):
            env_passed = False
...
Extract factual excerpts as JSON: {"excerpts": [{"text": "...", "topic": "...", "excerpt_type": "technique|vulnerability|defense"}], "source_quality": "high|medium|low"}"""
        ...
        # Test each sample
        passed = 0
        monitor = TaskClient("batch-quality", total=len(sample_items[:samples])) if TaskClient else None
        for i, item in enumerate(sample_items[:samples]):
            ...
            if monitor:
                monitor.update(item=item.get('id', f'sample_{i+1}'))
        if monitor:
            monitor.finish()
    ...
    if not conn:
        ...
        # Generic quality checks if no contract
            ...
            if stage in ["05", "05_extract_knowledge"]:
                ...
                tables_to_check = [
                    ("qra_count", "SELECT COUNT(*) FROM qra", 5),
                    ("qra_grounding", "SELECT AVG(grounding_score) FROM qra WHERE grounding_score > 0", 0.65),
                ]
            elif stage in ["10b", "10b_cwe_mapping"]:
                tables_to_check = [
                    ("cwe_mapping_count", "SELECT COUNT(*) FROM control_cwe_mappings", 10),
                    ("controls_with_cwes", "SELECT COUNT(DISTINCT control_id) FROM control_cwe_mappings", 5),
                    ("cwe_diversity", "SELECT COUNT(DISTINCT cwe_id) FROM control_cwe_mappings", 3),
                ]
            else:
                tables_to_check = []
