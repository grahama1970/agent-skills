...
- 2026-08-13: `runtime_verification_requests.jsonl` now emits `ingest-code.runtime_verification_request.v1` rows derived from static debugger candidates. Requests are freshness-bound and verifiable without running target code; ready means only that a downstream Tau/debugger workflow may attempt verification under declared limits.
...
| runtime_verification_request.py | Freshness-bound static request emitter/verifier for downstream runtime verification workflows |
