...
echo "=== [best-practices-competition] Sanity Check ==="
...
if meta["name"] != "best-practices-competition":
    raise SystemExit(f"FAIL: wrong skill name: {meta['name']!r}")
for dep in ("ask", "best-practices-tau-dag", "brave-search", "github-search", "dogpile"):
...
for phrase in (
    ...
    "does not share participant information",
    "brave-search",
    "github-search",
    "dogpile",
...
echo -n "Check 2 - Ask composes competition practices: "
if ! grep -A25 '^composes:' "$ASK_SKILL" | grep -q 'best-practices-competition'; then
    echo "FAIL"
    exit 1
fi
echo "PASS"

echo -n "Check 3 - Ask body points compete to this skill: "
if ! grep -q 'best-practices-competition' "$ASK_SKILL"; then
    echo "FAIL"
    exit 1
fi
echo "PASS"
