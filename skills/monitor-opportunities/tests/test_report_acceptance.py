...
def test_report_acceptance_passes_for_receipt_backed_run(tmp_path: Path) -> None:
    ...
    assert payload["checks"]["truth_status_present"] is True
    assert payload["checks"]["truth_status_not_withheld"] is True
    assert payload["counts"]["opportunities"] <= 8
    assert (out / "report-acceptance-receipt.json").is_file()
    assert (out / "truth-status.json").is_file()
...
def test_report_acceptance_requires_stage_ledger_when_promoted(tmp_path: Path) -> None:
    out = _run_fixture(tmp_path)
    replay = runner.invoke(app, ["replay", "--run", str(out)])
    assert replay.exit_code == 0, replay.output
    ...
    assert payload["checks"]["stage_ledger_present"] is False
    assert any(row["check"] == "stage_ledger_present" for row in payload["failures"])


def test_report_acceptance_accepts_required_passing_stage_ledger(tmp_path: Path) -> None:
    out = _run_fixture(tmp_path)
    replay = runner.invoke(app, ["replay", "--run", str(out)])
    assert replay.exit_code == 0, replay.output
        ...
        json.dumps(
            {
                ...
                "ok": True,
                "counts": {"discovered": 1, "accepted": 1, "rejected": 0},
                "violations": [],
            },
            indent=2,
            sort_keys=True,
    ...
    assert payload["checks"]["stage_ledger_ok"] is True
