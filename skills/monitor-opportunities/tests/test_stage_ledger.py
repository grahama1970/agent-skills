...
def test_explicit_eligible_receipts_account_below_limit_rows() -> None:
    ok, ledger = build_ledger(
        [_c("a"), _c("b"), _c("ghost")],
        [_c("a")],
        [],
    ...
    assert any(v["candidate_id"] == "ghost" for v in ledger["violations"])


def test_readback_promoted_linkedin_locator_is_accounted(tmp_path: Path) -> None:
    run = tmp_path / "run"
    locator_url = "https://www.linkedin.com/jobs/view/corporate-ai-security-architect-at-moog-inc"
    primary_url = (
        "https://moog.wd5.myworkdayjobs.com/MOOG_External_Career_Site/job/"
        "Buffalo-NY/Corporate-AI-Security-Architect_R-26-19040"
    )
    _write_jsonl(
        run / "discovery" / "candidates.jsonl",
        [
            {
                "candidate_id": "candidate:a:workday-moog",
                "lane": "A",
                "primary_evidence_url": primary_url,
                "posting_url": primary_url,
            },
            {
                "candidate_id": "candidate:a:linkedin-moog",
                "lane": "A",
                "primary_evidence_url": locator_url,
                "posting_url": locator_url,
            },
        ],
    )
    _write_jsonl(run / "discovery" / "source-receipts.jsonl", [])
    _write_json(run / "ranking" / "shortlist.json", [_c("candidate:a:workday-moog")])
    _write_json(run / "ranking" / "source-intel-shortlist.json", [])
    _write_json(run / "ranking" / "rejections.json", [])
    _write_json(
        run / "ranking" / "ranking-receipt.json",
        {"admitted": 1, "duplicates_merged_into": {}},
    )
    _write_jsonl(
        run / "ranking" / "eligibility-receipts.jsonl",
        [
            {
                "candidate_id": "candidate:a:workday-moog",
                "state": "ELIGIBLE_WNY_ONSITE",
            }
        ],
    )
    _write_jsonl(
        run / "ranking" / "readback-receipts.jsonl",
        [
            {
    ...
    assert ok, ledger["violations"]
    assert ledger["counts"]["accepted"] == 1
