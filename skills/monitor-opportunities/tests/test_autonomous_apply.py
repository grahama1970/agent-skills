...
"""Application-prep resolver: truthful auto-fill, honest queue, never guess."""
...
def test_ready_for_human_authorization_when_required_all_resolve() -> None:
    ...
    assert r["ready_for_human_authorization"] is True
    assert r["required_queued"] == []


def test_not_ready_for_human_authorization_when_required_field_queued() -> None:
    form = {"provider": "ashby", "site": "x", "fields": [
        _f("Email"), _f("Salary expectation"),  # required + placeholder -> blocks
    ]}
    r = resolve_application(form, "/r.pdf", BANK)
    assert r["ready_for_human_authorization"] is False
