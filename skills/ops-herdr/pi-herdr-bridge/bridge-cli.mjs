...
if (command === "list") {
  ...
  const { flushDeferred } = await import("./flush.mjs");
  const result = await flushDeferred({
    dryRun: Boolean(args["dry-run"]),
    ttlMs: args.ttl ? Number(args.ttl) * 60000 : undefined,
    fromName: args.from || "herdr-bridge-flush",
  });
  process.stdout.write(JSON.stringify(result, null, 2) + "\n");
  process.exit(result.ok ? 0 : 1);
} else {
  fail("usage: bridge-cli.mjs <list|send|listen|inbox|flush> [options]");
}
