...
test("roster keeps Herdr-only terminal sessions discoverable", () => {
  const herdrOnly = normalizeHerdrAgents({
    result: { agents: [
      { agent: "pi", agent_status: "idle", cwd: "/repo/shame", pane_id: "w9:p1", tab_id: "w9:t1", terminal_id: "term-shame", terminal_title_stripped: "shame", workspace_id: "w9" },
    ] },
  });
  const roster = mergeRoster([], herdrOnly);
  const resolved = resolveTarget(roster, "shame").entry;
  assert.equal(resolved.source, "herdr");
  assert.equal(resolved.paneId, "w9:p1");
