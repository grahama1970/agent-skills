...
Targets resolve in priority order: intercom name, Herdr terminal title, Herdr
tab label (from `herdr tab list`, joined on `tab_id`), session-ref value, pane
id. A tab named `devops` is addressable as `--to devops` even when its pane
title is `π - agent-skills`; `list` shows `{tab:<label>}` for enriched panes.
Tab-label enrichment is best-effort and degrades to pane-id/session-ref
resolution if `herdr tab list` is unavailable.

## CLI

```bash
node bridge-cli.mjs list [--json]
node bridge-cli.mjs send --to <name|terminal-title|tab-label|session-ref|pane-id> --text "..." [--from <name>] [--expects-reply]
