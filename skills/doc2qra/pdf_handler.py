...
def extract_pdf_accurate(pdf_path: Path, stream: bool = True) -> str:
    ...
    # Use scillm config for unified model selection (all LLM calls via scillm)
    # marker-pdf is a subprocess that needs raw OpenAI-compatible credentials,
    # so we pass scillm's resolved endpoint directly.
    from .config import get_scillm_config
    scillm_cfg = get_scillm_config()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # LLM enhancement flags — uses scillm-resolved API credentials
        llm_flags = [
            "--use_llm",
            "--llm_service", "marker.services.openai.OpenAIService",
            "--openai_base_url", scillm_cfg["api_base"],
            "--openai_api_key", scillm_cfg["api_key"],
            "--openai_model", scillm_cfg["model"],
        ]
    ...
    if raw.lstrip().startswith("{"):
        try:
            ...
            if isinstance(data, dict) and ("transcript" in data or "full_text" in data) and "meta" in data:
                text = _extract_transcript_text(data, p.stem)
                if text.strip():
                    log(f"Transcript extracted ({len(text):,} chars)", style="bold cyan")
                    return text
...
def _extract_transcript_text(data: dict, fallback_title: str = "") -> str:
    """Extract clean text from YouTube transcript JSON.

    Produces a structured document with metadata header + full transcript,
    matching /extractor's JSONProvider output for pipeline consistency.
    """
    meta = data.get("meta", {})
    ...
    if upload_date and len(upload_date) == 8:
        meta_lines.append(f"Date: {upload_date[:4]}-{upload_date[4:6]}-{upload_date[6:]}")
    if duration_sec:
        meta_lines.append(f"Duration: {duration_sec // 60} minutes")
    if meta_lines:
        parts.append("\n".join(meta_lines))

    # Description (skip boilerplate)
    if description:
        desc_lines = []
        for line in description.split("\n"):
            stripped = line.strip()
            if stripped.startswith(("http", "#", "SUBSCRIBE", "RECOMMENDED", "Looking for")):
                break
    ...
    # Full transcript as single block
