...
def build_sections(
    ...
    for title, content in sections:
        ...
        for sent in sents:
            # Force-split oversized "sentences" (e.g. unpunctuated transcripts)
            # at word boundaries to stay under max_section_chars
            if len(sent) > max_section_chars:
                words = sent.split()
                sub_chunk: List[str] = []
                sub_chars = 0
                for word in words:
                    if sub_chars + len(word) + 1 > max_section_chars and sub_chunk:
                        chunk_title = f"{title} (part {part_num})" if title else f"Part {part_num}"
                        result.append((chunk_title, " ".join(sub_chunk)))
                        part_num += 1
                        sub_chunk = []
                        sub_chars = 0
                    sub_chunk.append(word)
                    sub_chars += len(word) + 1
                if sub_chunk:
                    # Remaining words become a "sentence" for the main loop
                    sent = " ".join(sub_chunk)
                else:
                    continue
