...
async def generate_summary_async(
    ...
    if not config["api_key"]:
        log("CHUTES_API_KEY not set, using heuristic summary", style="yellow")
        return _heuristic_summary(content)

    # Try to import scillm
    try:
    ...
    # Truncate content to fit in context window (leave room for response)
    max_chars = 12000  # ~3000 tokens, leaving room for response
    ...
    try:
        ...
        log(f"Summary generation failed: {e}", style="red")
        return _heuristic_summary(content)
...
def _heuristic_summary(content: str, max_length: int = 500) -> str:
