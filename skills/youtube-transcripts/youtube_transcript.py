...
# ============================================================================
# TRANSCRIPT CLEANING - Noise patterns and content detection
# ============================================================================
...
NOISE_PATTERNS = [
    ...
    r"^\[Applause\]$",
    r"^\[Laughter\]$",
    r"^fore$",  # Common YouTube auto-caption artifact
    r"^\s*$",  # Empty
]
NOISE_REGEX = [re.compile(p, re.IGNORECASE) for p in NOISE_PATTERNS]

# Gaming content indicators (if these dominate, it's not lore)
GAMING_INDICATORS = [
    "subscribe", "like the video", "patreon", "hit the bell",
    "gameplay", "let's play", "arma", "squad", "milsim",
    "good game", "gg", "we won", "alpha team", "bravo team",
]

# Lore content indicators (Warhammer 40k focused, but can be extended)
LORE_INDICATORS = [
    "emperor", "primarch", "horus", "heresy", "astartes", "space marine",
    "chaos", "warp", "imperium", "terra", "crusade", "legion",
    "sanguinius", "magnus", "lorgar", "angron", "fulgrim",
    "thousand sons", "death guard", "world eaters", "luna wolves",
]

# Entity extraction (basic rule-based)
DEFAULT_ENTITIES = {
    "primarchs": ["Horus", "Sanguinius", "Fulgrim", "Angron", "Magnus", "Lorgar", "Roboute", "Guilliman", "Vulkan", "Ferrus", "Perturabo", "Mortarion", "Konrad", "Alpharius", "Corax", "Rogal", "Dorn", "Lion", "Khan", "Russ"],
    "emperor": ["Emperor", "Master of Mankind", "Golden Throne"],
    "chaos_gods": ["Khorne", "Nurgle", "Tzeentch", "Slaanesh", "Chaos"],
    "events": ["Horus Heresy", "Siege of Terra", "Isstvan", "Davin", "Great Crusade", "Unification Wars"],
    "locations": ["Terra", "Davin", "Prospero", "Calth", "Macragge", "Baal", "Fenris"],
}

MIN_WORD_COUNT = 100  # Skip very short transcripts


def _is_noise(text: str) -> bool:
    """Check if segment is noise to filter."""
    if len(text.strip()) < 3:
        return True
...
def _detect_content_type(full_text: str) -> str:
    """Detect if content is lore, gaming, or mixed."""
    text_lower = full_text.lower()

    lore_score = sum(1 for term in LORE_INDICATORS if term in text_lower)
    gaming_score = sum(1 for term in GAMING_INDICATORS if term in text_lower)
    ...
    lore_density = lore_score / (word_count / 1000)
    gaming_density = gaming_score / (word_count / 1000)

    if lore_score > 5 and lore_density > gaming_density * 2:
        return "lore"
    elif gaming_score > 3 and gaming_density > lore_density * 2:
        return "gaming"
    elif lore_score > 0 and gaming_score > 0:
        return "mixed"
    elif lore_score > 0:
        return "lore"
    elif gaming_score > 0:
...
def _extract_entities(text: str, entities: dict | None = None) -> list[str]:
...
def _clean_transcript_data(raw_data: dict, video_id: str = "", channel: str = "") -> dict:
    """
    Clean a raw transcript into LLM-friendly format.

    Args:
        raw_data: Original JSON with meta and transcript fields
        video_id: YouTube video ID (optional, extracted from meta if not provided)
    ...
    final_video_id = video_id or meta.get("video_id", "")
    final_channel = channel or meta.get("channel", "")
    title = meta.get("title", "")
    description = meta.get("description", "")

    # Combine text segments, filtering noise
    text_parts = []
    ...
    # Extract entities
    entities_found = _extract_entities(full_text)
    ...
    return {
        ...
        "original_segment_count": len(transcript),
        "cleaned_segment_count": len(text_parts),
...
def get(
    ...
    clean: bool = typer.Option(False, "--clean", help="Also include cleaned version (noise filtered, entities extracted)"),
    channel: str = typer.Option("", "--channel", "-c", help="Channel name to tag transcript with (used with --clean)"),
):
    ...
    # Add cleaned version if requested
    if clean:
        cleaned_data = _clean_transcript_data(out, vid, channel or metadata.get("channel", ""))
        out["cleaned"] = {
            "full_text": cleaned_data["full_text"],
            "word_count": cleaned_data["word_count"],
            "content_type": cleaned_data["content_type"],
            "entities_found": cleaned_data["entities_found"],
            "original_segment_count": cleaned_data["original_segment_count"],
            "cleaned_segment_count": cleaned_data["cleaned_segment_count"],
        }
...
@app.command()
def clean(
    input_path: str = typer.Option(..., "--input", "-i", help="Input JSON file or directory of transcripts"),
    output_dir: str = typer.Option("", "--output", "-o", help="Output directory for cleaned transcripts (default: input/clean)"),
    channel: str = typer.Option("", "--channel", "-c", help="Channel name to tag transcripts with"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Show stats without writing files"),
    min_words: int = typer.Option(MIN_WORD_COUNT, "--min-words", help=f"Skip transcripts under N words (default: {MIN_WORD_COUNT})"),
):
    """Clean raw transcripts into LLM-friendly format.

    Cleans transcripts by:
    - Combining segments into single full_text field
    - Filtering noise ([Music], [Applause], etc.)
    - Detecting content type (lore/gaming/mixed)
    - Extracting named entities

    Works with single files or entire directories.

    Examples:
        python youtube_transcript.py clean -i dQw4w9WgXcQ.json
        python youtube_transcript.py clean -i ./transcripts -o ./clean
        python youtube_transcript.py clean -i ./transcripts --dry-run
    """
    ...
    if output_dir:
        ...
        out_dir = input_p.parent / "clean"
    else:
        out_dir = input_p / "clean"

    # Get list of files to process
    if input_p.is_file():
        files = [input_p]
    else:
        files = sorted(input_p.rglob("*.json"))
        # Exclude clean/ directory and state files
        files = [f for f in files if "clean" not in str(f) and not f.name.startswith(".")]

    typer.echo(f"Processing {len(files)} files...", err=True)
    if not dry_run:
        out_dir.mkdir(parents=True, exist_ok=True)
        typer.echo(f"Output directory: {out_dir}", err=True)

    # Process files
    results = []
    stats = {"lore": 0, "gaming": 0, "mixed": 0, "unknown": 0, "skipped": 0, "error": 0}
    ...
    for file_path in files:
        ...
        cleaned = _clean_transcript_data(raw_data, video_id, channel)
        ...
        if dry_run:
            results.append({
                ...
                "entities": len(cleaned["entities_found"]),
                "dry_run": True,
            })
            content_type = cleaned["content_type"]
        ...
        if cleaned["word_count"] < min_words:
            results.append({
                "video_id": video_id,
                "skipped": True,
                "reason": f"too short ({cleaned['word_count']} words)",
            })
            stats["skipped"] += 1
            continue

        # Write cleaned output
        output_file = out_dir / f"{video_id}.json"
    ...
    typer.echo(f"Total files:     {len(files)}", err=True)
    typer.echo(f"Lore content:    {stats['lore']}", err=True)
    typer.echo(f"Gaming content:  {stats['gaming']}", err=True)
    typer.echo(f"Mixed content:   {stats['mixed']}", err=True)
    typer.echo(f"Unknown:         {stats['unknown']}", err=True)
    typer.echo(f"Skipped:         {stats['skipped']}", err=True)
    typer.echo(f"Errors:          {stats['error']}", err=True)
    typer.echo("=" * 60, err=True)

    if not dry_run:
        typer.echo(f"\nCleaned files written to: {out_dir}", err=True)

    print(json.dumps({"results": results, "stats": stats, "output_dir": str(out_dir)}, indent=2))


@app.command("get-clean")
def get_clean(
    url: str = typer.Option("", "--url", "-u", help="YouTube video URL"),
    video_id: str = typer.Option("", "--video-id", "-i", help="YouTube video ID"),
    lang: str = typer.Option("en", "--lang", "-l", help="Language code"),
    channel: str = typer.Option("", "--channel", "-c", help="Channel name to tag transcript with"),
    no_proxy: bool = typer.Option(False, "--no-proxy", help="Skip proxy tier"),
    no_whisper: bool = typer.Option(False, "--no-whisper", help="Skip Whisper fallback"),
    retries: int = typer.Option(3, "--retries", "-r", help="Max retries per tier"),
    raw: bool = typer.Option(False, "--raw", help="Also include raw transcript segments in output"),
):
    """Get transcript AND clean it in one step.

    Combines `get` and `clean` commands - fetches transcript, then cleans it.
    Output is LLM-friendly format with full_text, content_type, entities.

    Examples:
        python youtube_transcript.py get-clean -i dQw4w9WgXcQ
        python youtube_transcript.py get-clean -u "https://youtube.com/watch?v=..." --raw
    """
    t0 = time.time()
    ...
    vid = _extract_video_id(video_id or url)
    if not vid:
        out = {"error": "Could not extract video ID from URL or --video-id"}
        print(json.dumps(out, ensure_ascii=False, indent=2))
        raise typer.Exit(code=1)
    ...
    if errors and not no_whisper and os.getenv("OPENAI_API_KEY"):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if dl_error:
                ...
                typer.echo("  Transcribing with local Whisper...", err=True)
                transcript, full_text, whisper_error = _transcribe_whisper_local(audio_path, lang)
                if whisper_error and os.getenv("OPENAI_API_KEY"):
                    typer.echo("  Local failed, trying Whisper API...", err=True)
                    transcript, full_text, whisper_error = _transcribe_whisper(audio_path, lang)
                if whisper_error:
                    all_errors.append(f"Tier 3 (whisper): {whisper_error}")
    ...
    # Build raw data structure for cleaning
    raw_data = {
        "meta": {
            "video_id": vid,
            "language": lang,
    ...
    cleaned = _clean_transcript_data(raw_data, vid, channel)
    ...
    # Optionally include raw segments
    if raw:
        out["raw_segments"] = transcript
