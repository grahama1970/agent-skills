...
### Pre-hook: `recall_prior_stories(theme, genre, k=5)`
Recalls prior creative patterns, character arcs, and narrative decisions to inform new story creation.

### Post-hook: `learn_story(title, genre, themes, characters, narrative_decisions, ...)`
Learns story decisions after completion — genre, themes, characters, techniques, and scores.

### Bridge Keywords
| Bridge | Keywords |
|--------|----------|
| Precision | craft, technique, structure, pacing |
| Resilience | arc resolution, catharsis, growth, redemption |
| Fragility | plot hole, inconsistency, weak, flat |
| Stealth | subtext, symbolism, foreshadowing, metaphor |

Tags: `["create_story", genre] + bridges`

## Legacy Memory Integration

Stories are also stored in `/memory` with scope `horus-stories`:
...
### WRONG: Writing from nothing without research phase
```bash
./run.sh draft "A story about a robot discovering emotions"  # no research context
```

### RIGHT: Full workflow with research first
```bash
./run.sh create "A story about a robot discovering emotions"
# This runs: research → dogpile → draft → critique → refine
```

### WRONG: Skipping /memory recall for past creative patterns
```bash
./run.sh create "A noir screenplay"  # ignores what Horus has written before
```

### RIGHT: Recall prior stories informs new creation
```bash
# create already calls /memory recall internally, but when drafting manually:
/memory recall "noir screenplay techniques" --scope horus-stories
```

### WRONG: Using only self-critique for all drafts
```bash
./run.sh create "A story" --format screenplay  # default self-critique only
```

### RIGHT: Use --external-critique for objective analysis
```bash
./run.sh create "A story" --format screenplay --external-critique
```
