...
- Confirm JSON-only mediation: should status values be exactly needs_input, in_progress, complete, error, and include resume_command?
- Shot approvals: accept batch defaults with per-shot overrides (approve/modify/skip) and scene-level defaults?
- Integration: file-based session handoff under output_dir/session_id with JSON artifacts is acceptable for /create-movie?
...
+    Emits JSON with status and resume_command when input is required.
+    """
...
+):
+    """
+    Continue a session: apply answers, advance phase, ask next questions or complete.
+    """
...
Total duration (API):  22.6s
Total duration (wall): 24.3s
Total code changes:    0 lines added, 0 lines removed
Usage by model:
    gpt-5                44.7k input, 3.7k output, 0 cache read, 0 cache write (Est. 1 Premium request)
