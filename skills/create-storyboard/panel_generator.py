...
# Feature flags default off to avoid altering current sequencing/layout
STORYBOARD_FLAGS = {
    "ENABLE_LAYOUT_INVARIANTS": os.environ.get("STORYBOARD_ENABLE_LAYOUT_INVARIANTS", "0") == "1",
}

def _enforce_layout_invariants(panels: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def generate_ai_panel(shot: dict, config: PanelConfig, output_path: Path) -> Path:
    """
    Generate an AI-generated panel using create-image skill (Nano Banana/Fal.ai).
    """
    import subprocess
    import shutil
    ...
    shot_type = shot.get('shot_name', '')
    visual_style = "cinematic, high quality, photorealistic, 8k" # Default style
    
    # Enhance prompt with camera info
    camera = shot.get('camera_movement', '')
    lighting = shot.get('lighting', '') # If available
    
    prompt = f"{description}, {shot_type}, {camera}, {lighting}, {visual_style}"
    
    # Locate create-image script
    # Assuming standard relative directory structure: ../create-image/generate.py
    current_dir = Path(__file__).parent
    generate_script = current_dir.parent / "create-image" / "generate.py"
    
    if not generate_script.exists():
        # Fallback if structure differs
        print(f"Warning: create-image script not found at {generate_script}. Falling back to sketch.")
        return generate_sketch_panel(shot, config, output_path)

    try:
        # Call generate.py via uv run
        # Use --backend fal and Nano Banana model
        cmd = [
            "uv", "run", "--script", str(generate_script),
            "generate",
            prompt,
            "--output", str(output_path),
            "--size", f"{config.width}x{config.height}",
            "--backend", "fal",  # Force fal backend
            "--model", "fal-ai/nano-banana-pro"
        ]
        
        # print(f"Generating AI panel for shot {shot.get('shot_number')}...")
        result = subprocess.run(cmd, capture_output=True, text=True)
        
        if result.returncode == 0 and output_path.exists():
            return output_path
        else:
            print(f"AI Generation failed: {result.stderr}")
            return generate_sketch_panel(shot, config, output_path)
...
def generate_panels(
    ...
    for shot in shot_plan.get('shots', []):
        ...
        if fidelity == 'sketch':
            ...
            generate_ai_panel(shot, config, output_path)
        else:
            generate_sketch_panel(shot, config, output_path)
