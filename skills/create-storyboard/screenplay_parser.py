...
# Support markdown headings: ## INT. LOCATION - TIME
SCENE_HEADING_PATTERN = re.compile(
    r'^#{0,6}\s*(INT\.|EXT\.|INT/EXT\.|I/E\.)\s*(.+?)\s*[-–—]\s*(.+?)$',
    re.IGNORECASE | re.MULTILINE
)

# Alternative: Simpler scene heading (just INT./EXT. location)
# Support markdown headings: ## INT. LOCATION
SIMPLE_SCENE_HEADING = re.compile(
    r'^#{0,6}\s*(INT\.|EXT\.|INT/EXT\.|I/E\.)\s*(.+?)$',
    re.IGNORECASE | re.MULTILINE
)

# Character name (ALL CAPS, optionally with parenthetical)
# Supports: CHARACTER, CHARACTER (whisper), **CHARACTER**, **CHARACTER (whisper)**
        ...
        if current_character and current_scene:
            ...
            if paren_match:
                ...
                if i < len(lines) and lines[i].strip():
                    ...
                    # Handle markdown blockquote dialogue
                ...
                # This is regular dialogue - handle markdown blockquote
