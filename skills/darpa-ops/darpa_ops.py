...
def fetch_url(url: str, method: str = "GET", data: bytes = None, headers: dict = None) -> str:
    ...
    req.add_header("User-Agent", "darpa-ops/1.0")
    if headers:
...
def generate(
    ...
    """
    Generate DARPA proposal draft using paper-writer.
    ...
    # Find paper-writer skill
    skill_paths = [
        Path(__file__).parent.parent / "paper-writer" / "run.sh",
        Path.home() / ".pi" / "skills" / "paper-writer" / "run.sh",
    ]
    ...
    if not paper_writer:
        console.print("[red]paper-writer skill not found.[/red]")
        console.print("Install it from: .pi/skills/paper-writer")
        raise typer.Exit(1)
    
    console.print(f"\n[dim]Using paper-writer: {paper_writer}[/dim]")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            console.print(f"[red]Error running paper-writer:[/red]")
            console.print(result.stderr)
            raise typer.Exit(1)
    except subprocess.TimeoutExpired:
        console.print("[red]Timeout: paper-writer took too long[/red]")
        raise typer.Exit(1)
    except Exception as e:
...
def version():
    """Show version."""
    console.print("darpa-ops v0.1.0")
