...
- IMMUTABLE GOAL v2 (human goal change, 2026-08-22): follow ANY consented meeting and surface three card families -- research cards (bounded external), memory-recall cards (Graph Memory prior knowledge), and code cards -- plus briefing packs and human-approved actions; cards must be glance-scannable by a human whose attention is on the conversation (three-second blinded reading bar), and insufficient local evidence auto-PROPOSES bounded external research for approval. The v1 coding-interview proof is achieved and stays binding as the regression floor; v2's completion standard is a frozen-build daily-use campaign (20+ real sessions, onset-to-card latency measured from speech start, zero policy violations).
...
- Suite: 32 cases; last full 30-case run READY with 60/60 trials (le-30-final); case 31 (briefing) runner-verified PASS 2/2; case 32 (meeting-campaign-goal-v2) green at meeting-campaign/20260822T191307Z — real chatterbox-rendered Sparta meeting + pinned interview wav through the live audio path, all three card families correct, onset-to-card 20-22s (synthetic) / ~109s (interview wav question position). The campaign forced four product fixes now landed: newest-completed-card-wins publication fences, deterministic question forking (<30% vocabulary overlap opens a new question id), lane-diversity card-source selection, and a problem-statement trigger (>=2 declarative markers like "we're given"/"return the" open the resolver gate — a code walkthrough that never phrases a question now cards in ~8s, and the pinned interview's onset-to-card fell from ~109s to ~20s because the spoken problem statement itself triggers). Third session youtube-two-sum: public NeetCode Two Sum audio (yt-dlp, first 150s), oracle derived from its auto-captions before the run; fixture code lives in fixtures/code-fixture-repo/. The scorer also reports speech_end_to_card_s (sub-second for most questions) and waits for journal quiescence instead of a fixed settle.
