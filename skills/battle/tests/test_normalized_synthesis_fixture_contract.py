...
"""test_normalized_synthesis_fixture_contract - tests.
...
def test_pr3c_synthesis_fixture_normalizes_provider_authorship_without_runtime_paths(tmp_path: Path) -> None:
    ...
    assert fixture["provider_authorship"]["provider_request_id_present"] is True
    assert fixture["provider_authorship"]["provider_response_id_present"] is True
    assert fixture["provider_authorship"]["provider_call_count"] == 1
    assert fixture["provider_authorship"]["transport_source_hash_bound"] is True
    assert fixture["provider_authorship"]["transport_source_hash_matches"] is True
    assert "provider_run_id" not in fixture["provider_authorship"]
    assert "provider_request_id" not in fixture["provider_authorship"]
    assert "provider_response_id" not in fixture["provider_authorship"]
...
def _write_pr3c_source_run(tmp_path: Path, *, provider_live: bool = True) -> Path:
    ...
    authorship = {
        ...
        "provider_request_id": "request-secret-value",
        "provider_response_id": "response-secret-value",
        "provider_call_count": 1,
        "code_artifact_sha256": code_sha,
        "code_artifact_bytes": code_path.stat().st_size,
        "transport_source_sha256": code_sha,
        "compile_status": "NOT_RUN",
        "runtime_status": "NOT_RUN",
        "target_contact": "NOT_RUN",
        "judge_status": "NOT_RUN",
        ...
        "validation": {
            ...
            "transport_request_id_bound": True,
            "transport_response_id_bound": True,
            "provider_call_count_bound": True,
            "output_declared_by_worker": True,
            "output_hash_bound": True,
            "transport_source_hash_bound": True,
            "transport_source_hash_matches": True,
