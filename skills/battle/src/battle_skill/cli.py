...
@app.command("vm-round")
def vm_round_cmd(
    image: Path = typer.Option(..., help="Base qcow2 image (e.g. Ubuntu cloud image)"),
    payload: Optional[Path] = typer.Option(None, help="Payload script executed inside the VM via SSH"),
    command: Optional[str] = typer.Option(None, help="Inline shell command instead of a payload file"),
    authorization_manifest: Path = typer.Option(..., help="security.target_authorization.v1 manifest binding the image"),
    out_dir: Path = typer.Option(Path("/tmp/battle-vm-rounds"), help="Output directory for receipts"),
    ssh_port: Optional[int] = typer.Option(None, help="Host SSH forward port (default: ephemeral)"),
    boot_timeout: int = typer.Option(300, help="Seconds to wait for SSH"),
    keep: bool = typer.Option(False, help="Keep the round directory (disk overlay, keys, serial log)"),
):
    """Run one full-system VM round: fresh overlay, KVM boot, SSH payload, receipt."""
    if bool(payload) == bool(command):
        raise typer.BadParameter("provide exactly one of --payload or --command")
    script = payload.read_text() if payload else command + "\n"
    import hashlib
    image_hash = hashlib.sha256(image.read_bytes()).hexdigest()
    authorization = validate_target_authorization(
        authorization_manifest,
        expected_target=f"vm-image:{image.resolve()}@sha256:{image_hash}",
        requested_action="qemu-vm-round",
        requested_runtime_mode="qemu",
    )
    authorization["authorization_id"] = json.loads(authorization_manifest.read_text()).get("authorization_id", "")
    if authorization.get("status") != "PASS":
        logger.error("authorization failed: {}", authorization.get("errors"))
        raise typer.Exit(2)
    receipt = run_vm_round(
        image, script,
