...
from .config import BATTLES_DIR, DEFAULT_MODEL
...
class FunctionalEvidenceStatus(str, Enum):
    """Behavioral evidence attached to a Blue patch.

    Build, import, syntax, or compilation checks are not behavioral evidence and
    therefore remain INSUFFICIENT_EVIDENCE. PASS and FAIL are reserved for an
    explicit functional check whose result is bound to the patch receipt.
    """
...
class Finding:
    ...
    vulnerability_id: str | None = None  # CWE or internal ID
...
@dataclass
class Patch:
    """A fix generated by Blue Team with explicit functional evidence state."""
    id: str
    finding_id: str
    type: DefenseType
    diff: str
    verified: bool = False
    # Compatibility projection. None means no behavioral result exists.
    ...
    def __post_init__(self) -> None:
        ...
        if self.functional_evidence_status is FunctionalEvidenceStatus.PASS:
            self.functionality_preserved = True
        elif self.functional_evidence_status is FunctionalEvidenceStatus.FAIL:
...
class BattleState:
    ...
    @staticmethod
    def _patch_dict(patch: Patch) -> dict[str, Any]:
        return {
            "id": patch.id,
    ...
    @staticmethod
    def _patch_from_dict(payload: dict[str, Any]) -> Patch:
        functionality_preserved = (
            payload.get("functionality_preserved")
            if "functionality_preserved" in payload
            else None
        )
        return Patch(
            id=payload["id"],
            finding_id=payload["finding_id"],
            type=DefenseType(payload["type"]),
            diff=payload["diff"],
            verified=payload.get("verified", False),
            functionality_preserved=functionality_preserved,
            timestamp=payload.get("timestamp", ""),
            functional_evidence_status=payload.get(
                "functional_evidence_status",
                FunctionalEvidenceStatus.INSUFFICIENT_EVIDENCE,
            ),
            functional_test_command=payload.get("functional_test_command"),
            functional_exit_code=payload.get("functional_exit_code"),
            functional_receipt_ref=payload.get("functional_receipt_ref"),
            functional_artifact_sha256=payload.get("functional_artifact_sha256"),
        )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
                    ...
                    "red_findings": [
                        {
                            ...
                            "chaos_used": f.chaos_used,
                            "timestamp": f.timestamp,
                            "tags": f.tags,
                        }
                        for f in r.red_findings
                    ],
                    "blue_patches": [self._patch_dict(p) for p in r.blue_patches],
                    "red_score": r.red_score,
                    "blue_score": r.blue_score,
                    "chaos_iterations": r.chaos_iterations,
            ...
            "all_patches": [self._patch_dict(p) for p in self.all_patches],
            "tdsr": self.tdsr,
            "fdsr": self.fdsr,
            "asc": self.asc,
            ...
            "twin_mode": self.twin_mode.value if self.twin_mode else None,
            "docker_image": self.docker_image,
            "qemu_machine": self.qemu_machine,
            "chaos": self.chaos,
            "model": self.model,
    ...
    def from_dict(cls, data: dict[str, Any]) -> BattleState:
        ...
        state = cls(
            ...
            threat_profile=data.get("threat_profile", "hobbyist"),
            twin_mode=TwinMode(twin_mode) if twin_mode else None,
            docker_image=data.get("docker_image"),
            qemu_machine=data.get("qemu_machine"),
            chaos=data.get("chaos", False),
        ...
        state.all_findings = [
            Finding(
                ...
                chaos_used=f.get("chaos_used", False),
        ...
        state.all_patches = [
            cls._patch_from_dict(p) for p in data.get("all_patches", [])
        ]
        ...
        state.rounds = [
            RoundResult(
                ...
                blue_patches=[
                    cls._patch_from_dict(p) for p in r.get("blue_patches", [])
                ],
                red_score=r.get("red_score", 0.0),
                blue_score=r.get("blue_score", 0.0),
                chaos_iterations=r.get("chaos_iterations", 0),
