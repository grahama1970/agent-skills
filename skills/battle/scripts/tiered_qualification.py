...
def validate_same_run(same_run_receipt: Path, out: Path) -> int:
    receipt_in = _read(same_run_receipt)
    source = current_source()
    errors: list[str] = []
    commit, tree = _source_values(receipt_in)
    browser = receipt_in.get("browser") if isinstance(receipt_in.get("browser"), dict) else {}
    cdp_command = browser.get("cdp_command") if isinstance(browser.get("cdp_command"), dict) else {}
    published_fixture = (
        receipt_in.get("published_fixture")
        if isinstance(receipt_in.get("published_fixture"), dict)
        else {}
    )
    ...
    if receipt_in.get("mocked") is not False:
        errors.append("same_run_receipt_not_mocked_false")
    if receipt_in.get("live") is not True:
        errors.append("same_run_receipt_not_live_true")
    if not receipt_in.get("run_id"):
        errors.append("same_run_receipt_missing_run_id")
    if commit != source["commit"]:
        errors.append("same_run_source_commit_stale_or_missing")
    if tree != source["battle_tree"]:
        errors.append("same_run_source_tree_stale_or_missing")
    if browser.get("status") != "PASS":
        errors.append("same_run_browser_status_not_pass")
    if cdp_command.get("exit_code") != 0:
        errors.append("same_run_cdp_command_not_pass")
    if published_fixture.get("fixture_backed") is True:
        errors.append("same_run_fixture_backed")
    if not published_fixture.get("fixture_key") or not published_fixture.get("fixture_sha256"):
    ...
    receipt = {
        ...
        "current_source": source,
        "inputs": {
            "same_run_receipt": str(same_run_receipt),
            "run_id": receipt_in.get("run_id"),
            "source_commit": commit,
            "source_tree": tree,
            "fixture_key": published_fixture.get("fixture_key"),
            "fixture_sha256": published_fixture.get("fixture_sha256"),
            "cdp_read_json": _field(receipt_in, ["browser.cdp_meta.read_json"]),
            "cdp_screenshot": _field(receipt_in, ["browser.cdp_meta.screenshot"]),
        },
        "errors": errors,
        "proves": [
            "A same-run Arena/Tau/Judge receipt was published through Pixi.",
            "The same-run receipt matches the current Battle source commit and tree.",
            "Browser/CDP evidence completed for the same-run Pixi route.",
        ],
        "does_not_prove": [
            "A new Arena or browser run was generated by this validator.",
            "Provider reliability beyond the supplied live same-run receipt.",
        ],
    }
    _write(out, receipt)
    print(json.dumps({"status": receipt["status"], "receipt": str(out), "errors": errors}, indent=2))
...
def main() -> int:
    ...
    same_run = sub.add_parser("same-run-live", help="Validate one live same-run Arena-to-Pixi qualification receipt")
    same_run.add_argument("--same-run-receipt", type=Path, required=True)
    same_run.add_argument("--out", type=Path, required=True)
    ...
    if args.command == "same-run-live":
        return validate_same_run(args.same_run_receipt, args.out)
    return 2
