...
export function BattleToolbar({ mode, setMode, filter, setFilter, query, setQuery }: Props) {
  useRegisterAction("battle:roster:search", { action: "BATTLE_SEARCH", label: "Search Battle roster", description: "Search receipt-backed Battle lanes and events.", tags: ["battle"] });
  ...
  return (
    <section className="flex min-h-[54px] items-center justify-between gap-3 rounded-2xl border border-white/10 bg-battle-panel/70 px-3 py-2 shadow-acrylic backdrop-blur-xl">
      <label className="relative w-[300px] shrink-0">
        ...
        <input data-qid="battle:roster:search" data-qs-action="BATTLE_SEARCH" title="Search payload, agent, or receipt" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search payload, agent, receipt..." className="h-11 w-full rounded-xl border border-white/10 bg-black/20 pl-9 pr-3 text-sm text-slate-200 outline-none transition-colors placeholder:text-slate-600 focus:border-battle-cyan/40" />
      </label>
