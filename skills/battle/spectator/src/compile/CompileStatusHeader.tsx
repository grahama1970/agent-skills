...
function toneToVariant(tone: CompileViewModel["badges"][number]["tone"]) {
...
export function CompileStatusHeader({ model }: { model: CompileViewModel }) {
	const { fixture } = model;
	return (
		<header className="battle-proof-panel" data-qid="battle:compile:status">
			<div className="flex flex-wrap items-start justify-between gap-3">
				<div className="min-w-0">
					<div className="battle-label">
						{fixture.battle_id.toUpperCase()} · {fixture.proof_mode.replace(/_/g, " ").toUpperCase()}
					</div>
					<h1 className="mt-1 text-2xl font-black tracking-tight text-white" data-qid="battle:compile:headline">
						{model.headline}
					</h1>
			...
			<div className="battle-compile-banners" data-qid="battle:compile:banners">
				{model.banners.map((banner) => (
					<span key={banner.id} className="battle-compile-banner" data-tone={banner.tone} data-qid={`battle:compile:banner:${banner.id}`}>
						{banner.label}
					</span>
				))}
			</div>
			...
			<div
				...
				data-qid="battle:compile:execution-boundary"
			>
				<div className="battle-proof-notice" data-qid="battle:compile:boundary-notice">
					{model.boundaryNotice}
				</div>
				{model.executionBoundary.map((row) => (
					<div key={row.id} data-qid={`battle:compile:boundary:${row.id}`}>
			...
			<div className="mt-4 flex flex-wrap gap-2" data-qid="battle:compile:badges">
				{model.badges.map((badge) => (
					<Badge key={badge.id} variant={toneToVariant(badge.tone)} data-battle-compile-badge={badge.id}>
