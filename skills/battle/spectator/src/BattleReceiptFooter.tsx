  ...
  return (
    ...
    >
      <div className="battle-receipt-footer-transport flex min-w-0 items-center gap-1.5">
        <Button data-qid="battle:control:playhead" data-qs-action="BATTLE_REPLAY_PLAYHEAD_TOGGLE" title={playing ? "Pause receipt replay playhead" : "Play receipt replay playhead"} variant={playing ? "green" : "outline"} size="icon" className="h-8 min-h-0 w-8 min-w-0" onClick={() => setPlaying((value) => !value)}>
          {playing ? <Icons.Pause className="h-3.5 w-3.5" /> : <Icons.Play className="h-3.5 w-3.5" />}
        </Button>
        <Button data-qid="battle:control:highlight-next" data-qs-action="BATTLE_HIGHLIGHT_NEXT" title="Jump playhead to next receipt-backed highlight" variant="outline" size="sm" className="h-8 min-h-0 px-2 text-xs" onClick={onJumpToNextHighlight}>Next</Button>
        <Button data-qid="battle:control:highlight-reel" data-qs-action="BATTLE_HIGHLIGHT_REEL_TOGGLE" title={highlightReel ? "Disable highlight-reel transport" : "Play highlight reel (jump between proven terminal beats)"} variant={highlightReel ? "green" : "outline"} size="sm" className="h-8 min-h-0 px-2 text-xs" onClick={() => onHighlightReelChange(!highlightReel)}>Highlights</Button>
        <Button data-qid="battle:control:sound-arm" data-qs-action="BATTLE_SOUND_ARM" title={enabled ? "Spectator mode live" : "Arm sound for receipt-backed events with explicit cues"} variant={enabled ? "green" : "outline"} size="sm" className="h-8 min-h-0 gap-1.5 px-2 text-xs" onClick={arm}>
          <Icons.Eye className="h-3.5 w-3.5" /> {enabled ? "Live" : "Arm"}
        </Button>
        <ToggleGroup className="h-8" data-qid="battle:control:speed" data-qs-action="BATTLE_SPEED_SET" title="Set receipt-backed replay speed" type="single" value={speed} onValueChange={(value) => value && setSpeed(value)}>
          {["1x", "2x", "4x", "8x"].map((item) => (
            <ToggleGroupItem key={item} data-qid={`battle:control:speed:${item}`} data-qs-action="BATTLE_SPEED_SET" title={`Set replay speed to ${item}`} value={item} className="h-8 min-h-0 min-w-8 px-2 text-xs">{item}</ToggleGroupItem>
          ))}
        </ToggleGroup>
      </div>
