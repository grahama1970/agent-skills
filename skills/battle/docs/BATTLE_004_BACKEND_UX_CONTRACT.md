...
- segments
- lanes
- optional actor visual identity/state timeline
- receipts
- provenance
...
## 4.1 Actor visual identity

The backend may emit stable **renderer-neutral** actor visual identity so replay
fixtures stay visually consistent across runs, but this is cosmetic metadata,
not proof.

Schema:

```text
skills/battle/schemas/battle.actor_visual.v1.schema.json
```

Allowed backend-owned visual fields:

```json
{
  "schema": "battle.actor_visual.v1",
  "actor_id": "payload-857-A",
  "lane_id": "payload-857-A",
  "role": "red_exploit",
  "team": "red",
  "archetype": "chaos_marine_light",
  "variant_id": "replay-fork",
  "style_family": "vintage_16bit_genesis",
  "facing": "right",
  "scale_class": "normal_48",
  "initial_state": "spawn",
  ...
  "state_timeline": [
    {
      "at_seconds": 614.0,
      "state": "spawn",
      "source_event_id": "evt-lineage-payload-857-spawn-A",
      "source_receipt_id": "lineage-spawn-payload-857-A"
    },
    {
      "at_seconds": 620.5,
      "state": "run",
      "source_event_id": "evt-payload-857-A-active"
    },
    {
      "at_seconds": 644.2,
      "state": "fastest_crash",
      "source_event_id": "evt-judge-crash-confirmed-payload-857-A",
      "source_receipt_id": "judge-crash-confirmed-payload-857-A"
    }
  ],
  "proof_scope": {
...
`variant_id` is a stable roster id. It is **not** a Pixi alias,
spritesheet path, animation name, frame id, or outcome proof.

The UI/Pixi project maps `variant_id` and semantic `state` values through a
...
Production BATTLE art direction is:
...
hit, blocked, killed, fastest_crash, promoted, stale_pending
```

Receipt gates:

```text
spawn          -> lineage spawn receipt
blocked        -> Blue/Judge block receipt
killed         -> Judge kill receipt
fastest_crash  -> crash proof + timing receipt
promoted       -> promotion receipt
```

The backend can say **what semantic state starts at what time**. The UI/Pixi
engine decides which `AnimatedSprite`, animation name, frame timing, easing,
particles, and effects render that state.
