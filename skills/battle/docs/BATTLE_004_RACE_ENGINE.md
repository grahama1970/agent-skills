...
Authoritative engine contract for the BATTLE-004 center race viewport. Complements `BACKEND_UX_CONTRACT.md` (clock/events) and `DESIGN.md` (shell parity).
...
BATTLE exploit agents should become real animated side-view sprites once Phase
1 viewport/receipt gates pass. The target style is original 16-bit grimdark
sci-fi pixel art with a vintage Sega Genesis-like feel, transparent PNG
spritesheets, and nearest-neighbor scaling.

Backend fixture data may provide `actor_visual.variant_id`,
`actor_visual.archetype`, `actor_visual.style_family`, and receipt-backed
semantic state transitions. Pixi maps those values through UI-owned manifests:
...
Backend must not emit spritesheet paths, Pixi aliases, animation names, or
frame indices. Terminal states stay receipt-gated.

Required initial character states:

```text
idle, run, research, payload, spawn, blocked, killed
```

Full production states:
...
Shared effect sheets are preferred for spawn bursts, shield impacts, destroy
bursts, useful-signal pings, handoff forks, promotion glows, and rocket streaks.
...
Layer 1 — Official PixiJS (generic v8 API)
  agent-skills/skills/pixijs  →  experiments/pixijs-skills/skills/pixijs
  Nested API subskills live under agent-skills/skills/pixijs/skills/pixijs-*
