...
def opaque_command_is_reviewed(task: dict[str, Any]) -> bool:
    return (
        bool(task.get("opaque_command_reviewed", False))
        and str(task.get("dod_scope") or "").strip() == "worktree_local"
        and not bool(task.get("requires_network", False))
        and not bool(task.get("requires_live_server", False))
        and not bool(task.get("browser_required", False))
    )
...
def audit_code_runner_routing(data: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    for task in tasks:
        ...
        if dod_uses_opaque_command(code_runner_live_surface(task, dod_command)) and not opaque_command_is_reviewed(task):
            issues.append(
