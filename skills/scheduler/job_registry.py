...
def load_jobs() -> dict[str, Any]:
    ...
    if not content.strip():
        logger.warning(f"jobs.json is empty, attempting backup recovery")
        return _recover_from_backup()

    try:
        return json.loads(content)
    except json.JSONDecodeError as e:
        logger.error(f"jobs.json corrupt: {e}, attempting backup recovery")
        return _recover_from_backup()


def _recover_from_backup() -> dict[str, Any]:
    """Try to recover from backup files when jobs.json is corrupt/empty."""
    for suffix in (".bak", ".bak.2", ".bak-recovered", ".bak-20260217"):
        backup = JOBS_FILE.parent / f"jobs.json{suffix}"
        if not backup.exists():
            continue
        try:
            content = backup.read_text()
            decoder = json.JSONDecoder()
            jobs, _ = decoder.raw_decode(content)
            logger.info(f"Recovered {len(jobs)} jobs from {backup.name}")
            save_jobs(jobs)
            return jobs
...
def _rotate_backup() -> None:
    """Rotate jobs.json → jobs.json.bak (keep last 2 backups).

    Best-effort — never raises. Called before each save so there's
    always a recent known-good copy to recover from.
    """
    try:
        if not JOBS_FILE.exists() or JOBS_FILE.stat().st_size == 0:
            return
        bak = JOBS_FILE.parent / "jobs.json.bak"
        bak2 = JOBS_FILE.parent / "jobs.json.bak.2"
        if bak.exists():
            # Rotate .bak → .bak.2 (overwrite oldest)
            try:
                os.replace(str(bak), str(bak2))
            except OSError:
                pass
        # Copy current → .bak (copy, don't move — we still need the original)
        import shutil
...
def save_jobs(jobs: dict[str, Any]) -> None:
    """
    Save jobs to JSON file using atomic write (temp + rename).

    Rotates a backup before writing so recovery is always possible.
    ...
    tmp_fd, tmp_path = tempfile.mkstemp(
        dir=JOBS_FILE.parent, suffix=".tmp", prefix="jobs_"
    )
    try:
        ...
        # Clean up temp file on failure — jobs.json is untouched
        try:
            os.unlink(tmp_path)
        except OSError:
            pass
