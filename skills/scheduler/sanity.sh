...
# 9. Test CLI help
# Note: capture to variable first to avoid pipefail+SIGPIPE with grep -q
HELP_OUTPUT=$("$SCRIPT_DIR/run.sh" --help 2>&1 || true)
if echo "$HELP_OUTPUT" | grep -q "Background task scheduler"; then
...
# Note: capture to variable first to avoid pipefail+SIGPIPE with grep -q
STATUS_OUTPUT=$("$SCRIPT_DIR/run.sh" status --json 2>&1 || true)
