#!/usr/bin/env python3
"""Create Python-first VS Code debug harness artifacts.

This script intentionally prepares debug artifacts only. The VS Code extension
runs the debugger and records runtime observations.
"""
...
def init_python(
    ...
    target_arg: Annotated[list[str], typer.Option(help="Argument passed to the target script. Repeat for multiple args.")] = [],
    breakpoint: Annotated[list[str], typer.Option(help="Breakpoint as file:line or file:line:expr,expr. Repeatable.")] = [],
    ...
    launch_config_name = launch_name or f"Human Debug: {task_slug}"

    argv_literal = repr([target_rel, *target_arg])
    harness = f'''#!/usr/bin/env python3
"""Generated by human-debug-harness. Do not edit production code for debugging."""
...
'''
    ...
    breakpoint_specs = [_parse_breakpoint(item) for item in breakpoint]
    breakpoints = [
        {
            "file": spec.file,
            "line": spec.line,
    ...
    _write_json(
        state_path,
        {
            "schema": "human_debug_session_state.v1",
            "status": "not_started",
    ...
    manifest = {
        "schema": "human_debug_manifest.v1",
        "version": 1,
        "language": "python",
        "task": task_slug,
    ...
    notes_path.write_text(
        ...
        f"Target: `{target_rel} {shlex.join(target_arg)}`\n\n"
        "Use the VS Code command `Human Debug: Run Current Manifest` after loading this manifest.\n",
        encoding="utf-8",
    )
