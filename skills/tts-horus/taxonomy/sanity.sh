#!/usr/bin/env bash
# Sanity check for taxonomy skill
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

echo "=== Sanity check for taxonomy ==="
...
if [[ ! -f "$SCRIPT_DIR/taxonomy.py" ]]; then
    echo "FAIL: taxonomy.py not found"
    exit 1
fi
echo "PASS: taxonomy.py exists"
...
if python3 "$SCRIPT_DIR/taxonomy.py" --help >/dev/null 2>&1; then
