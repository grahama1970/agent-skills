...
def create_app(db_path: Path = DEFAULT_DB_PATH) -> FastAPI:
    ...
    def claim(request: ClaimRequest) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(status_code=409, detail=str(exc)) from exc
        return {"schema": "embry.listener_event_claim.v1", "events": events}

    @app.post("/v1/listener/events/ack")
    def ack(request: AckRequest) -> dict[str, Any]:
        try:
            event = ack_event(db_path, request.consumer_name, request.event_id)
        except ValueError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc
        return {"schema": "embry.listener_event_ack.v1", "event": event}

    @app.get("/v1/listener/consumers/{consumer_name}/offset/{session_id}")
