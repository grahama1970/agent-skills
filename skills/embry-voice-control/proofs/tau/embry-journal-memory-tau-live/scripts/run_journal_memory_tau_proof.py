...
def main() -> int:
    ...
    with httpx.Client(base_url=args.journal_url, timeout=10.0) as journal:
        ...
        with httpx.Client(base_url=args.memory_url, timeout=20.0) as memory:
            ...
            intent_request = {"q": turn_text, "scope": "embry", "session_id": source["session_id"], "fast": True, "speaker_id": "horus_lupercal", "speaker_resolution": speaker_response, "listener_evidence": {"source": "physical_hot_mic", "source_event_id": source["event_id"], "source_event_sequence": source["sequence"], "source_event_receipt_hash": source["receipt_hash"], "physical_speaker_profile_hash": enrollment["profile_sha256"]}}
            ...
            answer_request = {"q": turn_text, "scope": "embry", "session_id": source["session_id"], "fast": True, "speaker_id": "horus_lupercal", "speaker_resolution": speaker_response}
            answer_response = post(memory, "/answer", answer_request)
            answer_call = {"schema": "embry.memory.answer_call_receipt.v1", "request": answer_request, "request_sha256": event_hash(answer_request), "response": answer_response, "response_sha256": event_hash(answer_response), "source_event": {key: source[key] for key in ("event_id", "sequence", "session_id", "turn_id", "type")}}
            answer_path = out / "memory-answer-receipt.json"
            write_json(answer_path, answer_call)
            answer_event = append_derived(journal, source, "memory.answer_resolved", intent_event["event_id"], {"response_path": str(answer_path), "response_sha256": sha256_path(answer_path)})
        receipts = {"source_event_claim": {"path": str(claim_path), "sha256": sha256_path(claim_path)}, "speaker_resolution": {"path": str(speaker_path), "sha256": sha256_path(speaker_path)}, "memory_intent": {"path": str(intent_path), "sha256": sha256_path(intent_path)}, "memory_answer": {"path": str(answer_path), "sha256": sha256_path(answer_path)}}
        ...
        if (turn_plan.get("session_id"), turn_plan.get("turn_id"), turn_plan.get("source_event_id")) != (source["session_id"], source["turn_id"], source["event_id"]):
            raise ValueError("tau_turn_plan_lineage_mismatch")
        if turn_plan.get("tts_render_text_sha256") != hashlib.sha256(str(turn_plan.get("tts_render_text", "")).encode()).hexdigest():
            raise ValueError("tau_turn_plan_text_hash_mismatch")
        plan_event = append_derived(journal, source, "tau.turn_plan.completed", answer_event["event_id"], {"turn_plan_path": str(plan_path), "turn_plan_sha256": plan_locator["sha256"], "tts_render_text_sha256": turn_plan["tts_render_text_sha256"]})
        tau_event = append_derived(journal, source, "tau.persistent_tick.completed", plan_event["event_id"], {"dag_receipt_sha256": sha256_path(dag_receipt_path), "agent_handoff_sha256": event_hash(handoff), "tick_receipt_sha256": tick["receipt_sha256"]})
    ...
    receipt = {"schema": "embry.voice.journal_memory_tau_proof_receipt.v1", "status": "PASS" if duplicate_blocked else "FAIL", "ok": duplicate_blocked, "live": True, "mocked": False, "lineage": lineage, "journal_events": {"listener_final": source, "memory_speaker_resolution": speaker_event, "memory_intent": intent_event, "memory_answer": answer_event, "tau_turn_plan": plan_event, "tau_tick_completed": tau_event}, "memory": {"speaker_resolution": speaker_response, "speaker_resolution_path": str(speaker_path), "speaker_resolution_sha256": sha256_path(speaker_path), "intent_response_path": str(intent_path), "intent_response_sha256": sha256_path(intent_path), "answer_response_path": str(answer_path), "answer_response_sha256": sha256_path(answer_path)}, "tau": {"goal_hash": goal_hash, "dag_contract_path": str(dag_path), "dag_contract_sha256": sha256_path(dag_path), "dag_receipt_path": str(dag_receipt_path), "dag_receipt_sha256": sha256_path(dag_receipt_path), "agent_handoff_sha256": event_hash(handoff), "turn_plan_path": str(plan_path), "turn_plan_sha256": plan_locator["sha256"], "turn_plan": turn_plan, "tick_count": 1}, "ack": {"consumer_name": args.consumer_name, "event_id": source["event_id"], "acked": True}, "duplicate_probe": {"claim_rejected_as_already_acked": duplicate_blocked, "memory_call_count": 0, "tau_tick_count": 0}, "forbidden_activity": {"global_latest_json_reads": [], "chatterbox_calls": [], "browser_calls": [], "typed_transcript_used": False, "source_wav_used": False}, "failed_gates": [] if duplicate_blocked else ["duplicate_source_event_execution"]}
