...
def main() -> int:
    ...
    intent_call = json.loads(Path(receipts["memory_intent"]["path"]).read_text(encoding="utf-8"))
    answer_call = json.loads(Path(receipts["memory_answer"]["path"]).read_text(encoding="utf-8"))
    source_claim = json.loads(Path(receipts["source_event_claim"]["path"]).read_text(encoding="utf-8"))
    turn_text = str(source_claim["source_event"]["payload"]["text"])
    if intent_call["request"]["q"] != turn_text or answer_call["request"]["q"] != turn_text:
        raise ValueError("memory_query_source_event_mismatch")
    ...
    tick_receipt = {
        ...
        "memory_answer_receipt": receipts["memory_answer"],
        "tau_turn_plan_receipt": {"path": str(plan_path), "sha256": sha256_path(plan_path)},
    ...
    evidence = [
        ...
        {"kind": "embry.memory.answer_call_receipt.v1", **receipts["memory_answer"]},
        {"kind": "tau.turn_plan.v1", "path": str(plan_path), "sha256": sha256_path(plan_path)},
    ...
    handoff = {
        ...
        "context": {"summary": "One physical listener event was Memory-routed and consumed by one bounded Embry Tau tick.", "source_event": source, "journal": lineage["journal"], "memory": {"speaker_resolution_receipt": receipts["speaker_resolution"], "intent_receipt": receipts["memory_intent"], "answer_receipt": receipts["memory_answer"]}, "tau_turn_plan": {"schema": "tau.turn_plan.v1", "path": str(plan_path), "sha256": sha256_path(plan_path), "plan_id": turn_plan["plan_id"], "session_id": source["session_id"], "turn_id": source["turn_id"], "source_event_id": source["event_id"]}, "persistent_subagent": persistent, "tau_tick": {"tick_index": 1, "tick_budget": 1, "receipt_path": str(tick_path), "receipt_sha256": sha256_path(tick_path)}, "artifacts": [item["path"] for item in evidence]},
