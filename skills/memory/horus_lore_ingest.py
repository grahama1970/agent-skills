...
def ingest_youtube_transcript(
    ...
    # Extract taxonomy (Federated Taxonomy Bridges)
    bridge_attributes = []
    try:
        tax_res = extract_taxonomy_features(
            ...
            artist=channel,
            description=full_text[:2000],  # Use first 2k chars for extraction
            tags=[]
        )
        bridge_attributes = tax_res.get("bridge_attributes", [])
    except Exception as e:
        print(f"Warning: taxonomy extraction failed for video {video_id}: {e}", file=sys.stderr)
    ...
    doc = {
        ...
        "tags": bridge_attributes, # Seed tags with bridges for searchability
...
def ingest_audiobook(
    ...
    try:
        tax_res = extract_taxonomy_features(
            ...
            title=book_title,
            author=series or "",
            description=full_text[:5000], # Books need more context usually
            tags=[]
        )
        bridge_attributes = tax_res.get("bridge_attributes", [])
    except Exception as e:
        print(f"Warning: taxonomy extraction failed for book {book_title}: {e}", file=sys.stderr)
