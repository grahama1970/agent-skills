...
def validate_contract_shape(contract: dict[str, Any]) -> list[str]:
    ...
    panel_payloads = contract.get("panel_payloads") if isinstance(contract.get("panel_payloads"), list) else []
    if not panel_payloads:
        blockers.append("BLOCKED_PHASE10_PANEL_PAYLOADS_MISSING")
    for panel_payload in panel_payloads:
        if not isinstance(panel_payload, dict):
            blockers.append("BLOCKED_PHASE10_PANEL_PAYLOAD_SCHEMA")
            break
        for field in (
            "panel_id",
            "source_evidence",
            ...
            if field not in panel_payload:
                blockers.append("BLOCKED_PHASE10_PANEL_PAYLOAD_SCHEMA")
                break
        projection = panel_payload.get("provider_payload_projection")
        projection_is_object = isinstance(projection, dict)
        projection_input = projection.get("input") if projection_is_object and isinstance(projection.get("input"), dict) else {}
        if not projection_is_object or projection.get("submitted") is not False:
            blockers.append("BLOCKED_PHASE10_PANEL_PAYLOAD_SUBMITTED")
        if not projection_input.get("start_frame_asset_id") or not projection_input.get("end_frame_asset_id"):
            blockers.append("BLOCKED_PHASE10_PANEL_FRAME_MAPPING_MISSING")
        if not projection_input.get("prompt"):
            blockers.append("BLOCKED_PHASE10_PANEL_PROMPT_MISSING")
        evidence = panel_payload.get("source_evidence") if isinstance(panel_payload.get("source_evidence"), dict) else {}
        if not evidence.get("action") or not evidence.get("camera"):
            blockers.append("BLOCKED_PHASE10_PANEL_SOURCE_EVIDENCE_MISSING")
