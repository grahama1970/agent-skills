...
def _identity_review_prompt(panel: Mapping[str, Any], *, frame_key: str, required_entities: list[str]) -> str:
    return "\n".join(
        [
            ...
            "",
            "SPECIFIC-IDENTITY STANDARD (read carefully):",
            "This is a SPECIFIC-IDENTITY check, not a demographic-category check. A frame passes",
            "ONLY if the depicted person is recognizably the SAME INDIVIDUAL as the attached",
            "reference sheet -- same face. It is NOT enough that the demographic category matches",
            "(e.g. 'an adult woman with brown hair in a navy top', or 'a tan young man with dark",
            "curly hair'). Two different people can share a category. You must verify the specific",
            "face: face shape, apparent age, eyebrow shape, nose shape, jawline, and any distinctive",
            "features (freckles, cheekbones, lip shape) against the reference. If the person only",
            "matches the category but is visibly a DIFFERENT individual than the reference, that is a",
            "FAIL, not a pass.",
            "",
            "AGE CONSISTENCY: FAIL if the depicted person appears a clearly different age than the",
            "reference identity (roughly a decade or more older or younger). The Embry reference is a",
            "specific late-20s woman; an older/weathered ~40s-50s woman is a DIFFERENT identity even",
            "if hair colour and clothing match.",
            "",
            "FEATURE GROUNDING (mandatory): For each required identity you PASS, you must be able to",
            "name 2-3 concrete facial features you actually verified match the reference (e.g. 'same",
            "rounded nose tip, same arched eyebrows, same narrow jaw'). Record these in",
            "'verified_facial_features'. If you cannot name specific matching correspondences and are",
            "relying on vibes, category, hair, or clothing, you must FAIL that identity -- do not pass",
            "on generic resemblance.",
            "",
            "SKIN TONE / COMPLEXION IS AN IDENTITY SIGNAL: compare complexion (warm-olive vs cool-pale),",
            "undertone, tan, and freckling/skin texture against the reference. A visibly different",
            "complexion or freckling pattern (e.g. reference is warm olive and freckled but the frame",
            "shows a cooler, paler, un-freckled face) indicates a DIFFERENT specific individual even if",
            "hair colour, clothing, and demographic category match. Weigh it as a disqualifier.",
            "",
            "ADVERSARIAL STANCE (do this FIRST): before deciding PASS, actively hunt for at least one",
            "DIVERGENCE between the frame's face and the reference -- face width/length, complexion and",
            "freckling, apparent age, eyebrow and nose shape. Record every divergence you observe in",
            "'observed_divergences'. Do NOT confirm a match by listing features that could describe any",
            "person in the category; only PASS if, after honestly hunting for divergences, none of them",
            "are identity-changing. If you find yourself asserting matching features you did not actually",
            "trace to the reference face, treat that as a FAIL.",
            "",
            "DO NOT EXCUSE STRUCTURAL DIVERGENCE AS LIGHTING/WETNESS: wet hair, sea spray, harsh side",
            "light, and expression change surface appearance but NEVER change bone structure, face",
            "width-to-length ratio, base skin undertone, or the freckling map. If the frame's face reads",
            "as a COOLER or PALER undertone, has LESS freckling, or is NARROWER/LONGER than the reference",
            "(the Embry reference is a warm-olive, freckled, fuller-oval-faced late-20s woman), that is a",
            "structural divergence -- you may NOT dismiss it as lighting or wet hair. Affirmatively",
            "confirm complexion warmth and face proportion match; if they read even somewhat cooler,",
            "paler, or narrower than the reference, FAIL with 'IDENTITY_MISMATCH_DESPITE_GENERIC_MATCH'.",
            "",
            "FAIL-CLOSED ON UNCERTAINTY: identity must be affirmatively proven, not assumed. If your",
            "confidence that this is the SAME specific individual (not merely a similar-looking person of",
            "the same type) is anything short of high after the divergence hunt, return FAIL. A plausible",
            "look-alike under surf conditions is a FAIL, not a PASS.",
            "",
            "Pass only if all of the following are true:",
            "1. Embry is visible in the foreground.",
            "2. Embry is the SAME SPECIFIC INDIVIDUAL as the attached Embry reference sheet at the",
            "   feature level (face shape, apparent age, eyebrows, nose, distinctive features), not",
            "   merely the same demographic category.",
            ...
            "5. Kai is the SAME SPECIFIC INDIVIDUAL as the attached Kai reference sheet at the feature",
            "   level (face shape, apparent age, eyebrows, nose, distinctive features), not merely the",
            ...
            "7. Both faces are visible enough to verify SPECIFIC identity at the feature level.",
            "8. Embry and Kai are not replaced by generic surfers OR by different individuals who merely fit the category.",
            "9. No other foreground person competes with or confuses the identities.",
            "10. You can enumerate 2-3 concrete matching facial features for each passed identity.",
            "",
            ...
            "- A required identity matches the demographic CATEGORY but is a visibly DIFFERENT specific",
            "  individual than the reference (wrong face shape, wrong distinctive features). In this case",
            "  add the failure code 'IDENTITY_MISMATCH_DESPITE_GENERIC_MATCH' to blocking_findings and",
            "  name which identity and which features diverge.",
            "- A required identity appears a clearly different age (a decade or more) than the reference.",
            "  Add the failure code 'IDENTITY_AGE_MISMATCH' to blocking_findings.",
            "- You cannot name 2-3 specific matching facial features for a required identity. Add the",
            "  failure code 'IDENTITY_FEATURES_NOT_GROUNDED' to blocking_findings.",
            "",
            "Return strict JSON with exactly these keys:",
            '{"verdict":"PASS|FAIL","visible_entities":["Embry"],"blocking_findings":["..."],'
            '"verified_facial_features":{"Embry":["..."],"Kai":["..."]},'
            '"observed_divergences":{"Embry":["..."],"Kai":["..."]},"identity_notes":"..."}',
            ...
            "When uncertain whether it is the same specific individual, FAIL -- specific identity must be",
            "affirmatively verified, never assumed.",
        ]
    )
