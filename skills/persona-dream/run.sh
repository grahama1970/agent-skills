...
Commands:
  ...
  agent-listening-signoff Agent prescreen gate before human listener collection
...
case "$COMMAND" in
  ...
  validate-blinded-listener-study)
    ...
    ;;
  agent-listening-signoff)
    exec "${PYTHON[@]}" "${SCRIPT_DIR}/scripts/agent_listening_signoff.py" "$@"
    ;;
