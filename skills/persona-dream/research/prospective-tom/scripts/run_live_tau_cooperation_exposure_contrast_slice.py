#!/usr/bin/env python3
"""Run the deterministic cooperation exposure/contrast instrument through live Tau."""
...
EXPOSURE_CONTRAST_SCRIPT = RESEARCH_ROOT / "scripts" / "check_cooperation_exposure_contrast_instrument.py"
...
PASS_STATUS = "PASS_LIVE_TAU_PCTOM_COOPERATION_EXPOSURE_CONTRAST_SLICE"
BLOCKED_STATUS = "BLOCKED_LIVE_TAU_PCTOM_COOPERATION_EXPOSURE_CONTRAST_SLICE"
...
def _run_or_load_exposure_contrast(
    *,
    exposure_contrast: Any,
    output_root: Path,
    receipt_path: Path,
    reuse_exposure_contrast_receipt: Path | None,
    prefix: str,
    variant_start: int,
    pair_count: int,
    errors: list[str],
) -> dict[str, Any]:
    if reuse_exposure_contrast_receipt is not None:
        receipt = _load_json(reuse_exposure_contrast_receipt.resolve(), errors, "reuse_exposure_contrast_receipt")
        return receipt if isinstance(receipt, dict) else {}
    try:
        return exposure_contrast.run_check(
        ...
        errors.append(f"exposure_contrast_instrument_exception:{type(exc).__name__}:{exc}")
        receipt = _load_json(receipt_path, errors, "exposure_contrast_receipt_after_exception")
        return receipt if isinstance(receipt, dict) else {}
...
def _select_exposure_contrast_conclusion(
    cd_offer_candidates: int,
    keep_offer_candidates: int,
    avoid_offer_candidates: int,
) -> str:
    if avoid_offer_candidates > 0 and keep_offer_candidates > 0:
        return "EXPOSURE_CONTRAST_SLICE_HAS_KEEP_AND_AVOID_CD_OFFER_EXPOSURE"
    if cd_offer_candidates > 0:
        return "EXPOSURE_CONTRAST_SLICE_PARTIAL_CD_OFFER_EXPOSURE"
    return "EXPOSURE_CONTRAST_SLICE_LIVE_TAU_NO_CD_OFFER_EXPOSURE"
...
def run_slice(
    *,
    ...
    bootstrap_seed: int,
    alpha: float,
    reuse_exposure_contrast_receipt: Path | None = None,
    ...
    exposure_contrast_root = output_root / "cooperation_exposure_contrast_instrument"
    ...
    rows_path = artifacts_root / "cooperation_exposure_contrast_slice_rows.json"
    summary_path = artifacts_root / "cooperation_exposure_contrast_slice_summary.json"
    errors: list[str] = []

    exposure_contrast = _load_module(EXPOSURE_CONTRAST_SCRIPT, "pctom_cooperation_exposure_contrast_instrument")
    ...
    exposure_contrast_receipt_path = exposure_contrast_root / "cooperation_exposure_contrast_instrument_receipt.v1.json"
    exposure_contrast_receipt = _run_or_load_exposure_contrast(
        exposure_contrast=exposure_contrast,
        output_root=exposure_contrast_root,
        receipt_path=exposure_contrast_receipt_path,
        reuse_exposure_contrast_receipt=reuse_exposure_contrast_receipt,
        prefix=prefix,
        variant_start=variant_start,
        pair_count=pair_count,
        errors=errors,
    )
    if exposure_contrast_receipt.get("status") != exposure_contrast.PASS_STATUS:
        errors.append(f"exposure_contrast_status_not_pass:{exposure_contrast_receipt.get('status')}")
        errors.extend(f"exposure_contrast_error:{error}" for error in exposure_contrast_receipt.get("errors", []))
    corpus_path_value = exposure_contrast_receipt.get("corpus_path")
    corpus_path = (
        Path(str(corpus_path_value))
        if isinstance(corpus_path_value, str)
        else exposure_contrast_root / "artifacts" / "cooperation_exposure_contrast_instrument_corpus.v1.json"
    )
    ...
    contrast_counts = exposure_contrast_receipt.get("counts") if isinstance(exposure_contrast_receipt.get("counts"), dict) else {}
    ...
    offer_affordance_rows = contrast_counts.get("offer_cooperation_affordance_rows")
    contrast_by_episode = {
    ...
    conclusion = _select_exposure_contrast_conclusion(
        cd_offer_candidates,
        keep_offer_candidates,
        avoid_offer_candidates,
    )

    _write_json(
        rows_path,
        {
            "schema": "persona_dream.research.prospective_tom.cooperation_exposure_contrast_slice_rows.v1",
            "policy_id": POLICY_ID,
    ...
    _write_json(
        summary_path,
        {
            "schema": "persona_dream.research.prospective_tom.cooperation_exposure_contrast_slice_summary.v1",
            "policy_id": POLICY_ID,
            "corpus_path": str(corpus_path),
            "variant_start": variant_start,
            "pair_count": pair_count,
    ...
    _validate_zero_writes(exposure_contrast_receipt, errors, "contrast")
    ...
    checks = {
        ...
        "exposure_contrast_receipt_passed": exposure_contrast_receipt.get("status") == exposure_contrast.PASS_STATUS,
        "contrast_keep_rows_present": keep_rows == pair_count,
        "contrast_avoid_rows_present": avoid_rows == pair_count,
        "all_rows_expose_offer_cooperation_affordance": offer_affordance_rows == expected_episode_count,
        ...
        "zero_unsupported_writes": all(
            receipt.get(key) == 0
            for receipt in (derivation_receipt, exposure_contrast_receipt, condition_receipt, action_receipt)
            for key in ZERO_WRITE_KEYS
        ),
    }
    ...
    exposure_contrast_receipt_path_for_hash = (
        exposure_contrast_receipt_path if reuse_exposure_contrast_receipt is None else reuse_exposure_contrast_receipt.resolve()
    )
    receipt = {
        "schema": "persona_dream.research.prospective_tom.live_tau_cooperation_exposure_contrast_slice_receipt.v1",
        "created_at": _now_iso(),
        "status": status,
        ...
        "exposure_contrast_receipt": str(exposure_contrast_receipt_path_for_hash),
        "exposure_contrast_receipt_sha256": _file_sha256(exposure_contrast_receipt_path_for_hash)
        if exposure_contrast_receipt_path_for_hash.exists()
        else None,
        ...
        "checks": checks,
        "counts": {
            "exposure_contrast_episodes": expected_episode_count,
            "keep_cooperation_positive_rows": keep_rows,
            "avoid_or_unsafe_cooperation_contrast_rows": avoid_rows,
            "offer_cooperation_affordance_rows": offer_affordance_rows,
        ...
        "claims": {
            "proves": [
                "the deterministic cooperation exposure/contrast corpus was consumed by the live Tau M/R/D/CD condition runner",
                "all visible cases exposed OFFER_COOPERATION before deterministic outcome reveal and then passed through Gate 6 action scoring",
            ...
            if status == PASS_STATUS
                ...
                "the cooperation exposure/contrast live Tau slice failed closed before accepting a planning-benefit claim",
            ],
            "does_not_prove": [
...
def main() -> int:
    ...
    parser.add_argument("--variant-start", type=int, default=37)
    ...
    parser.add_argument("--bootstrap-seed", type=int, default=20721037)
    parser.add_argument("--alpha", type=float, default=0.05)
    parser.add_argument("--reuse-exposure-contrast-receipt", type=Path, default=None)
    ...
    receipt_out = args.receipt_out or (args.output_root / "live_tau_cooperation_exposure_contrast_slice_receipt.v1.json")
    receipt = run_slice(
        ...
        alpha=args.alpha,
        reuse_exposure_contrast_receipt=args.reuse_exposure_contrast_receipt,
