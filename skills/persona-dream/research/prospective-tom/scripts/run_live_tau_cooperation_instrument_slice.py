#!/usr/bin/env python3
"""Run the deterministic cooperation-exposure instrument through live Tau."""
...
INSTRUMENT_SCRIPT = RESEARCH_ROOT / "scripts" / "check_cooperation_exposure_instrument.py"
...
PASS_STATUS = "PASS_LIVE_TAU_PCTOM_COOPERATION_INSTRUMENT_SLICE"
BLOCKED_STATUS = "BLOCKED_LIVE_TAU_PCTOM_COOPERATION_INSTRUMENT_SLICE"
...
def run_slice(
    *,
    ...
    instrument_root = output_root / "cooperation_exposure_instrument"
    ...
    rows_path = artifacts_root / "cooperation_instrument_slice_rows.json"
    summary_path = artifacts_root / "cooperation_instrument_slice_summary.json"
    errors: list[str] = []

    instrument = _load_module(INSTRUMENT_SCRIPT, "pctom_cooperation_exposure_instrument")
    ...
    instrument_receipt_path = instrument_root / "cooperation_exposure_instrument_receipt.v1.json"
    ...
    corpus_path = Path(str(corpus_path_value)) if isinstance(corpus_path_value, str) else instrument_root / "artifacts" / "cooperation_exposure_instrument_corpus.v1.json"
    ...
    instrument_exposure_rows = instrument_counts.get("exposure_rows")
    cd_offer_candidates = int(summary.get("cd_offer_cooperation_candidates") or 0)
    changed_count = int(summary.get("cd_action_change_count") or 0)
    if cd_offer_candidates > 0:
        conclusion = "INSTRUMENT_COOPERATION_EXPOSURE_SCORED"
    elif instrument_exposure_rows == episode_count:
        conclusion = "INSTRUMENT_OUTCOME_EXPOSURE_PRESENT_BUT_CD_NO_OFFER_EXPOSURE"
    else:
        conclusion = "INSTRUMENT_COOPERATION_EXPOSURE_SLICE_FAILED_CLOSED"

    _write_json(
        rows_path,
        {
            "schema": "persona_dream.research.prospective_tom.cooperation_instrument_slice_rows.v1",
            "policy_id": POLICY_ID,
    ...
    _write_json(
        summary_path,
        {
            "schema": "persona_dream.research.prospective_tom.cooperation_instrument_slice_summary.v1",
            "policy_id": POLICY_ID,
            "corpus_path": str(corpus_path),
            "variant_start": variant_start,
            "episode_count": episode_count,
            **summary,
            "instrument_cooperation_exposure_rows": instrument_exposure_rows,
    ...
    checks = {
        ...
        "instrument_exposure_rows_complete": instrument_exposure_rows == episode_count,
        "condition_receipt_passed": condition_receipt.get("status") == condition.PASS_STATUS,
        "condition_used_external_instrument_corpus": condition_receipt.get("external_corpus_used") is True
    ...
    receipt = {
        "schema": "persona_dream.research.prospective_tom.live_tau_cooperation_instrument_slice_receipt.v1",
        "created_at": _now_iso(),
        "status": status,
        ...
        "instrument_receipt": str(instrument_receipt_path if reuse_instrument_receipt is None else reuse_instrument_receipt.resolve()),
        "instrument_receipt_sha256": _file_sha256(instrument_receipt_path if reuse_instrument_receipt is None else reuse_instrument_receipt.resolve())
        if (instrument_receipt_path if reuse_instrument_receipt is None else reuse_instrument_receipt.resolve()).exists()
        else None,
        ...
        "checks": checks,
        "counts": {
            "instrument_episodes": episode_count,
            ...
            "cd_action_change_count": changed_count,
        },
        "metrics": {
        ...
        "claims": {
            "proves": [
                "the deterministic cooperation-exposure instrument corpus was consumed by the live Tau M/R/D/CD condition runner",
                "instrument cases were sealed before deterministic outcome reveal and then passed through Gate 6 action scoring",
            ...
            if status == PASS_STATUS
                ...
                "the cooperation instrument slice failed closed before accepting a planning-benefit claim",
            ],
            "does_not_prove": [
...
def main() -> int:
    ...
    parser.add_argument("--variant-start", type=int, default=25)
    ...
    parser.add_argument("--bootstrap-seed", type=int, default=20721025)
    ...
    receipt_out = args.receipt_out or (args.output_root / "live_tau_cooperation_instrument_slice_receipt.v1.json")
    receipt = run_slice(
