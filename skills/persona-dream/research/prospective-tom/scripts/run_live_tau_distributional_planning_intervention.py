#!/usr/bin/env python3
"""Run a distributional expected-utility planning intervention over full64 live Tau artifacts."""
...
TRUST_COMMIT_FAMILY_VALUES = {"trust_commitment_relationship", "trust-commit"}
PASS_STATUS = "PASS_LIVE_TAU_PCTOM_DISTRIBUTIONAL_PLANNING_INTERVENTION"
BLOCKED_STATUS = "BLOCKED_LIVE_TAU_PCTOM_DISTRIBUTIONAL_PLANNING_INTERVENTION"
POLICY_ID = "distributional_expected_utility_over_predicted_next_action.v1"
...
def _utility(action: str, oracle_action: str) -> float:
    components = _action_components(action, oracle_action)
    return components["expected_task_reward"] + components["expected_information_gain"] - components["expected_social_cost"]
...
def _with_utility(action: str, oracle_action: str) -> dict[str, Any]:
    ...
    option["expected_utility"] = _utility(action, oracle_action)
    return option
...
def _validate_base(base_receipt: dict[str, Any], errors: list[str]) -> None:
    ...
    for key, value in expected_counts.items():
        if counts.get(key) != value:
            errors.append(f"base_counts_{key}_mismatch:{counts.get(key)}:{value}")
    for count_key in (
        ...
        for condition in CONDITIONS:
            if values.get(condition) != 64:
                errors.append(f"base_{count_key}_{condition}_mismatch:{values.get(condition)}:64")
...
def _predicted_distribution(commitment_bundle: dict[str, Any], errors: list[str], label: str) -> list[dict[str, Any]]:
    ...
    for idx, item in enumerate(distribution):
        ...
        normalized.append({"value": value, "probability": float(probability)})
    if abs(total - 1.0) > 1e-6:
        errors.append(f"{label}_predicted_action_probability_sum_mismatch:{total}")
    return normalized


def _expected_utility_scores(distribution: list[dict[str, Any]]) -> dict[str, float]:
    scores = {action: 0.0 for action in ACTION_VOCABULARY}
    for item in distribution:
        hypothesized_oracle = _counterpart_to_agent_action(str(item["value"]))
        probability = float(item["probability"])
        for action in ACTION_VOCABULARY:
            scores[action] += probability * _utility(action, hypothesized_oracle)
    return scores


def _select_distributional_action(expected_scores: dict[str, float]) -> str:
    return max(ACTION_VOCABULARY, key=lambda action: (expected_scores[action], -ACTION_VOCABULARY.index(action)))
...
def _build_action_selection(
    *,
    ...
    distribution = _predicted_distribution(commitment_bundle, errors, f"{outcome.get('episode_id')}_{condition}")
    expected_scores = _expected_utility_scores(distribution)
    selected_action = _select_distributional_action(expected_scores) if distribution else "ABSTAIN"
    ...
    return {
        ...
        "decision_basis": {
            ...
            "expected_utility_scores_by_agent_action": expected_scores,
...
def _summarize(
    *,
    ...
    original_rows: dict[tuple[str, str], dict[str, Any]],
    ...
    changed_cd_by_family: collections.Counter[str] = collections.Counter()
    ...
    for row in valid_rows:
        ...
        if changed:
            ...
            if row.get("condition") == "CD":
                changed_cd_by_family[str(row.get("family"))] += 1
    ...
    return {
        ...
        "summary": {
            ...
            "cd_action_policy_change_count_by_family": dict(changed_cd_by_family),
            "nonzero_action_policy_change_families": sorted(changed_by_family),
            "changes_beyond_trust_commit_subset": bool(non_trust_changed_families),
            "non_trust_commit_changed_families": non_trust_changed_families,
            "nonzero_planning_delta_families": sorted(
                {str(row.get("family")) for row in planning_rows if row["direction"] != "TIE"}
            ),
        },
    }
...
def run_intervention(
    *,
    ...
    base_receipt = _load_json(base_receipt_path, errors, "base_full64_receipt")
    ...
    original_action_index = _load_json(original_action_index_path, errors, "original_action_index")
    ...
    if intervention_summary["action_policy_change_count"] <= 0:
        errors.append("intervention_action_policy_change_count_zero")
    if intervention_summary["changes_beyond_trust_commit_subset"] is not True:
        errors.append("intervention_did_not_change_beyond_trust_commit_subset")
    ...
    decision_index_path = output_root / "artifacts" / "distributional_action_decisions.json"
    planning_rows_path = output_root / "artifacts" / "distributional_planning_rows.json"
    summary_path = output_root / "artifacts" / "distributional_planning_summary.json"
    _write_json(decision_index_path, {"schema": "pctom.distributional_action_decisions.v1", "rows": action_rows})
    _write_json(planning_rows_path, {"schema": "pctom.distributional_planning_rows.v1", "rows": summary_payload["planning_rows"]})
    _write_json(
        summary_path,
        {
            "schema": "pctom.distributional_planning_intervention_summary.v1",
            "intervention_policy": POLICY_ID,
    ...
    receipt = {
        "schema": "pctom.live_tau_distributional_planning_intervention_receipt.v1",
        "created_at": _now_iso(),
        "status": status,
        ...
        "counts": {
            ...
            "cd_action_policy_change_count_by_family": intervention_summary["cd_action_policy_change_count_by_family"],
        ...
        "metrics": {
            ...
            "nonzero_action_policy_change_families": intervention_summary["nonzero_action_policy_change_families"],
        ...
        "claims": {
            "proves": [
                "a deterministic distributional expected-utility planning intervention consumed full64 live Tau M/R/D/CD artifacts",
                "the intervention changed selected actions beyond the current trust/commitment subset",
                "all intervention decisions were rescored by Gate 6 against deterministic simulator oracle policy",
                "planning benefit was kept separate from belief-prediction benefit and was not claimed when deltas tied",
            ...
            if status == PASS_STATUS
                ...
                "the distributional planning intervention failed closed before claiming broader planning-intervention evidence",
            ],
            "does_not_prove": [
