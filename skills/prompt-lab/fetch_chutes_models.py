...
@app.command()
def main(
    command: str = typer.Argument("update", help="Command: update, list, json, reasoning, agentic"),
):
    """Manage Chutes model catalog."""
    if command == "update":
        update_models()
    elif command == "list":
        list_all_models()
        ...
        typer.echo(f"Unknown command: {command}. Choose from: update, list, json, reasoning, agentic")
        raise typer.Exit(1)
