...
python3 -m py_compile batch_checkpoint.py
python3 -m py_compile task_monitor_client.py
...
# Check module line counts (< 500 except CLI and llm.py)
echo "[7/8] Checking module line counts..."
for module in config.py models.py evaluation.py qra_evaluation.py sparta_connector.py prompt_extractor.py ground_truth.py optimization.py utils.py batch_checkpoint.py task_monitor_client.py; do
    lines=$(wc -l < "$module")
...
# llm.py can be up to 600 (includes retry infrastructure)
llm_lines=$(wc -l < llm.py)
if [ "$llm_lines" -gt 600 ]; then
    echo "  FAIL - llm.py has $llm_lines lines (> 600)"
    exit 1
fi
echo "  llm.py: $llm_lines lines (includes retry infrastructure)"
# CLI can be up to 1800 (includes convergence mode, relationship anchoring, JSON output, batch hardening)
cli_lines=$(wc -l < prompt_lab.py)
if [ "$cli_lines" -gt 1800 ]; then
    echo "  FAIL - prompt_lab.py has $cli_lines lines (> 1800)"
    exit 1
fi
