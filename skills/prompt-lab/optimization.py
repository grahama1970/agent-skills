...
def build_optimization_prompt(
    ...
    Args:
        ...
        error_cases: List of error cases from evaluation results
        failure_cases: List of failure cases from batch monitoring (entity anchor failures, etc.)

    Returns:
        Optimization prompt string
    """
    prompt = f"""Analyze this prompt and suggest targeted improvements based on the error cases below.
...
"""
    if error_cases:
        prompt += "EVALUATION ERROR CASES (showing expected vs predicted):\n"
        for i, case in enumerate(error_cases[:10], 1):
            prompt += f"""
Case {i}: ID={case.get('id', i)}
...
"""
    ...
    if failure_cases:
        prompt += "\nBATCH MONITORING FAILURE CASES (real production failures):\n"
        for i, fc in enumerate(failure_cases[:15], 1):
            prompt += f"""
Failure {i}: {fc.get('entity_check', fc.get('error', 'unknown'))}
  Question: {fc.get('question', 'N/A')}
  Expected: {fc.get('expected', 'N/A')}
"""

    prompt += """

Based on these errors, suggest specific improvements to the prompt. Focus on:
1. Fixing the specific failure patterns shown above
2. Strengthening entity anchoring (if entity check failures are present)
3. Adding examples that address the exact failure modes
4. CRITICAL: Do NOT remove existing instructions - only add or strengthen them
