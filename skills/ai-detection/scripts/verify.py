...
class Report(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)
    schema_version: Literal["ai_detection.mechanism_verification.v1"] = "ai_detection.mechanism_verification.v1"
    run_id: str
    created_at: str
...
def main(output: Path = Path("reports/qualification"), trials: int = 3, samples: int = 128, profile: str = "full"):
    ...
    for number in range(1, trials + 1):
        ...
        env = dict(os.environ, AI_DETECTION_TEST_ARTIFACTS=str(location / "browser"))
    ...
    rendered = "<!doctype html><meta charset='utf-8'><title>ai-detection verification</title>"
    ...
    rendered += f"<h1>ai-detection: {report.mechanism_readiness}</h1><h2>Release and detection efficacy: NOT_ESTABLISHED</h2>"
