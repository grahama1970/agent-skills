...
def real_server(tmp_path, evidence_dir):
    ...
    env = dict(os.environ, AI_DETECTION_HOME=str(tmp_path / "server"),
               AI_DETECTION_POLICY=str(root / "specs/policy.json"))
    env.pop("AI_DETECTION_MODEL", None)
    env.pop("AI_DETECTION_MODEL_SHA256", None)
    with (evidence_dir / f"http-server-{port}.log").open("wb") as output:
        process = subprocess.Popen([sys.executable, "-m", "ai_detection", "serve", "--port", str(port)],
    ...
    with real_server(tmp_path, evidence_dir) as base, sync_playwright() as playwright:
        ...
        try:
            ...
            atomic_json(evidence_dir / "browser-readback.json", {
                "schema_version": "ai_detection.browser_proof.v1", "status": "PASS",
