...
`/ask argue` uses `ask.citations.v1`.

- Advocate `evidence` must include `evidence_citations`.
- Judge `evidence_used` must include `evidence_citations`.
- Citation objects must include `source_id`, `source_kind`,
  `quote_or_summary`, and `supports`.
- `QUESTION` and `MEMORY.N` citations are admissible for knowledge/context
  arguments.
- Memory citations are not admissible for code/review safety claims.
- `FOR` and `AGAINST` verdicts fail verification when structured citations are
  missing or cite sources outside the source bundle.
