...
bundle through `/scillm` `source` so grounding diagnostics and citation checks
can attach to reviewer and judge calls without trusting model-generated IDs.

## Citation Contract

`/ask parallel-review` uses `ask.citations.v1`.

- Reviewer verdict evidence, judge verdict evidence, and each finding must carry
  `evidence_citations`.
- Citation objects must include `source_id`, `source_kind`,
  `quote_or_summary`, and `supports`.
- `SAFE` and `SAFE_WITH_CONDITIONS` require target/file/diff/artifact citations.
- Memory citations may guide context, but are not admissible for code/review
  safety claims.
- Large target bundles are chunked as `TARGET_BUNDLE.1`,
  `TARGET_BUNDLE.2`, and so on; citations must target the relevant source ID.
