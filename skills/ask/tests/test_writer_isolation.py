...
from ask.writer_isolation import (  # noqa: E402
    ...
    diff_capture_plan,
    verify_diff_capture,
    verify_setup_hook_result,
)


def test_a_bare_command_name_is_refused_as_a_setup_hook() -> None:
    """PATH resolution makes what executes depend on the ambient environment."""
    with pytest.raises(SetupHookError, match="bare command name"):
        compile_setup_hook("setup-worktree")


@pytest.mark.parametrize("spec", ["./scripts/setup.mjs", "~/bin/setup.sh", "/opt/setup.sh", "scripts/setup.mjs"])
def test_a_path_shaped_hook_compiles(spec: str) -> None:
    compiled = compile_setup_hook(spec)
    assert compiled["declared"] is True
    assert compiled["timeout_ms"] == 30_000
    assert "baseCommit" in compiled["stdin_fields"]


def test_no_hook_is_not_an_error() -> None:
    assert compile_setup_hook("")["declared"] is False


def test_an_absurd_hook_timeout_is_refused() -> None:
    with pytest.raises(SetupHookError, match="timeout"):
        compile_setup_hook("./setup.sh", timeout_ms=999_999)


def test_synthetic_paths_are_accepted_and_deduped() -> None:
    result = verify_setup_hook_result({"syntheticPaths": [".venv", ".venv", ".env.local"]})
    assert result["synthetic_paths"] == [".env.local", ".venv"]


def test_a_tracked_path_can_never_be_declared_synthetic() -> None:
    """Otherwise a writer hides a real edit from the patch and the reviewer."""
    with pytest.raises(SetupHookError, match="is tracked"):
        verify_setup_hook_result(
            {"syntheticPaths": ["src/app/main.py"]}, tracked_paths=["src/app/main.py"]
        )


@pytest.mark.parametrize("bad", ["/etc/passwd", "~/secrets", "../outside"])
def test_a_synthetic_path_must_stay_inside_the_worktree(bad: str) -> None:
    with pytest.raises(SetupHookError):
        verify_setup_hook_result({"syntheticPaths": [bad]})


def test_non_object_hook_output_is_refused() -> None:
    with pytest.raises(SetupHookError, match="one JSON object"):
        verify_setup_hook_result([".venv"])


def test_a_hook_that_created_nothing_is_fine() -> None:
    assert verify_setup_hook_result({})["synthetic_paths"] == []


def test_the_capture_plan_excludes_scaffolding() -> None:
    contract = compile_isolation({"workstreams": [_writer("w1", ["src/app"])]})
    plan = diff_capture_plan(contract["workstreams"][0], synthetic_paths=[".venv"])
    assert plan["pathspecs"] == [":(exclude).venv"]
    assert plan["base_commit"] == BASE
    assert "patch_digest" in plan["required_outputs"]


def test_a_capture_without_a_retrievable_patch_is_rejected() -> None:
    contract = compile_isolation({"workstreams": [_writer("w1", ["src/app"])]})
    verdict = verify_diff_capture(
        {"patch_digest": "sha256:a", "diff_stat": {"insertions": 1}, "changed_files": ["src/app/x.py"]},
        contract["workstreams"][0],
    )
    assert verdict["accepted"] is False
    assert any("patch_path" in p for p in verdict["problems"])


def test_scaffolding_leaking_into_the_patch_is_rejected() -> None:
    contract = compile_isolation({"workstreams": [_writer("w1", ["src/app"])]})
    verdict = verify_diff_capture(
        {
            "patch_digest": "sha256:a",
            "patch_path": "/tmp/w1.patch",
            "diff_stat": {"insertions": 3, "deletions": 1},
            "changed_files": ["src/app/x.py", ".venv"],
        },
        contract["workstreams"][0],
        synthetic_paths=[".venv"],
    )
    assert verdict["accepted"] is False
    assert any("scaffolding leaked" in p for p in verdict["problems"])


def test_a_complete_capture_is_accepted_with_its_stats() -> None:
    contract = compile_isolation({"workstreams": [_writer("w1", ["src/app"])]})
    verdict = verify_diff_capture(
        {
            "patch_digest": "sha256:a",
            ...
            "changed_files": ["src/app/x.py"],
        },
        contract["workstreams"][0],
    )
    assert verdict["accepted"] is True
    assert (verdict["insertions"], verdict["deletions"], verdict["files_changed"]) == (12, 3, 1)


def test_scaffolding_does_not_make_a_writer_receipt_out_of_scope() -> None:
    """Rejecting a receipt for the hook's own .venv trains everyone to ignore the verdict."""
    contract = compile_isolation({"workstreams": [_writer("w1", ["src/app"])]})
    verdict = verify_writer_receipt(
        {
            "workstream": "w1",
            "patch_digest": "sha256:d",
            "changed_files": ["src/app/main.py", ".venv"],
            "test_evidence": {"exit_code": 0},
        },
        contract["workstreams"][0],
        synthetic_paths=[".venv"],
    ...
    assert verdict["accepted"] is True
    assert verdict["ignored_synthetic_paths"] == [".venv"]
    assert verdict["changed_file_count"] == 1


def test_an_undeclared_stray_file_is_still_out_of_scope() -> None:
    contract = compile_isolation({"workstreams": [_writer("w1", ["src/app"])]})
    verdict = verify_writer_receipt(
        {
            "workstream": "w1",
