...
def test_two_node_cycle_rejected() -> None:
    ...
    ok, errors = validate_project_plan(plan)
    assert not ok and any("cycle" in e for e in errors)


def test_three_node_cycle_rejected() -> None:
    plan = copy.deepcopy(VALID_PLAN)
    plan["workstreams"] = [
        {"id": "a", "role": "backend", "depends_on": ["c"]},
        {"id": "b", "role": "frontend", "depends_on": ["a"]},
        {"id": "c", "role": "testing", "depends_on": ["b"]},
    ]
    ok, errors = validate_project_plan(plan)
    assert not ok and any("cycle" in e for e in errors)


def test_valid_dag_dependencies_still_accepted() -> None:
    # The canonical VALID_PLAN has a proper DAG (api->ui->docs/review); must pass.
    ok, errors = validate_project_plan(VALID_PLAN)
    assert ok, errors
