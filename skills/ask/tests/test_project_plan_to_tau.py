...
def test_freeze_hash_invariant_over_run_dir(tmp_path: Path) -> None:
    # #1246: the freeze stamp must be reproducible for the same logical plan
    # regardless of run location (it previously hashed absolute run_dir paths).
    a = compile_plan_to_tau_spec(PLAN, run_id="same", run_dir=tmp_path / "aaa")
    b = compile_plan_to_tau_spec(PLAN, run_id="same", run_dir=tmp_path / "bbb")
    assert a["extensions"]["spec_sha256"] == b["extensions"]["spec_sha256"]


def test_freeze_hash_changes_when_logical_plan_changes(tmp_path: Path) -> None:
    import copy

    a = compile_plan_to_tau_spec(PLAN, run_id="same", run_dir=tmp_path / "a")
    changed = copy.deepcopy(PLAN)
    changed["workstreams"][0]["role"] = "frontend"
    b = compile_plan_to_tau_spec(changed, run_id="same", run_dir=tmp_path / "b")
    assert a["extensions"]["spec_sha256"] != b["extensions"]["spec_sha256"]
