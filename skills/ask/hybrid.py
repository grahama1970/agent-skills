...
from skills_exec import MEMORY_AGENT_CLI, parse_memory_output, run_memory_recall, _get_memory_client
...
def learn_back(result: dict, persona_id: str, scope: str) -> None:
    ...
    if client is not None:
        try:
            client.learn(
                problem=problem, solution=solution, scope=scope,
                tags=["persona-finding", f"persona:{persona_id}"],
            )
            log.info("Learn-back stored for %s: %s", persona_id, question[:60])
            return
        except Exception as exc:
            log.debug("Direct learn-back failed: %s", exc)

    # Subprocess fallback
    if not MEMORY_AGENT_CLI.exists():
        log.debug("memory-agent CLI not found, skipping learn-back")
        return
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            log.debug("Learn-back failed (rc=%d): %s", proc.returncode, proc.stderr[:100])
    except (subprocess.TimeoutExpired, OSError) as exc:
        log.debug("Learn-back error: %s", exc)
