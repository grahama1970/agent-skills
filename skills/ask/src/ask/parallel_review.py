...
async def _run_judge_async(
    ...
    payload = {
        ...
        "required_output_schema": {
            ...
            "evidence_citations": [
                {
                    "source_id": "TARGET_BUNDLE or TARGET_BUNDLE.N",
                    "source_kind": "target_bundle",
                    "quote_or_summary": "quoted or summarized target evidence",
                    "supports": "verdict",
                }
            ],
            "findings": [],
...
Preserve structured evidence_citations from reviewers. SAFE or SAFE_WITH_CONDITIONS
requires target_bundle citations, not memory-only context.
"""
...
- Findings require structured evidence_citations from the target bundle.
- SAFE and SAFE_WITH_CONDITIONS require target_bundle citations.
...
def parse_reviewer_output(content: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    parsed["evidence_citations"] = normalize_citations(parsed.get("evidence_citations"), supports="verdict")
    ...
    for finding in parsed["findings"]:
        finding["evidence_citations"] = normalize_citations(finding.get("evidence_citations"), supports="finding")
    ...
    *,
    ...
    for output in reviewer_outputs:
        for finding in _list_value(output.get("findings")):
            if isinstance(finding, dict):
                ...
                evidence_citations.extend(normalize_citations(finding.get("evidence_citations"), supports="finding"))
        ...
        evidence_citations.extend(normalize_citations(output.get("evidence_citations"), supports="reviewer_verdict"))
    ...
    for finding in _list_value(judge_output.get("findings")):
        if isinstance(finding, dict):
            ...
            evidence_citations.extend(normalize_citations(finding.get("evidence_citations"), supports="judge_finding"))
    ...
    evidence_citations.extend(normalize_citations(judge_output.get("evidence_citations"), supports="judge_verdict"))
...
def verify_parallel_review_bundle(verdict: dict[str, Any]) -> dict[str, Any]:
    ...
    source_bundle = verdict.get("source_bundle") if isinstance(verdict.get("source_bundle"), dict) else {}
    ...
    if verdict.get("verdict") in SAFE_VERDICTS:
        ...
        if isinstance(target_bundle, dict) and target_bundle.get("truncated"):
            failures.append("safe verdict requires untruncated target bundle or chunk-complete citations")
        failures.extend(validate_citations(
            "verdict evidence_citations",
            normalize_citations(verdict.get("evidence_citations"), supports="verdict"),
            source_bundle=source_bundle,
    ...
    for output in reviewers or []:
        ...
        failures.extend(validate_citations(
            f"{output.get('reviewer', 'reviewer')}: evidence_citations",
            normalize_citations(output.get("evidence_citations"), supports="reviewer_verdict"),
            source_bundle=source_bundle,
            allowed_source_kinds=CODE_REVIEW_CITATION_KINDS,
            require_any=output.get("verdict") in SAFE_VERDICTS,
        ))
        ...
        for finding in _list_value(output.get("findings")):
            if isinstance(finding, dict):
                ...
                failures.extend(validate_citations(
                    f"{output.get('reviewer', 'reviewer')}: finding {finding.get('title', 'finding')}",
    ...
    if not isinstance(judge, dict):
        ...
        failures.extend(validate_citations(
            ...
            normalize_citations(judge.get("evidence_citations"), supports="judge_verdict"),
            source_bundle=source_bundle,
            allowed_source_kinds=CODE_REVIEW_CITATION_KINDS,
            require_any=judge.get("verdict") in SAFE_VERDICTS,
        ))
        for finding in _list_value(judge.get("findings")):
            if isinstance(finding, dict):
                failures.extend(validate_citations(
                    f"judge finding {finding.get('title', 'finding')}",
                    normalize_citations(finding.get("evidence_citations"), supports="judge_finding"),
                    source_bundle=source_bundle,
                    allowed_source_kinds=CODE_REVIEW_CITATION_KINDS,
                    require_any=True,
                ))
...
def render_parallel_review_markdown(verdict: dict[str, Any]) -> str:
    ...
    for finding in verdict.get("findings", []):
        lines.extend([
            ...
            f"  - Citations: {render_citations_markdown(finding.get('evidence_citations', [])).replace(chr(10), '; ')}",
            f"  - Impact: {finding.get('impact', '')}",
            f"  - Fix: {finding.get('fix', '')}",
            f"  - Verification: {finding.get('verification', '')}",
        ])
    lines.extend(["", "## Citations", "", render_citations_markdown(verdict.get("evidence_citations", []))])
    lines.append("")
    return "\n".join(lines)
