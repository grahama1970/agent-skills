...
from .skills_exec import parse_json_output, parse_memory_output, run_memory_recall, run_skill
...
def query_feed_items(query: str, limit: int = 10) -> list[dict]:
    """Query RSS feed items through the memory skill.
    ...
    for item in items[:limit]:
        feed_items.append({
            ...
            "summary": item.get("summary") or item.get("solution", ""),
            "source_key": item.get("source_key", item.get("_key", "")),
            "published_at": item.get("published_at", ""),
            "tags": item.get("tags", []),
        })

    log.info("Feed query '%s' returned %d items", query[:30], len(feed_items))
    return feed_items
