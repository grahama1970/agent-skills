...
description: >
  ...
  export editable JSON for Graham's own LinkedIn profile; and prepare bounded
  Surf own-profile sync plans after explicit account-risk acknowledgement.
  ...
  otherwise asks for LinkedIn operations. This skill does not automate outreach,
  scraping, third-party profiles, messages, posts, comments, or applications.
triggers:
  - linkedin
  - update my linkedin
  - linkedin profile
...
to a human. For Graham's own profile only, the skill can export an editable
`ops-linkedin.profile_entry.v1` JSON file from `RESUME.md` and prepare a bounded Surf
sync plan. The normal content/outreach lanes still stop before LinkedIn access.
...
- open, drive, inspect, or modify LinkedIn through a browser, extension, DOM bridge,
  ...
  own-profile sync plan generated by `profile-sync-plan`;
