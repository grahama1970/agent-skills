...
def test_promote_rejects_blocked_receipt(tmp_path):
    job = tmp_path
    (job / "repair-receipt.json").write_text(json.dumps({"status": "BLOCKED_MISSING_FRAMES"}), encoding="utf-8")
    (job / "validation.json").write_text(json.dumps({"passed": False}), encoding="utf-8")
    (job / "atlas.candidate.png").write_bytes(b"")
    (job / "atlas.candidate.json").write_text("{}", encoding="utf-8")
    with pytest.raises(RuntimeError):
        promote_candidate(job_dir=job, out_png=tmp_path / "out.png", out_json=tmp_path / "out.json")


def test_promote_accepts_passing_frame_patch_receipt(tmp_path):
    job = tmp_path / "job"
    job.mkdir()
    (job / "frame-patch-receipt.json").write_text(json.dumps({"status": "PASS_FRAME_PATCH"}), encoding="utf-8")
