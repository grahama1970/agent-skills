...
def horus_question(intent: str, stop: dict, state_str: str, spec: dict,
                   workspace: Path | None, line: int | None, prior: str = "") -> str:
    """Generate Horus's interrupting question with a DIFFERENT model than Embry.

    Horus is a second agent: his question is produced by DEBUGGER_HORUS_HANDLER
    (default claude-opus-5-low) via /ask, so the two speakers differ in both voice
    AND model. `intent` is the kind of question to ask (e.g. "a short followup tied
    to this exact breakpoint and Embry's last point", or "what the entire module is
    for"); the handler turns it into one concrete spoken question grounded in the
    breakpoint. Fail-soft: returns the intent verbatim if /ask is unavailable, so
    the walk still proceeds.
    """
    # Fable 5 (Anthropic) pairs with Embry on gpt-5.5 (OpenAI) for cross-vendor
    # model diversity; both return a grounded question in ~14s.
    handler = os.environ.get("DEBUGGER_HORUS_HANDLER", "claude-fable-5").strip()
    if handler in ("", "none"):
        return intent
    source = _source_context(workspace, stop, line)
    prior_line = f" Embry just said: \"{prior}\"." if prior else ""
    prompt = (
        f"You are Horus, a sharp engineer sitting in on a live debugger walkthrough. "
        f"Embry is paused at {stop.get('file')} -- {stop.get('say')} Paused state: {state_str}.{source}"
        f"{prior_line} Ask ONE short, natural spoken question of the kind: {intent}. "
        f"Output only the question itself, one sentence, no preamble."
    )
    try:
        child_env = {k: v for k, v in os.environ.items()
                     if k not in ("UV_PROJECT_ENVIRONMENT", "VIRTUAL_ENV", "UV_LINK_MODE")}
        ...
        for row in result.stdout.splitlines():
            if row.startswith("ANSWER ") and ":" in row:
                ...
                if path.is_file():
                    ...
                    body = text.split("## Evidence", 1)[0].replace("## Position", "").strip()
                    lines = [ln.strip().strip('"').strip() for ln in (body or text).splitlines() if ln.strip()]
                    # Prefer an actual question (a line ending in '?'); models often
                    # add a preamble line, so first-line is wrong. Fall back to the
                    # longest line.
...
def write_launch(workspace: Path, launch: dict, config_name: str) -> None:
    ...
    # Forward launch.env (KEY=VALUE strings) and launch.args so a spec can run a
    # module that lives in a subdir of the open workspace -- e.g.
    # PYTHONPATH=${workspaceFolder}/path/to/module -- rather than requiring the
    # workspace root to BE the module dir.
    for kv in launch.get("env", []):
        cmd += ["--env", kv]
    ...
    while index < len(stops):
        ...
        while not advanced:
            ...
            if low in ("", "c", "continue", "next", "n"):
                ...
                print("  (interrupted — Embry pauses)")
                acknowledge_pause()
            elif low.startswith("horus:"):
                # Horus is a full second agent: his question is GENERATED by a
                # different model (DEBUGGER_HORUS_HANDLER) from an intent hint, then
                # spoken in his voice; Embry answers with HER model. Two voices, two
                # models. The text after "horus:" is the intent, not the words.
                intent = cmd.split(":", 1)[1].strip()
                agent_voice = os.environ.get("DEBUGGER_AGENT_VOICE", "/voices/horus_ref.wav")
                prior = transcript[-1].get("last_answer", "") if transcript else ""
                question = horus_question(intent, stop, state_str, spec, workspace, line, prior)
                print(f"  HORUS asks: {question}")
                if args.speak:
                ...
                if transcript:
                    transcript[-1]["last_answer"] = answer
                if args.speak:
