...
def main() -> int:
    ...
    print("entities", sorted({overlay.get("entity", {}).get("name", "") for overlay in overlays}))
    ...
    for (stream_id, asset_uid, segment_id, track_id), track_events in sorted(grouped.items()):
        ...
        overlay = {
            ...
            "time_range": {
                ...
                "end_seconds": max(event["media_time_seconds"] for event in track_events),
            },
...
def _inspection(payload_path: Path, payload: dict[str, Any]) -> str:
    ...
    overlay_lines = "\n".join(
        f"- `{overlay['overlay_id']}`: `{overlay['entity']['name'] if overlay['entity'] else 'unresolved'}` "
