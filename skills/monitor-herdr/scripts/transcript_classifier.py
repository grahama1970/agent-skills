...
def transcript_goal_claim(text: str, *, project_root: Path | None = None) -> dict[str, str]:
    ...
    if re.search(r"^\s*DONE_WITH_RECEIPT(?:\s|$)", block, flags=re.MULTILINE) and completion_claim_has_evidence(block, project_root=project_root):
        return {"state": "achieved", "source": "status_line"}
...
def completion_claim_has_evidence(block: str, *, project_root: Path | None = None) -> bool:
    ...
    if goal.lower().startswith("achieved_with_receipt:"):
        ...
        if path_exists(receipt, project_root=project_root):
            return True
    evidence = structured_line_value(block, "Evidence")
    if not evidence or evidence.lower() == "none":
        return False
    return any(path_exists(value, project_root=project_root) for value in candidate_path_values(evidence))
...
def path_exists(value: str, *, project_root: Path | None = None) -> bool:
    if not value:
        return False
    path = Path(value).expanduser()
    candidates = [path]
    if not path.is_absolute() and project_root is not None:
        candidates.insert(0, project_root / path)
