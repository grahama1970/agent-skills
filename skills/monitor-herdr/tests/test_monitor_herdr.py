...
def test_monitor_prompt_body_does_not_hide_prior_done_receipt(tmp_path: Path) -> None:
    receipt = tmp_path / "receipt.json"
    receipt.write_text("{}", encoding="utf-8")
    text = """
    Status/Phase: Stop-condition proof completed.
    Immutable Goal: DONE_WITH_RECEIPT
    Evidence: pytest passed and receipt at {receipt}
...
def test_bare_done_with_receipt_without_evidence_is_unmet() -> None:
    text = """
    Status/Phase: Stop-condition proof completed.
    Immutable Goal: DONE_WITH_RECEIPT
    Next: STOP_ALLOWED because the goal has a fresh receipt.
...
def test_achieved_receipt_line_with_existing_path_allows_stop_without_duplicate_evidence_path(tmp_path: Path) -> None:
    receipt = tmp_path / "receipt.json"
    receipt.write_text("{}", encoding="utf-8")
    text = f"""
    Status/Phase: Stop-condition proof completed.
...
def test_achieved_receipt_line_with_missing_path_is_unmet(tmp_path: Path) -> None:
...
def test_achieved_receipt_with_existing_evidence_allows_stop(tmp_path: Path) -> None:
    receipt = tmp_path / "receipt.json"
    receipt.write_text("{}", encoding="utf-8")
    text = f"""
    Status/Phase: Stop-condition proof completed.
...
def test_completion_between_selection_and_send_sends_nothing_with_valid_receipt(tmp_path: Path) -> None:
    ...
    client = FakeCompletionBeforeSendHerdr(
        ...
        f"Evidence: pytest returned 0; receipt {receipt}\n"
        "Disposition: DONE_WITH_RECEIPT\n"
    )
    original_wait = monitor.wait_for_agent_idle
    monitor.wait_for_agent_idle = lambda pane_id, socket_path=None: {"ok": True, "exit_code": 0}
...
def test_describe_available_spaces_lists_every_addressable_key() -> None:
    """An unknown --space is a caller error; the error must teach the caller."""
    text = monitor.describe_available_spaces(
        [
            {"workspace_id": "w11", "label": "codex", "number": 1},
            {"workspace_id": "w12", "label": "autoupdate", "number": 2},
        ]
    )
    for token in ("w11", "codex", "w12", "autoupdate", "number=1", "number=2"):
        assert token in text
    assert "--space '*'" in text


def test_describe_available_spaces_handles_no_open_workspaces() -> None:
    text = monitor.describe_available_spaces([])
    assert "No Herdr workspaces are open" in text


def test_resolve_workspace_error_names_the_available_spaces() -> None:
    class FakeClient:
        def call(self, method, params):
            assert method == "workspace.list"
            return {"workspaces": [{"workspace_id": "w11", "label": "codex", "number": 1}]}

    with pytest.raises(RuntimeError) as excinfo:
        monitor.resolve_workspace(FakeClient(), "nope")
    message = str(excinfo.value)
    assert "nope" in message
    assert "codex" in message and "w11" in message
