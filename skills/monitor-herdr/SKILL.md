...
The restart prompt must treat completion claims as fail-closed. The agent may
answer `ACHIEVED_WITH_RECEIPT` or `DONE_WITH_RECEIPT` only when its `Evidence:`
line names a concrete local receipt/artifact path plus the command or
verification result that proves the immutable goal, and the current transcript
has no remaining human-requested work. A partial checkpoint, advisory reviewer
response, screenshot for only one surface, or receipt that proves a subtask must
be reported as `Immutable Goal: NOT_MET` with `Disposition: RESUMING_NOW`.
