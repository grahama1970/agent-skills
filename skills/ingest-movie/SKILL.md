...
**Composable Skills:**
- `ops-nzbgeek` - All NZBGeek search operations delegate to this skill for composability

This skill follows the **verb-prefix composability pattern**: instead of duplicating NZBGeek API code, it composes with `ops-nzbgeek` which provides clean verb-based commands (`search`, `download`, `status`). This ensures:
- Single source of truth for NZBGeek API
- Bug fixes benefit all consuming skills
- Other skills can easily compose with the same operations
- Clear separation of concerns (ingest-movie focuses on emotion extraction, ops-nzbgeek handles Usenet)
...
| Variable | Purpose |
|----------|---------|
| `NZBD_GEEK_API_KEY` (or `NZB_GEEK_API_KEY`) | Auth for NZBGeek search (delegated to ops-nzbgeek skill) |
| `NZBD_GEEK_BASE_URL` (or `NZB_GEEK_BASE_URL`) | Override for private Usenet proxies (delegated to ops-nzbgeek skill) |
| `SABNZBD_API_KEY` | SABnzbd download manager API key (delegated to ops-nzbgeek skill) |
| `SABNZBD_BASE_URL` | SABnzbd base URL (default `http://localhost:8090`, delegated to ops-nzbgeek skill) |
...
> **Note**: NZBGeek and SABnzbd environment variables are handled by the `ops-nzbgeek` skill. See `~/.pi/skills/ops-nzbgeek/SKILL.md` for details.
