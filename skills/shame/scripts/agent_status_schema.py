...
def validate_known_receipt(path: Path, text: str) -> dict[str, Any] | None:
    ...
    if schema == "agentic_evals.report.v2":
        ...
        if data.get("generated_by") != "lazy-report-shame-shame-shame":
            raise PydanticCustomError(
                "cross_provider_review_not_hook_generated",
                "cross-provider review proof must be generated by the Shame stop hook",
                {"proof": str(path)},
            )
        ...
        if metadata.get("generated_by") != "lazy-report-shame-shame-shame":
            raise PydanticCustomError(
                "cross_provider_review_metadata_not_hook_generated",
                "cross-provider review metadata must be generated by the Shame stop hook",
                {"proof": str(path)},
            )
        ...
        output_path = data.get("review_output_path") or metadata.get("output_path")
        if not output_path:
            raise PydanticCustomError(
                "cross_provider_review_output_missing",
                "cross-provider review proof must cite reviewer output",
                {"proof": str(path)},
            )
        ...
        if not output_file.is_file():
            raise PydanticCustomError(
                "cross_provider_review_output_not_found",
                "cross-provider review output path does not exist",
                {"proof": str(path), "review_output_path": str(output_file)},
            )
