...
def validate_known_receipt(path: Path, text: str) -> dict[str, Any] | None:
    ...
    if schema == "agentic_evals.report.v2":
        ...
        if data.get("generated_by") != "pi-subagents":
            raise PydanticCustomError(
                "cross_provider_review_not_hook_generated",
                "cross-provider review proof must be generated by pi-subagents",
                {"proof": str(path)},
            )
        ...
        if metadata.get("generated_by") != "pi-subagents":
            raise PydanticCustomError(
                "cross_provider_review_metadata_not_hook_generated",
                "cross-provider review metadata must be generated by pi-subagents",
                {"proof": str(path)},
            )
