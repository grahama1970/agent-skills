...
def fetch_transcript_with_retry(
    ...
    for attempt in range(max_retries + 1):
        try:
            # Webshare handles rotation automatically via WebshareProxyConfig
            proxy_settings = load_proxy_settings() if use_proxy else None

            if proxy_settings:
                proxy_used = True
                if attempt > 0:
                    typer.echo(f"Retry {attempt}/{max_retries} (Webshare auto-rotates IP)...", err=True)
    ...
    try:
        ...
        if hasattr(response, "segments") and response.segments:
            for seg in response.segments:
                # OpenAI SDK returns typed objects, not dicts
