...
def build_result(
    ...
    Args:
        ...
        summary: Document summary from doc2qra enrichment
        qra: Q&A pairs from doc2qra enrichment
    ...
    # Merge metadata if provided (title, channel, upload_date, duration_sec)
    if metadata:
...
def build_proxy_check_result(
    ...
    """
    import os
    from youtube_transcripts.config import WEBSHARE_API_KEY_ENV

    if not configured:
        return {
            "configured": False,
            "error": error or "Missing WEBSHARE_API_KEY environment variable",
            "env_vars": {
                WEBSHARE_API_KEY_ENV: "(set)" if os.getenv(WEBSHARE_API_KEY_ENV) else "(not set)",
            },
        }
