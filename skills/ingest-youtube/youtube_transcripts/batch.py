...
def run_batch(
    ...
    if use_proxy and load_proxy_settings() is None:
        typer.echo("WARNING: Webshare proxy not configured. Bulk downloads may fail.", err=True)
        typer.echo("Set: WEBSHARE_API_KEY", err=True)
        use_proxy = False
    ...
    total = len(pending)
    proxy_status = "Webshare proxy" if use_proxy else "direct (no proxy)"
