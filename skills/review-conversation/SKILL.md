...
### WRONG: Using grep to search conversation content
```bash
grep -r "GPS spoofing" results/sessions/  # misses semantic matches
```

### RIGHT: Use semantic search via /memory or structured find
```bash
./run.sh search "GPS spoofing questions that got resolved"
./run.sh find --text "GPS spoofing" --resolution resolved
```

### WRONG: Reviewing sessions without ingesting to memory first
```bash
./run.sh search "fake control missed"  # returns nothing — not ingested
```

### RIGHT: Ingest sessions to /memory, then search semantically
```bash
./run.sh ingest        # store session summaries in /memory
./run.sh search "fake control missed"  # now finds semantic matches
```

### WRONG: Looking at raw JSONL files instead of using the skill
```bash
cat results/sessions/sessions_20260222.jsonl | jq  # opaque, no grading context
```

### RIGHT: Use brief or show for rich context
```bash
./run.sh brief -n 3   # plain markdown with grades, evaluations, agreement
./run.sh show -n 3    # full Rich transcript with all metadata
```
