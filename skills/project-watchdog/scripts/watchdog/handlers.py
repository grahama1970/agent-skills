...
def build_repair_task(
    *,
    ...
    return (
        ...
        f"CREATOR ROUND: implement the scoped fix, record your content commit, and end your "
        f"response with a line 'CREATOR: DONE' plus the commit SHA and the changed paths. "
        f"The creator does NOT answer VERDICT and is NOT expected to have run the ticket's "
        f"proof command -- running the proof and judging it belongs to the reviewer round "
        f"and the watchdog's deterministic verification, not to you. Do run any focused tests "
        f"you need to be confident your change is correct, and name what you ran.\n\n"
        f"REVIEWER ROUND: check whether the code works, whether the ticket's acceptance criterion "
        ...
        f"The verdict must be on a line of its own and only the reviewer emits it. VERDICT: PASS is review, not closure, "
...
def _handle_ticket_repair_primary(run_id: str, receipt_dir: Path, project: dict[str, Any],
    ...
    task += (f"\nPrimary checkout on MAIN only: {root}. Creator uses that exact path.\n"
             ...
             "deterministic one-line shell commands suitable for ticket verify; no heredocs or literal newlines), "
             "artifacts (all result paths), and coverage "
             "(map from EVERY exact required clause below to the real command/artifact that proves it; "
             "use the single key all_required_clauses when the exact clause text would make the JSON huge). "
             "Keep VERIFY_PLAN compact and under 3000 characters. "
