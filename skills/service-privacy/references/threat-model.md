...
AppArmor kernel fixes CVE-2026-72456 and CVE-2026-23404, the
CVE-2026-43503 "DirtyClone" kernel LPE, the CVE-2026-46333 ptrace-path
local-root LPE + credential disclosure, and the CVE-2026-31431 kernel LPE), a
...
| osquery `curl` table / arbitrary logger-plugin HTTP exfiltration to any public URL (bypasses the "everything goes through the Kolide channel" assumption; network mode PUBLIC_EGRESS_LOCAL_DENY denies only local ranges, so arbitrary public egress is permitted) | Explicit non-claim: address-level local deny is not destination authentication. Closing it requires egress destination pinning (`IPAddressAllow=` Kolide endpoints + pinned DNS), an owner decision that changes agent capability and breaks update/CDN fetch unless the endpoint set is curated | Threat-model row + named residual decision | Owner-curated egress allowlist; hostname/DNS destination authentication |
| DNS exfiltration/tunneling via the owner-selected public resolvers (`public_dns` is deliberately exempt from IPAddressDeny; the confined agent can encode data in arbitrary queries to those resolvers) | Explicit non-claim: DNS is required for the egress mode to function. Restricting to a resolver with query logging/allowlisting, or DoH/DoT blocking, is an owner decision | Threat-model row + named residual decision | Owner resolver policy; resolver-side query allowlisting |
| io_uring ring fd acquired without `io_uring_setup` (inherited at exec or passed via SCM_RIGHTS), bypassing the seccomp `io_uring_setup` deny — seccomp filter theater | AppArmor profile emits `audit deny io_uring,` (AppArmor 4 io_uring mediation, independent of how the ring was created); unix connect/send/receive is peer-label-scoped so an unconfined peer cannot pass fds in; systemd closes non-stdio FDs at spawn and FileDescriptorStoreMax is rejected; RestrictAddressFamilies limits socket families | Renderer + native parser tests | io_uring mediation requires an AppArmor 4-capable kernel (Ubuntu 24.10+/6.10+); on older kernels the rule parses but is unenforced — the kernel-patch gate does not yet pin io_uring mediation support |
| Same-name AppArmor profile replacement passing the name-based readback for a polling interval (CrackArmor-class window: readback still shows `<name> (enforce)` while the loaded policy was swapped) | Watchdog now compares the LOADED-PROFILE READBACK CONTENT hash against an apply-time captured reference (`/etc/ubuntu-service-privacy/<name>/loaded-profile-sha256.txt`), not just the name; verify also flags LOADED_PROFILE_CONTENT_DRIFT | Watchdog artifact + transaction tests | The readback interface itself is what CrackArmor subverts; the owner-pinned apparmor/kernel patch gates and the unprivileged profile-management canary remain the primary controls, the hash check only narrows the window |
| Adjacent root-LPE binaries outside the unit (snapd/snap-confine: CVE-2026-3888, CVE-2026-8933 class) not covered by the apparmor/kernel version-pin gates | Executable allowlist is default-deny, so the confined agent cannot exec snap-confine; the profile additionally emits `audit deny /usr/lib/snapd/snap-confine`; host-wide snapd hardening (version pin/removal) is outside the unit threat and remains an owner decision | Renderer tests; explicit deny in profile | Owner host-hardening policy on snapd presence/version |
| systemd effective-directive drift between verify ticks (directive re-interaction after a daemon-reload/drop-in change; the old watchdog checked only the AppArmor enforce line — tamper-blinding between ticks) | Watchdog check script re-reads effective properties via `systemctl show` (AppArmorProfile, NoNewPrivileges, Protect*, PrivateTmp/Devices/IPC/Network) each tick and fails closed; verify_record already diffs the full effective set | Watchdog artifact tests | Full `systemd-analyze security` diffing per tick and osquery runtime-flag (`osquery_flags`) drift monitoring remain owner decisions |
...
| Syscall side-channels AppArmor does not mediate: perf_event_open (unprivileged cross-process perf sampling when kernel.perf_event_paranoid <= 2, historically default on Ubuntu), process_vm_readv, userfaultfd — cross-process memory reads/writes that bypass the file-policy boundary entirely | SystemCallFilter denies perf_event_open process_vm_readv userfaultfd (seccomp closes the syscall, independent of perf_event_paranoid) | Renderer and real systemd-analyze parse tests | A kernel.perf_event_paranoid>=3 sysctl gate in apply/verify (mirroring the userns gate) is left as an owner decision; deny covers the syscall path only |
| Tamper-detection signal blindness: the profile emits `audit deny` rules but nothing consumed the resulting AppArmor DENIED events, and watchdog coverage was 5-minute-tick only — a missing telemetry signal stayed silent (2026 defender guidance: a missing signal must become an alert) | Watchdog check scans kernel journal since the last recorded tick for `apparmor="DENIED"` events naming the profile and fails closed on a hit; unreadable/absent telemetry (journalctl missing or failing) is itself fail-closed; tick timestamp persisted at `/etc/ubuntu-service-privacy/<name>/last-tick` | Watchdog artifact tests | Per-event auditd consumption (rather than journal polling) and shorter-than-timer latency remain owner decisions; a deny attempt stops the service at the next tick, not instantly |
| Kernel ptrace-path LPE + credential disclosure (CVE-2026-46333: reading other processes' credentials via the ptrace path — a disclosure surface no file-policy row covers) | `audit deny ptrace,` mediates the ptrace entry point; the kernel-min-version pin must include the CVE-2026-46333 fix (pin list updated); absent/older pin fails closed at apply/verify | Renderer test (`audit deny ptrace,`); kernel pin gate tests | Updating the pin contents is a manual owner step — a stale pin silently fails to cover newer ptrace-path CVEs; continuous USN tracking remains an owner decision |
| IPv6 fragmentation container/host escape (CVE-2026-53362 "ipv6_frag_escape": carefully fragmented IPv6 packets toward local/link-local targets; unit-local IPAddressDeny BPF is a packet filter, not a fragmentation-aware firewall, so PUBLIC_EGRESS_LOCAL_DENY's loopback/local deny can plausibly be bypassed) | Not closed in this release: explicit non-claim. Mitigation (disabling IPv6 in the unit, or a fragmentation-aware host firewall, or the owner egress-pin decision already named below) changes connectivity/agent capability and requires an owner decision | Threat-model row + named residual decision | Owner decision: IPv6 disablement vs host-side stateful firewall; unit drift verification on target |
| Telemetry saturation blinding the DENIED-journal watchdog (flood audited denies to saturate journal/auditd rate limits or fill the audit backlog so the real DENIED event of a later attempt is dropped — "kill the telemetry path itself" per 2026 tamper-evasion guidance) | Watchdog fails closed on missing/unreadable telemetry, but ordinary kernel rate limiting can still drop individual events under flood | Watchdog artifact tests (fail-closed on absent telemetry) | Rate-limit/backlog anomaly alerting (alert when DENIED-event volume or dropped-event counters spike) and per-event auditd consumption remain owner decisions |
