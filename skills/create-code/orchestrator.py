...
SKILL_MAP = {
    ...
    # memory: accessed via httpx to Unix socket /run/user/1000/embry/memory.sock
...
"""
    ...
    try:
        top = subprocess.run(["git", "rev-parse", "--show-toplevel"], cwd=str(project_dir), capture_output=True, text=True, check=True,
        env={k: v for k, v in os.environ.items() if k != "VIRTUAL_ENV"},
        ).stdout.strip()
        repo = Path(top).name
        branch = subprocess.run(["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=str(project_dir), capture_output=True, text=True, check=True,
        env={k: v for k, v in os.environ.items() if k != "VIRTUAL_ENV"},
        ).stdout.strip() or branch
    except Exception as e:
        logger.warning("orchestrator error: {}", e)
    return repo, branch
