...
+  const prefix = controlId.trim().toUpperCase().split(/[-_.\s]/)[0];
...
+            limit: 0,
+          }),
+          postList<{ source?: string; target?: string; source_control_id?: string; target_control_id?: string; count?: number }>(
...
+
+    const controlsByFamily = new Map<NistFamily, Control[]>(
+      NIST_FAMILIES.map((family) => [family, [] as Control[]]),
+    );
+
