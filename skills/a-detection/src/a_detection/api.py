...
from a_detection.contracts import Edit, Health, Policy, SessionRequest, Strict, Submit
from a_detection.errors import Code, DetectionError, envelope
from a_detection.io import read_json, strict_json
from a_detection.model import load_model
...
def home_path() -> Path:
    explicit = os.environ.get("A_DETECTION_HOME")
    if explicit:
        return Path(explicit).expanduser().resolve()
    storage = Path("/mnt/storage12tb")
    if storage.is_dir() and os.access(storage, os.W_OK):
        return storage / "skills" / "a-detection"
    return Path(os.environ.get("XDG_DATA_HOME", str(Path.home() / ".local/share"))) / "a-detection"


def load_policy(path: Path | None = None) -> Policy:
    return Policy.model_validate(read_json(path) if path else
                                 strict_json(files("a_detection").joinpath("resources/policy.json").read_bytes()))
...
def create_app(db_path: Path | None = None, policy: Policy | None = None,
    ...
    selected_policy = policy or load_policy(Path(os.environ["A_DETECTION_POLICY"])
                                           if "A_DETECTION_POLICY" in os.environ else None)
    store = Store(db_path or home_path() / "sessions.sqlite3", selected_policy)
    configured = model_path or (Path(os.environ["A_DETECTION_MODEL"]) if "A_DETECTION_MODEL" in os.environ else None)
    pin = model_sha256 or os.environ.get("A_DETECTION_MODEL_SHA256")
    ...
    app = FastAPI(title="a-detection", version="0.1.0", lifespan=lifespan)
    ...
    static = Path(str(files("a_detection").joinpath("static")))
