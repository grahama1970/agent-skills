...
50 samples. Case-specific receipts are our own `a_detection.*` records, not fake
...
Set `A_DETECTION_EFFICACY_REQUEST` to the absolute path of a completed
...
`uv run a-detection efficacy-gate /external/study-request.json --output /external/numeric-proof.json`
