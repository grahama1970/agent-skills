...
class Receipt(BaseModel):
    ...
    schema_version: str = "a_detection.case_execution.v1"
    case: str
    status: str
    executed: bool
    exit_code: int
...
def main(case: Case, samples: int = 128):
    ...
    efficacy_request = os.environ.get("A_DETECTION_EFFICACY_REQUEST")
    ...
    if executed:
        ...
        if case == Case.EFFICACY:
            argv = [sys.executable, "-m", "a_detection", "efficacy-gate", efficacy_request,
                    "--output", str(run / "numeric-gate.json"), "--policy", str(ROOT / "specs/policy.json")]
        env = dict(os.environ, A_DETECTION_TEST_ARTIFACTS=str(run / "readback"))
