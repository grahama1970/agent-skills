...
```bash
uv run a-detection analyze examples/example.py
uv run a-detection verify-evidence /path/to/a-detection-session.json
```
...
```bash
uv run a-detection audit-dataset /external/study.jsonl
uv run a-detection train /external/study.jsonl \
  --output /external/model.json --report /external/training-report.json
uv run a-detection benchmark /external/study.jsonl \
  --model /external/model.json --output /external/heldout-report.json
```

To serve a trained artifact, set `A_DETECTION_MODEL` and
`A_DETECTION_MODEL_SHA256` using the **canonical model digest emitted by train**,
...
uv run a-detection native-setup --plan
uv run a-detection native-setup
uv run a-detection native-evals
uv run a-detection native-evals --release
uv run a-detection release-gate
```
...
`A_DETECTION_HOME` controls private runtime data. Otherwise the application prefers
writable `/mnt/storage12tb/skills/a-detection`, then the user's XDG data directory.
