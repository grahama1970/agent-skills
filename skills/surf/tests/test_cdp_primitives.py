...
def test_send_handles_connection_failure_before_websocket_exists() -> None:
    cdp = CDPController()
    cdp.MAX_RETRIES = 1
    ...
    cdp.connect = fail_connect  # type: ignore[method-assign]

    with pytest.raises(ConnectionError, match="Failed to send command after 1 attempts"):
        cdp.send("Page.navigate", {"url": "http://127.0.0.1/"})
