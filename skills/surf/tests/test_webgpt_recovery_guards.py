...
def test_extract_sentinel_auto_recovers_latest_stable_assistant_without_literal_marker(tmp_path: Path) -> None:
...
case "${{1:-}}" in
  chatgpt.extract)
    printf 'Position\\n\\n[{{"slot_id":"F36B","answer":"complete visible answer"}}]\\n'
    ;;
  *)
...
''',
    ...
    proc = subprocess.run(
        [
            "bash", str(EXTRACT), "--tab-id", "837360930", "--sentinel", "auto",
            "--stable-polls", "3", "--output", str(output), "--meta-output", str(meta),
        ],
        env=env,
        text=True,
        capture_output=True,
    ...
    assert "complete visible answer" in output.read_text(encoding="utf-8")
    invocation = calls.read_text(encoding="utf-8")
    assert "chatgpt.extract" in invocation
    assert "--wait --stable-polls 3 --no-activate" in invocation
    assert "--sentinel auto" not in invocation
    payload = __import__("json").loads(meta.read_text(encoding="utf-8"))
    assert payload["sentinel"] is None
    assert payload["sentinel_auto_recovery"] is True
    assert payload["raw_chars"] > 32
