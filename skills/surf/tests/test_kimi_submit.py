...
def test_kimi_submit_sigterm_preserves_requested_tab_metadata(tmp_path: Path) -> None:
    request = tmp_path / "request.md"
    response = tmp_path / "response.md"
    raw = tmp_path / "response.raw.md"
    meta = tmp_path / "response.meta.json"
    ...
    request.write_text("Use the attached bundle and wait.\n", encoding="utf-8")
    attachment.write_text("# Evidence\n\nReadable attachment text.\n", encoding="utf-8")
    fake_run.write_text(
        """#!/usr/bin/env bash
set -euo pipefail
...
""",
    ...
    assert payload["requested_tab_id"] == "837360924"
    assert payload["requested_url"] == "https://www.kimi.com/chat/test"
    assert payload["attach_file"] == str(attachment.resolve())
    assert payload["submitted_output"] == str(response) + ".submitted.md"
    assert payload["sentinel"] == "<<<KIMI_DONE:test>>>"
