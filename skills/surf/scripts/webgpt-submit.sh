...
if [[ -z "$tab_id" && -z "$target_url" && "$create_tab" -eq 0 ]]; then
  ...
  if [[ -n "$bo_payload" ]]; then
    ...
    if [[ -n "$bo_resolved_project" && -n "$bo_resolved_tab" && -n "$bo_reconcile_status" && "$bo_reconcile_status" != "ready" ]]; then
      echo "browser-oracle binding for project ${bo_resolved_project} is ${bo_reconcile_status}" >&2
