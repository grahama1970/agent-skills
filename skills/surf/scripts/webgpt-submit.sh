...
rate_limit_retry_attempts="${SURF_WEBGPT_RATE_LIMIT_RETRY_ATTEMPTS:-0}"
model=""
...
if [[ -n "${requested_tab_id:-}" ]]; then
  ...
  run_cdp_probe() {
    local err_path="$1"
    local out status
    # --timeout bounds the host-lease queue wait: a tab-scoped probe does not
    # contend on the plain-socket file lock a full submit holds, so lease
    # contention otherwise hangs until the hard timeout with no lock marker.
    if command -v timeout >/dev/null 2>&1; then
      set +e
      out="$(timeout "${cdp_probe_hard_timeout_s}s" "$RUN_SH" js "return 'cdp-ok'" --no-activate --tab-id "$requested_tab_id" --lock-timeout "$cdp_lock_timeout_s" --timeout "$cdp_lock_timeout_s" 2>>"$err_path")"
      status=$?
      set -e
    ...
    "$RUN_SH" js "return 'cdp-ok'" --no-activate --tab-id "$requested_tab_id" --lock-timeout "$cdp_lock_timeout_s" --timeout "$cdp_lock_timeout_s" 2>>"$err_path" || true
  }
  probe_lock_blocked() {
    local err_file="$1"
    [[ -n "$err_file" ]] || return 1
    grep -Eq 'SURF_BROWSER_LOCK_BLOCKED|timed out waiting for browser lease|browser lease queue is full' "$err_file" 2>/dev/null
  }
  cdp_ok="$(run_cdp_probe "$cdp_probe_err")"
  cdp_browser_lock_blocked=0
  cdp_retry_attempted=0
  if probe_lock_blocked "$cdp_probe_err"; then
  ...
  if [[ "$cdp_ok" != "cdp-ok" && "$cdp_ok" != '"cdp-ok"' && "$cdp_browser_lock_blocked" -eq 0 ]]; then
    ...
    if probe_lock_blocked "$cdp_retry_err"; then
      cdp_browser_lock_blocked=1
    fi
  fi
  if [[ "$cdp_ok" != "cdp-ok" && "$cdp_ok" != '"cdp-ok"' ]]; then
    if probe_lock_blocked "$cdp_probe_err" || probe_lock_blocked "${cdp_retry_err:-}"; then
      cdp_browser_lock_blocked=1
    fi
    failed_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
