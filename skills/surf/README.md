...
$surf webgpt.submit --input request.md --output response.md --tab-id 837344453 --reasoning "Pro"
...
| WebGPT reasoning | You need to switch ChatGPT's reasoning dropdown before submit | defaults to `Pro`; add `--reasoning "Heavy Reasoning"` only when explicitly needed |
...
```bash
./run.sh webgpt.submit --input REQ.md --output RESP.md --tab-id ID [--reasoning "Pro"] [--no-activate]
