...
function extractLatestAssistantSnapshot(candidates) {
  ...
  for (let i = candidates.length - 1; i >= 0; i--) {
    ...
    const rawTexts = [candidate?.text || ""];
    if (Array.isArray(candidate?.textAlternates)) {
      for (const alternate of candidate.textAlternates) {
        rawTexts.push(alternate || "");
      }
    }
    const text = rawTexts
      .map((value) => cleanChatGPTResponseText(value || ""))
...
async function readChatGPTResponseSnapshot(cdp, sentinel = null, signal) {
  return evaluate(
    cdp,
    `(() => {
      ...
      const SENTINEL = ${JSON.stringify(sentinel || null)};
      ...
      const candidateTextsFrom = (turnNode, searchRoot) => {
        const roots = [];
        addUnique(roots, searchRoot);
        addUnique(roots, turnNode);
        for (const selector of CONTENT_SELECTORS) {
          try {
            if (searchRoot?.matches?.(selector)) addUnique(roots, searchRoot);
            for (const match of Array.from(searchRoot?.querySelectorAll?.(selector) || [])) {
              addUnique(roots, match);
            }
          } catch (_) {}
        }
        const texts = [];
        for (const root of roots) {
          ...
          if (findSentinel(text)) return [text];
        }
        return texts;
      };
      ...
      const toCandidate = (turnNode, messageRoot = null) => {
        ...
        const textAlternates = candidateTextsFrom(turnNode, searchRoot);
        const text = textAlternates.reduce((best, value) => value.length > best.length ? value : best, '');
        const messageId =
      ...
      return {
        ...
        pageText,
        pageTextContainsSentinel: Boolean(findSentinel(pageText)),
        conversationMaxLengthDetected: detectsConversationMaxLength(pageText),
        tooManyRequestsDetected: detectsTooManyRequests(pageText),
        documentHidden: document.hidden === true,
        visibilityState: document.visibilityState || null,
...
async function readDirectAssistantSnapshot(cdp, sentinel, signal) {
  const value = await evaluate(
    ...
    `(() => {
      ...
      const bestTextFrom = (root) => {
        const roots = [];
        addUnique(roots, root);
        for (const selector of CONTENT_SELECTORS) {
          try {
            if (root?.matches?.(selector)) addUnique(roots, root);
      ...
      const turns = Array.from(document.querySelectorAll(ASSISTANT_SELECTOR));
      let bestTurn = null;
      let bestText = '';
      for (let i = turns.length - 1; i >= 0; i--) {
        const turn = turns[i];
        const text = bestTextFrom(turn);
        if (findSentinel(text)) {
          bestTurn = turn;
          bestText = text;
          break;
        }
      ...
      return {
        text: bestText,
        messageId: bestTurn?.getAttribute('data-message-id') || null,
        hasFinishedActions: Boolean(bestTurn?.querySelector?.(FINISHED_SELECTOR)),
        stopVisible: Boolean(document.querySelector(STOP_SELECTOR)),
        sentinelMatch: findSentinel(bestText),
        pageTextContainsSentinel: Boolean(findSentinel(pageText)),
        conversationMaxLengthDetected: detectsConversationMaxLength(pageText),
        tooManyRequestsDetected: detectsTooManyRequests(pageText),
        documentHidden: document.hidden === true,
        visibilityState: document.visibilityState || null,
        source: bestTurn ? 'assistant-dom-direct-fallback' : 'page-state'
      };
    })()`,
    signal
  );
