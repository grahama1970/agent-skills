...
const ALL_SOCKET_TOOLS = [
  ...
  "tab.list", "tab.recovery-state", "focus.state", "tab.new", "tab.switch", "tab.close", "tab.move", "tab.name", "tab.unname", "tab.named",
...
if (toolArgs["tab-id"] !== undefined) {
  const rawTid = toolArgs["tab-id"];
  const tid = typeof rawTid === "number"
    ? rawTid
    : (/^\d+$/.test(String(rawTid)) ? Number(rawTid) : NaN);
