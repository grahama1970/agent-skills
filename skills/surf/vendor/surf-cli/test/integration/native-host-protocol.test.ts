...
describe("native host protocol integration", () => {
  ...
  it("serializes local and authenticated remote requests through one host lease", async () => {
    ...
    remoteTransport.writeFrame(local, {
      type: "tool_request",
      method: "execute_tool",
      params: { tool: "click", args: { selector: "#local" } },
      id: "local",
    });
    const firstExtensionRequest = await host.waitForMessage(
      (message) => message.type === "CLICK_SELECTOR" && message.selector === "#local",
      "first local click",
    );
    remoteTransport.writeFrame(remote, {
      type: "tool_request",
      method: "execute_tool",
      params: { tool: "click", args: { selector: "#remote" } },
      id: "remote",
    });
    await host.expectNoMessage(
      (message) => message.type === "CLICK_SELECTOR" && message.selector === "#remote",
      "remote request before local lease release",
    );
    host.send({ id: firstExtensionRequest.id, success: true });
    ...
    host.send({ id: secondExtensionRequest.id, success: true });
  ...
  it("keeps a disconnected active request leased until extension settlement", async () => {
    ...
    remoteTransport.writeFrame(first, {
      type: "tool_request",
      method: "execute_tool",
      params: { tool: "click", args: { selector: "#abandoned" } },
      id: "abandoned",
    });
    const extensionRequest = await host.waitForMessage(
      (message) => message.type === "CLICK_SELECTOR" && message.selector === "#abandoned",
      "abandoned click",
    );
    first.destroy();
    ...
    remoteTransport.writeFrame(second, {
      type: "tool_request",
      method: "execute_tool",
      params: { tool: "click", args: { selector: "#queued" } },
      id: "queued",
    });
    await host.expectNoMessage(
      (message) => message.type === "CLICK_SELECTOR" && message.selector === "#queued",
      "queued request before abandoned operation settles",
    );
    host.send({ id: extensionRequest.id, success: true });
    const secondExtensionRequest = await host.waitForMessage(
      (message) => message.type === "CLICK_SELECTOR" && message.selector === "#queued",
      "queued click",
    );
  ...
  it("retains a nested provider tombstone until its late page response", async () => {
    ...
    remoteTransport.writeFrame(second, {
      type: "tool_request",
      method: "execute_tool",
      params: { tool: "click", args: { selector: "#nested-next" } },
      id: "nested-next",
    });
    await host.expectNoMessage(
      (message) => message.type === "CLICK_SELECTOR" && message.selector === "#nested-next",
    ...
    host.send({ id: nextRequest.id, success: true });
    second.destroy();
  });
  ...
  it("holds the lease until provider tombstones and tab cleanup both drain", async () => {
    ...
    remoteTransport.writeFrame(second, {
      type: "tool_request",
      method: "execute_tool",
      params: { tool: "click", args: { selector: "#after-provider-cleanup" } },
      id: "after-provider-cleanup",
    });

    await host.expectNoMessage(
      (message) => message.type === "CLICK_SELECTOR" && message.selector === "#after-provider-cleanup",
