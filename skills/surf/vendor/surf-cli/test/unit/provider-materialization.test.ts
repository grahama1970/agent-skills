...
describe("provider path materialization", () => {
  ...
  it("passes Kimi model and reasoning preferences through host message mapping", () => {
    const message = helpers.mapToolToMessage(
      "kimi_tab",
      { query: "q", model: "Instant", reasoning: "High" },
      837360924,
    );

    expect(message.type).toBe("KIMI_TAB_QUERY");
    expect(message.model).toBe("Instant");
    expect(message.reasoning).toBe("High");
  });
});
