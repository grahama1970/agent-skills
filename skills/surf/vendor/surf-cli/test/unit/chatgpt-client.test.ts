...
describe("chatgpt-client", () => {
  ...
  describe("extractLatestAssistantSnapshot", () => {
    ...
    it("uses the longest assistant text alternate instead of a short DOM fragment", () => {
      const snapshot = chatgptClient.extractLatestAssistantSnapshot([
        {
          role: "assistant",
          turn: "assistant",
          ...
          messageId: "msg-fragment",
        },
      ]);

      expect(snapshot?.text).toContain("complete visible answer");
      expect(snapshot?.text.length).toBeGreaterThan(32);
    });
  });
