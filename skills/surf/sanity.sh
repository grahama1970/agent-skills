...
main() {
    ...
    if [[ $check_only -eq 1 ]]; then
        echo -e "${RED}═══════════════════════════════════════════════════════════════${NC}"
        echo -e "${RED}  SKIP - SETUP INCOMPLETE (external dependency: Chrome extension)${NC}"
        echo -e "${RED}═══════════════════════════════════════════════════════════════${NC}"
        echo ""
        echo "Run without --check-only for setup instructions."
        exit 3  # SKIP — external dependency not available
    fi
    ...
    # Exit 3 = SKIP (external dependency: Chrome extension not loaded)
    exit 3
}
