...
def add_note(
    ...
    # Verify movie exists (Support title-based resolution)
    registry = ContentRegistry(registry_path)
    movie = registry.get_content(movie_id)
    if not movie:
        # Try searching by title
        results = registry.search_content(movie_id)
        if results:
            movie = results[0]
            movie_id = movie["content_id"]
            console.print(f"[dim]Resolved '{args_movie if 'args_movie' in locals() else movie_id}' to ID {movie_id}[/dim]")
        else:
...
def main():
    ...
    parser = typer.Typer(description="Manage movie notes")
