...
def list_movies(json_output: bool = False, registry_path: Optional[Path] = None, use_jellyfin: bool = False) -> list[dict]:
    """List all ingested movies.

    Args:
        json_output: Output as JSON
        registry_path: Override registry path
        use_jellyfin: Browse Jellyfin library
    """
    ...
    if use_jellyfin:
        ...
        try:
            console.print("[cyan]Browsing Jellyfin library...[/cyan]")
            items = jellyfin.search_items("")
            if json_output:
                return items
            
            table = Table(title="Jellyfin Library")
            ...
            console.print(f"[red]Jellyfin error: {e}[/red]")
            return []
...
def main(
    ...
    jellyfin: bool = typer.Option(False, help="Browse Jellyfin library"),
):
    """CLI entry point for list."""

    movies = list_movies(json_output=json, use_jellyfin=jellyfin)
