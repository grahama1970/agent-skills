...
AGENTIC_PROOF="skills/agentic-evals/run.sh run skills/ticket/fixtures/agentic_eval.json --case sanity --output /tmp/ticket-agentic-eval.json shows READY; ./run.sh sanity-live.sh --allow-live then read back receipt.json"
...
grep -q 'closure-unverified' "$TMPDIR/ensure-labels.out"
grep -q 'agent-maintenance' "$TMPDIR/ensure-labels.out"
grep -q 'agent:skill-maintainer' "$TMPDIR/ensure-labels.out"
...
FE="$("$SCRIPT_DIR/run.sh" bug "p" --target skills/x --observed o --expected e --repro r \
  --proof "$AGENTIC_PROOF" --route frontend_code 2>&1 | grep '^Labels' || true)"
OV="$("$SCRIPT_DIR/run.sh" bug "p" --target skills/x --observed o --expected e --repro r \
  --proof "$AGENTIC_PROOF" --route backend_python_or_skill_runtime --lane data 2>&1 | grep '^Labels' || true)"
BAD="$("$SCRIPT_DIR/run.sh" bug "p" --target skills/x --observed o --expected e --repro r \
  --proof "$AGENTIC_PROOF" --route backend_python_or_skill_runtime --lane nope 2>&1 || true)"
