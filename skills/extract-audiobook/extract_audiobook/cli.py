...
Splits an audiobook by its embedded ffprobe chapters, optionally transcribes
each chapter with faster-whisper, and writes memory-ready book_chapter records.
Also normalizes an existing flat transcript into per-chapter text by splitting
on sentence/paragraph boundaries proportional to chapter audio durations.

RECONSTRUCTED 2026-08-12 from the surviving compiled bytecode
(cli.cpython-312.pyc) after the .py source was lost (never tracked in git, no
disk copy survived). Every function and the AudioChapter dataclass were rebuilt
faithfully from the 3.12 disassembly (pycdc could not recover the dataclass or
the typer-decorated commands). Now TRACKED so the skill cannot be lost again.
"""
...
# The original used Annotated[OutputMode, ...] on the --mode CLI options. typer
# (through 0.15) does not accept a Literal as a click option type, so the CLI
# params below take a plain str; behaviour is identical because every consumer
# checks `mode in frozenset({...})`. The Literal alias is kept for the internal
# function signatures where it is valid.
...
def probe_audio(audio: Path) -> dict[str, Any]:
    data = run_json([
        "ffprobe", "-v", "error", "-print_format", "json", "-show_chapters", str(audio),
    ])
...
def chapter_title(raw: dict[str, Any], index: int) -> str:
    ...
    return title if title else f"Chapter {index:02d}"


def build_chapter_plan(
    audio: Path, out: Path, book: str, book_id: str, ffprobe_data: dict[str, Any] | None = None
) -> list[AudioChapter]:
    ...
    with path.open("w", encoding="utf-8") as f:
        for chapter in chapters:
            ...
            row.update({
                ...
                "tags": ["book_chapter", f"book:{chapter.book_id}", f"chapter:{chapter.chapter_id}"],
            })
...
def write_estimated_chapters_jsonl(
    chapters: list[AudioChapter], chapter_texts: list[str], out: Path, source_transcript: Path
) -> None:
    path = out / "chapters.jsonl"
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as f:
...
def audio_chapter_from_row(row: dict[str, Any], book: str, book_id: str) -> AudioChapter:
    book = book or row.get("book")
    book_id = book_id or row.get("book_id")
    return AudioChapter(
        index=int(row["index"]),
...
def transcribe_one(
    chapter: AudioChapter, book: str, mode: OutputMode,
    model_name: str, force: bool, device: str, compute_type: str,
) -> str:
    text_path = Path(chapter.text_path)
    jsonl_path = Path(chapter.transcript_jsonl_path)
    wants_text = mode in frozenset({"both", "chapter-text"})
    wants_jsonl = mode in frozenset({"both", "transcript-jsonl"})
    if not force and (not wants_text or valid_text(text_path)) and (not wants_jsonl or valid_jsonl(jsonl_path)):
        return "skipped_existing"
    try:
        ...
        raise RuntimeError(
            "faster_whisper is required for transcription; install or use split/probe only"
        ) from exc


def write_manifest(
    out: Path, audio: Path, book: str, book_id: str, mode: str,
    chapters: list[AudioChapter], actions: list[dict[str, Any]],
) -> dict[str, Any]:
    manifest = {
...
def split(
    ...
    for chapter in selected:
        actions.append({"chapter_id": chapter.chapter_id, "split": split_one(audio, chapter, force, limit_seconds)})
...
def transcribe(
    ...
    book: Annotated[str, typer.Option("--book")],
    book_id: Annotated[str, typer.Option("--book-id")],
    mode: Annotated[str, typer.Option("--mode", help="chapter-text | transcript-jsonl | both")] = "both",
    ...
    audio = Path(selected[0].audio_path) if selected else Path("")
    manifest = write_manifest(out, audio, book, book_id, mode, chapters, actions)
    typer.echo(json.dumps({**manifest, "chapters": manifest["chapters"][:len(selected)]}, indent=2, ensure_ascii=False))
...
def extract(
    ...
    actions = []
    for chapter in selected:
        action = {"chapter_id": chapter.chapter_id}
