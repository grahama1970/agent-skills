...
except ImportError:  # pragma: no cover - optional outside uv-managed runtime
    load_dotenv = None
...
VERIFY_RECEIPT_SCHEMA_VERSION = "fact-extractor-verify-receipt.v1"
VERIFY_OK_OUTCOMES = {"ACCEPTED_FACT_EXTRACTION_ARTIFACT"}
FACT_EXTRACTOR_TARGET_PATHS = [
...
def check_result(checks: list[dict[str, Any]], check_id: str, passed: bool, detail: str, **extra: Any) -> None:
    checks.append({"id": check_id, "passed": passed, "detail": detail, **extra})
...
def parse_receipt_int(
    ...
    *,
    ...
    if isinstance(raw, bool):
        detail = f"{field}={raw!r}:boolean_not_integer"
        ...
        detail = f"{field}={raw!r}:not_integer"

    passed = value is not None and value >= minimum
    if value is not None and value < minimum:
        detail = f"{field}={value}:below_minimum_{minimum}"
    check_result(checks, f"{check_prefix}_{field}_integer", passed, detail)
...
def validate_job_artifacts(job_dir: Path) -> dict[str, Any]:
    ...
    check_result(checks, "job_dir_exists", job_dir.is_dir(), f"{job_dir}")
    aggregate_path = job_dir / "aggregate_report.json"
    check_result(checks, "aggregate_report_present", aggregate_path.is_file(), str(aggregate_path))

    aggregate: dict[str, Any] = {}
    if aggregate_path.exists():
        loaded, error = try_load_json(aggregate_path)
        check_result(
            checks,
            "aggregate_report_json_object",
            loaded is not None,
            error or "aggregate_report.json parsed",
        )
        aggregate = loaded or {}
    ...
    if aggregate:
        check_result(
            ...
            aggregate.get("schema_version") == "fact-extractor-result.v1",
            f"schema_version={aggregate.get('schema_version')!r}",
        )
        check_result(
            checks,
            "aggregate_outcome_accepted",
            aggregate.get("accepted") is True,
            f"accepted={aggregate.get('accepted')!r}",
        )
        completed_chunks = parse_receipt_int(checks, aggregate, "completed_chunks", check_prefix="aggregate")
        accepted_chunks = parse_receipt_int(checks, aggregate, "accepted_chunks", check_prefix="aggregate")
        failed_chunks = parse_receipt_int(checks, aggregate, "failed_chunks", check_prefix="aggregate")
        total_records = parse_receipt_int(checks, aggregate, "total_records", check_prefix="aggregate")
        chunk_counts_valid = None not in (completed_chunks, accepted_chunks, failed_chunks)
        check_result(
            checks,
            "aggregate_chunk_counts_acceptable",
            chunk_counts_valid and completed_chunks > 0 and accepted_chunks == completed_chunks and failed_chunks == 0,
            f"completed={completed_chunks!r} accepted={accepted_chunks!r} failed={failed_chunks!r}",
        )
        check_result(
            checks,
            "aggregate_total_records_positive",
            total_records is not None and total_records > 0,
            f"total_records={total_records!r}",
        )
        check_result(
            checks,
            "accepted_record_metadata_ok",
            aggregate.get("accepted_record_metadata_ok") is True
            and not aggregate.get("accepted_record_metadata_defects"),
            f"defects={aggregate.get('accepted_record_metadata_defects') or []}",
        )
        check_result(
            checks,
            "forbidden_writes_preserved",
            aggregate.get("memory_writes_performed") is False
            and aggregate.get("forbidden_writes") == ["persona_memory", "lessons", "arangodb"],
            f"memory_writes_performed={aggregate.get('memory_writes_performed')!r}",
        )

    chunks_path = job_dir / "chunks.jsonl"
    accepted_path = job_dir / "accepted_records.jsonl"
    status_path = job_dir / "status.md"
    check_result(checks, "chunks_jsonl_present", chunks_path.is_file(), str(chunks_path))
    check_result(checks, "accepted_records_jsonl_present", accepted_path.is_file(), str(accepted_path))
    check_result(checks, "status_md_present", status_path.is_file(), str(status_path))

    chunks: list[dict[str, Any]] = []
    if chunks_path.exists():
        try:
            chunks = load_jsonl(chunks_path)
            check_result(checks, "chunks_jsonl_valid", bool(chunks), f"rows={len(chunks)}")
        except Exception as exc:
            check_result(checks, "chunks_jsonl_valid", False, str(exc))
    ...
    if accepted_path.exists():
        try:
            ...
            expected_records = (
                parse_receipt_int(checks, aggregate, "total_records", check_prefix="accepted_records_expected")
                if aggregate
                else None
            )
            check_result(
                checks,
                "accepted_records_jsonl_valid",
                bool(accepted_records)
                and (expected_records is None or len(accepted_records) == expected_records),
                f"rows={len(accepted_records)} expected={expected_records if expected_records is not None else 'unknown'}",
            )
        except Exception as exc:
            check_result(checks, "accepted_records_jsonl_valid", False, str(exc))

    if status_path.exists():
        status_text = status_path.read_text(encoding="utf-8", errors="ignore")
        check_result(
            checks,
            "status_outcome_accepted",
            any(outcome in status_text for outcome in VERIFY_OK_OUTCOMES),
            status_text.splitlines()[0] if status_text.splitlines() else "empty_status",
        )

    result_rows = aggregate.get("results") if isinstance(aggregate.get("results"), list) else []
    if result_rows:
        for result in result_rows:
            chunk_id = str(result.get("chunk_id") or "")
            record_count = parse_receipt_int(
                checks,
                result,
                "record_count",
                check_prefix=f"chunk_{chunk_id or 'unknown'}",
            )
            chunk_records = job_dir / "chunks" / chunk_id / "accepted_records.jsonl"
            try:
                rows = load_jsonl(chunk_records)
                passed = (
                    record_count is not None
                    and result.get("accepted") is True
                    and len(rows) == record_count
                    and record_count > 0
                )
                detail = f"{chunk_id}: rows={len(rows)} expected={record_count!r}"
            except Exception as exc:
                passed = False
                detail = f"{chunk_id}: {exc}"
            check_result(checks, f"chunk_{chunk_id}_accepted_records", passed, detail)
...
@app.command("verify")
def verify_command(
    job_dir: Annotated[Path | None, typer.Option("--job-dir", help="Extraction output directory to verify.")] = None,
...
def file_maintainer_ticket_command(
    job_dir: Annotated[Path | None, typer.Option("--job-dir", help="Extraction output directory to inspect.")] = None,
    out_dir: Annotated[Path | None, typer.Option("--out-dir", help="Alias for --job-dir.")] = None,
    ...
    packet = helper.write_maintainer_ticket(
        ...
        target_paths=FACT_EXTRACTOR_TARGET_PATHS,
        repro_commands=[
            "cd skills/fact-extractor && ./sanity.sh",
            "cd skills/fact-extractor && uv run pytest -q",
            f"cd skills/fact-extractor && ./run.sh verify --job-dir {target_dir}",
            f"cd skills/fact-extractor && ./run.sh file-maintainer-ticket --job-dir {target_dir}",
            "cd skills/best-practices-skills && uv run python scripts/validate_skill.py ../fact-extractor",
        ],
        issue_url="https://github.com/grahama1970/agent-skills/issues/15",
    )
