...
```bash
skills/fact-extractor/run.sh verify \
  --job-dir /mnt/storage12tb/skills/fact-extractor/outputs/horus_rising_ch02
```

This writes `verify-receipt.json` in the verified output directory and exits
nonzero when required extraction artifacts are missing, malformed, or rejected.

Create a local maintainer packet from the verification receipt:

```bash
skills/fact-extractor/run.sh file-maintainer-ticket \
  --job-dir /mnt/storage12tb/skills/fact-extractor/outputs/horus_rising_ch02
```
