...
- `claude-svg` (default): Claude authors the scene as SVG — palette-exact by
  ...
  the .png so a human can EDIT the art instead of regenerating. INTERIM LANE — runs
  `claude -p` directly, which the operator reports as often degraded; the
  required path is /ask -> Tau DAG -> /scillm `claude-fable-low` (#1386).
  Swap this backend the moment that handler lands; until then prefer
  `fal --model fal-ai/nano-banana` when quality matters.
