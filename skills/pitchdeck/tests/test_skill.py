...
from pitchdeck.io import SkillError, load_yaml
...
def _base_models(tmp_path: Path):
    ...
    source_data = {
        "schema": "pitchdeck.source_manifest.v1",
        "project_name": "Demo",
        "deck_title": "Demo",
        "audience": "Reviewers",
        "sources": [
...
def test_emit_ui_bundle_fails_closed_on_validation_errors(tmp_path: Path) -> None:
    sources, ledger, assets = _base_models(tmp_path)
    from pitchdeck.models import ClaimGuard, DeckManifest as DM, DeckMeta, SlideSpec, VisualSpec
    from pitchdeck.ui_emitter import emit_ui_bundle
...
def test_emit_markdown_one_way_export(tmp_path: Path) -> None:
    ...
    from pitchdeck.md_emitter import emit_markdown
...
def test_deck_ops_add_move_delete(tmp_path: Path) -> None:
    ...
    from pitchdeck.slide_edit import apply_deck_op
...
def test_emit_html_self_contained(tmp_path: Path) -> None:
    ...
    from pitchdeck.html_emitter import emit_html
...
def test_session1_rails_cas_watermark_artifact_scan(tmp_path: Path) -> None:
    ...
    from pitchdeck.models import Claim, ClaimKind, ClaimRisk, ClaimStatus, SourceRef, Visibility
