...
## 2026-08-06 — WebGPT project-state review sets the design gate
- Review artifact: /mnt/storage12tb/skills/pitchdeck/outputs/webgpt-review-2026-08-06.md (webgpt seat, pinned to commit 6effdf6e3). Diagnosis: blocked at the DESIGN-REPRESENTATION boundary — claims collapse to title/message/body rows + layout enum; house style is advisory knowledge, not an executable composition grammar. Required: Claim -> NarrativeModule -> SlideIntent -> CompositionRecipe -> claim-bound visual primitives -> deck_document.v1 -> emitters.
- CURRENT_GATE (adopted): pinned cold-planned six-slide Sparta conference mini-arc (cover/value-prop/problem/architecture/proof/roadmap) lossless + house-native in deck_document.v1, no manual rescue, zero dropped roles/bindings, rendered-oracle checks passing. #1264 blocked on it; sequence #1262-contract -> #1264 -> #1251 (diagram recipe library fills the primitive contract).
- Review found real compiler false-greens, fixed same day (503ca9098): silent binding-path drops, footer/body/visual-items content never becoming elements. Strict raise now enforced; sparta bundle 67->79 elements.
- Generation boundary ruling: generative drafting only pre-materialization (arc/intent), everything a candidate ClaimRendering with per-clause claim ids; compiler/emitters stay non-generative; diagram edges are factual relationships requiring bindings or explicit decorative class.

## 2026-08-07 — Roundtable ticket sweep (#1266–#1288): 6 closed, 17 sequenced
Closed with closure evidence (commits e96ebeb79, 5785a88c4, 75747f065): #1266 public projection (served document is visible-slide-reachable public records only), #1267 staged atomic publish, #1268 SVG allowlist gate, #1269 renderer neutrality + scoped theme state, #1274 hash-pinned exemplar corpus (clean-clone verifiable), #1288 generated command docs + release receipt with sanity stale gate.
Open by design, sequenced: #1275 (design schemas) -> #1276 (style import); #1271 (primitives) -> #1270 (TS types) & #1287 (media elements); #1262 golden gate -> #1264 (emitters) -> #1283-#1286 (figure slices); #1272/#1273 (capability model, parity gate) ride #1264; #1277-#1282 (outline workflow, lint, SourceGraph, drift repairs, DesignChangeSets) build on the composition contract.

## 2026-08-07 — #1271 primitive expansion: freeze BLOCKED by design review (correctly)
Research (/brave-search) + webgpt review before implementation, per operator direction. Review verdict: kind names right, contract not freezable — the missing object is a RECURSIVE DISCRIMINATED scene-graph contract (exact-one payloads; all invariants recursing through groups; normative group transform preserving off/ext/chOff/chExt; canonical line endpoints w/ advisory site-hints; typed rich-text marks w/ plain-text projection; library-ref icons w/ target-profile fail-closed fallback; array-order z; additive-v1 compatibility with golden BYTES revalidating). Full synthesis on #1271; artifact webgpt-review-1271-2026-08-07.md. Latent bug flagged: public projection asset reachability doesn't recurse (harmless pre-groups). Gate: hand-authored "primitive-gauntlet" document is the freeze test and comes FIRST; emitters wait.

## 2026-08-07 — the cold pipeline exists end-to-end (#1277 core + #1278 closed)
DECK_CONTEXT -> claim-routed narrative outline (verbatim assertion candidates, typed questions, hash-bound approval) -> deterministic materializer through recipe constraints -> intent-carrying deck_document -> house HTML + NATIVE editable PPTX (DiagramGraph compiles to grouped primitives; 8 named shapes for the endpoint bridge; webp converts). Golden six-recipe arc reproduced from outline DATA with zero slide Python. Sweep tally: 10 of #1266-#1288 closed with evidence (#1266-69, 71, 74-76, 78, 88). #1277 open: React approval panel + context fixtures. #1262 gate remaining: rendered-oracle checks + pinned cold run + Graham blind rating.

## 2026-08-07 — first fully-authorized cold deck (human-approved renderings)
Graham approved the five reviewer-amended headline renderings in chat (provenance recorded per rendering + hash-bound outline re-approval). approved.document.json materialized WITHOUT preview flag — publish gate accepted it; design-lint zero findings; house HTML + native PPTX emitted (outputs/ticket-1278/approved.*). Full chain now proven: README claims -> outline -> agent proposals -> oracle review (3/5 reworded) -> human approval -> authorized deck. Remaining #1262 gate: rendered-oracle envelope checks + Graham blind rating; #1279 remainder: single-ceremony approval UX, ledger-digest approval binding, required_qualifier rendering.
