...
**Last updated:** 2026-08-11 14:37 by agent
...
- 2026-08-11: Full compare-and-adjust loop is live. (1) Assertion authorization: authorize_atom verifies transform classes against real claim text, generalizations need named human attestation, occurrence-scoped keys (text@element-id) block copy-paste reuse, decorative edges structurally cannot carry labels, approvals bind the ledger digest. Reproduced forgery (inverted edge label with valid claim_id) now REFUSED; was PASS. (2) Build manifest (#1332 chain half): content-addressed digest chain over sources/ledger/outline/document/template/icons/compiler-state/delivered-pptx; verify_manifest re-computes every digest; verify-publish --build-manifest refuses INPUT_DRIFT/TEMPLATE_HASH_MISMATCH/DELIVERED_ARTIFACT_DRIFT/DIRTY_COMPILER_STATE/COMMIT_UNRESOLVABLE. (3) House index fixed and expanded: embed field is 'image' with data URL (image_b64 was silently ignored — all 250 image vectors had been identical text embeddings); pdftotext page mapping corrects hidden-slide offsets; 203 real pages each have a JSON record, real Qdrant vectors, and a /memory doc (collection pitchdeck_house_slides, count verified 203). (4) house-similarity CLI gate: nearest-real-page score, calibrated (replica 0.72 / unrelated 0.46 / gate 0.55), with field-level diffs via extract_signatures on both sides. Sparta deck: 6/6 FAIL (0.36-0.46); causes converge on picture_count 0 vs median 2 and content_area 0.15 vs 0.35. (5) STYLE_GUIDE.md in pitchdeck + DESIGN_SLIDES.md archetype catalog (all 263 slides, ten archetypes) in best-practices-slide-design. README rewritten to state the audited NOT_READY status. Next: #1331 asset registry so real/registered artwork can enter the illustration zone — all three measurements (judges, replication probe, field diffs) converge there. Suite 115 passed.
