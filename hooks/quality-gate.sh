...
#   QUALITY_GATE_TIMEOUT=120  - Test timeout in seconds (default: 120)
...
MAX_RETRIES=3
TIMEOUT="${QUALITY_GATE_TIMEOUT:-120}"
...
if [[ -f "Makefile" ]]; then
    ...
    if grep -q "^smokes-cli-fast:" Makefile 2>/dev/null; then
        CHECK_CMD="timeout ${TIMEOUT}s make smokes-cli-fast"
    elif grep -q "^smokes-cli:" Makefile 2>/dev/null; then
...
if [[ -z "$CHECK_CMD" ]] && { [[ -f "pyproject.toml" ]] || [[ -f "setup.py" ]] || [[ -f "requirements.txt" ]]; }; then
    if [[ -d "tests" ]] || [[ -d "test" ]]; then
        # Use the actual test directory to avoid discovering tests in site-packages
        TEST_DIR="tests"
        [[ -d "test" ]] && TEST_DIR="test"
        # Prefer uv if available, fall back to python -m pytest
        if command -v uv &>/dev/null; then
            CHECK_CMD="timeout ${TIMEOUT}s uv run pytest $TEST_DIR -q -x --tb=short 2>&1 | tail -30"
        else
            CHECK_CMD="timeout ${TIMEOUT}s python -m pytest $TEST_DIR -q -x --tb=short 2>&1 | tail -30"
        fi
    else
...
# Node.js detection
# NEVER run npm test in a monorepo root — it fans out to all workspaces.
# 2026-03-17: npm test → vitest in all packages → 260+ processes → 100% CPU → KDE crash.
# Only run in single-package dirs (no "workspaces" in package.json).
if [[ -z "$CHECK_CMD" ]] && [[ -f "package.json" ]]; then
    if grep -q '"test"' package.json 2>/dev/null && ! grep -q '"workspaces"' package.json 2>/dev/null; then
