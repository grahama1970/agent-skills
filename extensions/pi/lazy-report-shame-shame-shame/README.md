...
8. after pydantic status/proof validation passes, runs a bounded semantic clarity review over only `plain_answer`/`answer`; the review checks responsiveness, plain English, and receipt/hash/handler jargon, but never validates proof, changes status facts, or marks work done;
9. leaves accepted model output unchanged after validation: the human answer stays plain, and the fenced `pi.agent_status.v1` JSON remains the terminal machine contract. It does not append a prose `Status Report` footer.
10. replaces rejected output with a compact `REJECTED_BY_SLOTH_COURT` notice that hashes raw diagnostics and points to the typed retry packet;
11. prepares at most one output-only `UNLAZY_FORCED_RETRY` correction per reporting episode, armed or unarmed, and dispatches it only at `agent_end`; correction turns cannot call tools or request another correction; the retry packet includes a copyable `suggested_status` continuing report; format-only retry output must be exactly one fenced `pi.agent_status.v1` JSON block, and guard-internal `lazy_report_shame.*` schemas are rejected as `format_retry_wrong_schema`;
12. tells the human how to label the raw rejected candidate with `/shame reject|allow|warn <reason> -- <note>` after automatic repair is exhausted.

The default mode is `normal`, but terminal stops are zero-trust by default: `LAZY_REPORT_SHAME_UNIVERSAL_STOP_REVIEW=1` forces every terminal assistant stop through the status contract and cross-family reviewer receipt gate, including trivial read-only answers. Set it to `0` only for deliberate local debugging. The reviewer is the real Pi `reviewer` subagent launched synchronously by the installed `pi-subagents` extension from the parent harness; Shame only requests the review and validates its child-run/session-bound receipt. If pi-subagents is unavailable, Shame suppresses the unreviewed terminal response and queues at most one harness-owned course correction per human turn; a repeated infrastructure failure remains suppressed without looping the model. If the reviewer returns `VERDICT: REJECT`, the guard restarts with its critique. Neither path asks the agent to create review proof. The reviewer model is operator-configurable in Pi settings under `subagents.stopReview`; the default cheap reviewer is `zai/glm-5.3-flash`, with fallback models used only when the reviewer provider fails to produce an explicit verdict. Use `LAZY_REPORT_SHAME_DEFAULT_MODE=strict` for the older session-wide strict badge semantics, or `/shame normal|off|strict` to override a session. Leading `$unlazy`/`/unlazy`/`/skill:unlazy` invocations add one-turn enforcement. Leading `$shame`/`/shame`/`/skill:shame` invocations request the Shame contract and permit self-correction. Mentioning these skills or the phrase `acceptance ledger` in an advisory question does not activate a separate gate. New human input clears stale correction and skill-read flags. Report corrections are output-only even without an armed task budget; they cannot launch checks or reopen accepted work. Read-only failure history remains available outside correction turns. The `/lazy-report-shame-shame-shame` command enables session-wide enforcement explicitly. A continuation ledger file also enables status enforcement for that session because the guard has machine-readable unfinished work to check.
...
The `advisory-question`, `advisory-question-scope`, `vague-status-report-concrete-anchors`, and `single-format-correction` cases exercise the actual conversational seam: one advisory answer, a deterministic no-status-required scope check, semantic clarity rejection for opaque/nonresponsive answers, and one tool-free correction after real file work.
...
`/mnt/storage12tb/skills/shame/ticket-outbox/`. By default the extension runs the
contained `$ticket` maintenance command to create an `agent-work` issue targeting
`skills/shame`; set `LAZY_REPORT_SHAME_SPIRAL_TICKET_APPLY=0` to leave only the
outbox record. `$project-watchdog` handles the created ticket through `ticket_repair`.
