...
The `advisory-question`, `advisory-question-scope`, and `single-format-correction` cases exercise the actual conversational seam: one advisory answer, a deterministic no-status-required scope check, and one tool-free correction after real file work.
