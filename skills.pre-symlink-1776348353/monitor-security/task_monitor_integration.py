"""Task Monitor Integration for monitor-security.

Thin wrapper around shared TaskClient. Preserves SecurityMonitorTracker interface.
"""
...
STATE_DIR = Path.home() / ".pi" / "monitor-security"


class SecurityMonitorTracker:
    """Task-monitor compatible tracker for security health probes."""

    def __init__(
        self,
        job_type: str = "check",
        name: str = "security-monitor",
        register: bool = True,
    ):
        ...
        self._client = TaskClient(
            ...
            total=1,
            description=f"Security Monitor: {job_type}",
...
_trackers: Dict[str, SecurityMonitorTracker] = {}


def start_job(job_type: str) -> SecurityMonitorTracker:
    tracker = SecurityMonitorTracker(job_type=job_type)
    _trackers[job_type] = tracker
    return tracker


def get_tracker(job_type: str) -> Optional[SecurityMonitorTracker]:
