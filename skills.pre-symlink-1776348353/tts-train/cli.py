    ...
    if transcribe:
        ...
        for af in processed_sources:
            ...
            if not json_path.exists():
                ...
                uid = os.getuid()
                gid = os.getgid()
                cache_huggingface = os.path.expanduser('~/.cache/huggingface')
                cache_torch = os.path.expanduser('~/.cache/torch')
                safe_mkdir(Path(cache_huggingface))
                safe_mkdir(Path(cache_torch))
                ...
                cmd = [
                    ...
                    "-v", f"{abs_in}:/app/in",
                    "-v", f"{abs_out}:/app/out",
                    "-v", f"{cache_huggingface}:/tmp/.cache/huggingface",
                    "-v", f"{cache_torch}:/tmp/.cache/torch",
