...
# Try to use 8-bit AdamW to save ~50% optimizer memory
try:
...
def train():
    ...
    parser.add_argument("--batch_size", type=int, default=1)  # Reduced for 24GB GPU
    parser.add_argument("--gradient_accumulation_steps", type=int, default=8)  # Compensate for smaller batch
    parser.add_argument("--lr", type=float, default=2e-5)
    parser.add_argument("--num_epochs", type=int, default=3)
    parser.add_argument("--speaker_name", type=str, default="speaker_test")
    parser.add_argument("--gradient_checkpointing", action="store_true", default=True,
                        help="Enable gradient checkpointing to reduce memory (required for 1.7B on 24GB)")
    args = parser.parse_args()

    import os as _os
    _os.makedirs(args.output_model_path, exist_ok=True)

    # Use higher gradient accumulation to compensate for smaller batch size
    ...
    accelerator.print(f"Loading model: {MODEL_PATH}")
    ...
    accelerator.print(f"8-bit optimizer: {USING_8BIT}")
    ...
    # This is critical for fitting 1.7B model on 24GB GPU
    if args.gradient_checkpointing:
        ...
        if hasattr(qwen3tts.model, 'gradient_checkpointing_enable'):
            ...
            accelerator.print("  ✓ Gradient checkpointing enabled on main model")
        if hasattr(qwen3tts.model, 'talker') and hasattr(qwen3tts.model.talker, 'gradient_checkpointing_enable'):
            qwen3tts.model.talker.gradient_checkpointing_enable()
            accelerator.print("  ✓ Gradient checkpointing enabled on talker")
    ...
    for epoch in range(num_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                # Use official loss weighting: main loss + 0.3 * sub_talker_loss
