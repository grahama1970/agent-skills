> **Review Metadata**: Round 2 | Step 1 | Provider: github | Model: claude-sonnet-4.5
---

I'll analyze the create-subagent skill to identify security, reliability, and code quality issues, then generate a comprehensive patch.

✓ List directory ~/workspace/experiments/pi-mono/.pi/skills/create-subagent (13 files)

✓ Read ~/workspace/experiments/pi-mono/.pi/skills/create-subagent/run.sh (287 lines)

✓ Read ~/workspace/experiments/pi-mono/.pi/skills/create-subagent/server.py (553 lines)

✓ Read ~/workspace/experiments/pi-mono/.pi/skills/create-subagent/backends.yml (62 lines)

✓ Read ~/workspace/experiments/pi-mono/.pi/skills/create-subagent/sanity.sh (65 lines)

✓ Read ~/workspace/experiments/pi-mono/.pi/skills/create-subagent/Dockerfile (40 lines)

Now I'll analyze the code to identify critical issues across security, reliability, and code quality:

## Clarifying Questions

1. **Environment variable blocklist scope**: Should the env var blocklist include all cloud provider credentials (AWS, GCP, Azure) or just the ones currently listed? Should we also block ANTHROPIC_API_KEY, OPENAI_API_KEY, etc. that might leak into the container?

2. **Credential validation timing**: Should credential validation happen at container start (pre-flight check) or at first request? Pre-flight is safer but adds startup dependency.

3. **Zombie process handling strategy**: The current code has inconsistent `await proc.wait()` after `proc.kill()`. Should we add a cleanup wrapper or fix case-by-case?

4. **Stderr handling approach**: The current code reads stderr twice in the `/chat` endpoint (lines 307, 318), which will fail. Should we return stderr from `_collect_with_idle_timeout()` or read it only once before that function?

5. **Host network security documentation**: The container uses `--network host` which exposes all host services. Should we add runtime warnings or documentation about this security trade-off?

**Assumed answers for this patch:**
- Blocklist: Include major cloud providers (AWS, GCP, Azure) and API keys
- Validation: Pre-flight check for credentials to fail fast
- Zombies: Fix all instances with `await proc.wait()`
- Stderr: Return from `_collect_with_idle_timeout()` to avoid double-read
- Security: Add validation and comments, defer major architecture changes
...
@@ -10,7 +10,6 @@ Timeout is inactivity-based (no output for N seconds), not total duration.
 ...
 import json
 import os
@@ -176,17 +175,23 @@ def _build_cmd(backend_name: str, model: str, prompt: str,
...
+    Uses blocklist approach to prevent credential leaks while allowing
+    CLIs to access standard environment variables they need.
     """
-    env = os.environ.copy()
...
@@ -268,9 +273,9 @@ class ChatResponse(BaseModel):
...
+    cfg = BACKENDS[backend_name]
     ...
     t0 = time.monotonic()
@@ -283,11 +288,13 @@ async def chat(req: ChatRequest):
     )
...
@@ -302,19 +309,13 @@ async def chat(req: ChatRequest):
...
@@ -344,9 +345,9 @@ async def chat(req: ChatRequest):
...
@@ -379,6 +380,7 @@ async def chat_stream(req: ChatRequest):
...
@@ -424,14 +426,15 @@ async def chat_stream(req: ChatRequest):
...
+    Returns: (result_text, num_events, events, stderr_text)
     ...
     text_lines: list[str] = []
     result_text = ""
@@ -466,11 +469,18 @@ async def _collect_with_idle_timeout(
...
+    if ! python3 -c "import json; json.load(open('${CLAUDE_HOME}/.credentials.json'))" 2>/dev/null; then
...
+        # Pass service URLs - container validates connectivity at runtime
...
+        echo "  Memory: ${MEMORY_ARANGO_URL} (will validate at runtime)"
+        echo "  Embedding: ${EMBEDDING_SERVICE_URL} (will validate at runtime)"
     fi
...
+    if ! python3 -c "import json; json.load(open('${HOME}/.claude/.credentials.json'))" 2>/dev/null; then
...
Total duration (API):  58.1s
Total duration (wall): 1m 0.3s
Total code changes:    0 lines added, 0 lines removed
Usage by model:
    claude-sonnet-4.5    105.2k input, 3.4k output, 0 cache read, 0 cache write (Est. 1 Premium request)
