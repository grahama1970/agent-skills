...
**Key Finding:** **NO existing tools automate code-to-paper generation.** Our create-paper skill fills a **unique gap** in the market.
...
```
Project Code → /assess + /code-review → /arxiv (learn style) → create-paper (generate) → Paper
```
...
## Recommendations for create-paper Skill
...
| Feature              | Manubot   | Pandoc    | PyLaTeX        | AI Tools     | **create-paper**              |
| -------------------- | --------- | --------- | -------------- | ------------ | ----------------------------- |
