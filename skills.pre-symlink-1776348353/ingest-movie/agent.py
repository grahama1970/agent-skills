...
def quick_extract(
    ...
    def _cleanup_on_failure():
        ...
        if emotion_clips_dir.exists() and not any(emotion_clips_dir.iterdir()):
            try:
                ...
                logger.debug("emotion_clips_dir failed: {}", e)
        ...
        if persona_dir.exists():
            try:
                ...
                try:
                    ...
                    logger.debug("persona_dir failed: {}", e)
    ...
    # Step 5: Produce KSML Bridge (Movie-to-KSML)
    console.print("\n[bold]Step 5/5:[/bold] Creating KSML Bridge...")
    ksml_path = None
    try:
        bridge = KSMLBridge(output_dir or emotion_clips_dir)
        ksml_path = bridge.create_ksml(movie_title, clip_video, final_output)
        console.print(f"[green]KSML Bridge created: {ksml_path.name}[/green]")
    except Exception as e:
        console.print(f"[yellow]Warning: Failed to create KSML bridge: {e}[/yellow]")
    ...
    return {
        ...
        "persona_json": str(final_output),
        "clip_id": clip_name,
        "ksml": str(ksml_path) if ksml_path else None,
    }
...
def show_inventory(emotion: Optional[str] = None, as_json: bool = False) -> Dict[str, Any]:
    ...
    for e in VALID_EMOTIONS:
        ...
        status = "[done]" if count >= 5 else "[need]"
