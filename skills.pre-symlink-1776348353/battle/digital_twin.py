...
class DigitalTwin:
    ...
    def _is_worktree_dirty(self, worktree: Path) -> bool:
        result = self._run_git("status", "--porcelain", cwd=worktree)
        return result.returncode == 0 and bool(result.stdout.strip())

    def _apply_dirty_worktree(self) -> bool:
        if not self.blue_worktree or not self.arena_worktree:
            return False
        diff = self._run_git("diff", "--binary", "HEAD", cwd=self.blue_worktree)
        ...
        if diff.stdout.strip():
            apply = subprocess.run(
                ...
                input=diff.stdout,
                text=True,
                cwd=self.arena_worktree,
            )
            if apply.returncode != 0:
                return False
        untracked = self._run_git("ls-files", "--others", "--exclude-standard", cwd=self.blue_worktree)
        if untracked.returncode != 0:
            return False
        for rel_path in [p for p in untracked.stdout.splitlines() if p]:
            src = self.blue_worktree / rel_path
            if not src.is_file():
                continue
    ...
    def sync_blue_to_arena(self) -> bool:
        ...
        if self.mode == TwinMode.GIT_WORKTREE:
            if not self.blue_worktree or not self.arena_worktree:
                return False
            try:
                if self._is_worktree_dirty(self.blue_worktree):
                    return self._apply_dirty_worktree()
                result = self._run_git("rev-parse", "HEAD", cwd=self.blue_worktree)
                if result.returncode != 0 or not result.stdout.strip():
                    return False
                pick = self._run_git("cherry-pick", result.stdout.strip(), cwd=self.arena_worktree)
                if pick.returncode != 0:
                    self._run_git("cherry-pick", "--abort", cwd=self.arena_worktree)
                    return False
                return True
            except Exception:
                return False
