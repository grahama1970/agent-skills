...
if __name__ == "__main__":
    ...
    @app.command()
    def main(
        command: str = typer.Argument(..., help="Command to run (seed, recall, recommend)"),
        model: Optional[str] = typer.Option(None, "--model", "-m", help="Model alias"),
        type: str = typer.Option("taxonomy", "--type", "-t", help="Prompt type"),
    ):
        memory = get_model_memory()
        ...
        if command == "seed":
            ...
            if not model:
                ...
                raise typer.Exit(1)
            notes = memory.recall_for_model(model, type)
            print(f"Notes for {model}:")
            for note in notes:
                print(f"  - {note.get('solution', '')[:100]}...")

        elif command == "recommend":
            print(get_model_recommendations(memory, type))

        else:
            print(f"Unknown command: {command}. Choose from: seed, recall, recommend")
            raise typer.Exit(1)
