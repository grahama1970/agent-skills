"""Prompt-lab evaluation module for Heart rationale generation prompts.

Evaluates the GPT rationale system prompt (gpt_heart-rationale.txt) which
explains WHY a text expresses a given emotion. This is the T1.5 GPT prompt,
distinct from the classification prompt (heart_tagger_v1.txt).
...
VALID_LABELS: Set[str] = {"anger", "fear", "joy", "neutral", "sadness", "trust"}
...
def _load_rationale_ground_truth(path: Path) -> List[dict]:
    gt_file = path / "heart_rationale.json"
...
def _load_rationale_prompt(path: Path) -> str:
    prompt_file = path / "gpt_heart-rationale.txt"
...
@app.command("eval-heart-rationale")
def eval_heart_rationale(
    ...
    """Evaluate Heart rationale generation prompt (gpt_heart-rationale.txt).
    ...
    console.print(
        f"[bold]Heart Rationale Eval[/bold]  model={model}  "
        f"cases={len(cases)}  prompt=gpt_heart-rationale.txt"
    )
    console.print()
    ...
    # Summary table
    summary = Table(title=f"Rationale Eval Summary ({model})")
    ...
    for lbl in sorted(by_label):
        ...
        l = sum(1 for r in rs if r.get("label_ok")) / cnt
        g = sum(1 for r in rs if r.get("grounded")) / cnt
        breakdown.add_row(lbl, str(cnt), f"{j:.0%}", f"{l:.0%}", f"{g:.0%}")
    ...
    out_path = RESULTS_DIR / f"heart_rationale_{model.replace('/', '_')}_{ts}.json"
    out_data = {
        "model": model,
        "prompt": "gpt_heart-rationale.txt",
        "timestamp": datetime.now().isoformat(),
        "cases": len(cases),
        "json_rate": round(json_rate, 4),
        "label_rate": round(label_rate, 4),
        "conf_rate": round(conf_rate, 4),
        "ground_rate": round(ground_rate, 4),
        "avg_rationale_len": round(avg_len, 1),
