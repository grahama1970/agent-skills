...
class EvalSummary:
    ...
    def correction_success_rate(self) -> float:
        needing = [r for r in self.results if r.correction_rounds > 0]
        if not needing:
            return 1.0
        return sum(1 for r in needing if r.correction_success) / len(needing)



class EvalRunner:
    """Base class for domain-specific evaluation runners.

    Extracts the common load → call LLM → score → gate → save → record pattern
    shared by pl_eval.py, pl_eval_sparta.py, pl_eval_heart.py, etc.

    Subclasses override:
        - domain: str — domain name for memory and results
        - load_cases(prompt, skill_dir) → List[TestCase]
        - format_request(system, user_template, case) → dict
        - score(case, llm_result) → EvalResult
    """

    domain: str = "taxonomy"
    prompt_default: str = "taxonomy_v1"

    def load_cases(self, gt_name: str, skill_dir: Path) -> List["TestCase"]:
        return load_ground_truth(gt_name, skill_dir)

    def format_request(self, system: str, user_template: str, case: "TestCase") -> dict:
        return {
            "system": system,
            "user": user_template.format(name=case.name, description=case.description),
        }

    def score(self, case: "TestCase", validated: Any, llm_result: Any) -> "EvalResult":
        from models import TaxonomyResponse
        v = validated or TaxonomyResponse()
        return EvalResult(
            case_id=case.id,
            predicted_conceptual=v.conceptual,
            predicted_tactical=v.tactical,
            expected_conceptual=case.expected_conceptual,
            expected_tactical=case.expected_tactical,
            rejected_tags=llm_result.rejected_tags,
            confidence=v.confidence,
            latency_ms=llm_result.total_latency_ms,
            correction_rounds=getattr(llm_result, "correction_rounds", 0),
            correction_success=getattr(llm_result, "success", True),
        )

    def quality_gate(self, summary: "EvalSummary") -> bool:
        from config import F1_THRESHOLD, CORRECTION_SUCCESS_THRESHOLD
        return (
            summary.avg_f1 >= F1_THRESHOLD
            and summary.correction_success_rate >= CORRECTION_SUCCESS_THRESHOLD
        )
...
def save_eval_results(
    ...
    if first_attempt_f1 is not None:
        metrics["first_attempt_f1"] = first_attempt_f1
        metrics["corrected_f1"] = summary.avg_f1
        metrics["correction_gain"] = summary.avg_f1 - first_attempt_f1
    ...
    # Task 1: Record prompt provenance when eval passes
    if passed:
        try:
            from provenance import record_prompt_provenance

            prompt_file = skill_dir / "prompts" / f"{summary.prompt_name}.txt"
            ...
            if prompt_content:
                record_prompt_provenance(
                    ...
                    eval_f1=summary.avg_f1,
                    model_tested=summary.model_name,
                    ground_truth_file="",
                    task_name=summary.prompt_name,
                )
        except Exception:
            pass  # Provenance is best-effort, don't break eval flow
