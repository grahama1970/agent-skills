...
if __name__ == "__main__":
    ...
    def _main(
        ...
        tags: Optional[list[str]] = typer.Option(None, "-t", "--tags", help="Filter by tags"),
        ...
        free: bool = typer.Option(False, "--free", help="Only free resources"),
        ...
        if stats:
            stat_data = registry.stats()
            if json_output:
                print(json.dumps(stat_data, indent=2))
            else:
                print("Resource Registry Statistics:")
                print(f"  Total resources: {stat_data['total_resources']}")
                print(f"  Categories: {stat_data['categories']}")
                print(f"  Unique tags: {stat_data['unique_tags']}")
                print(f"  With API: {stat_data['with_api']}")
                print(f"  Free: {stat_data['free']}")
                print(f"  Auth required: {stat_data['auth_required']}")
        else:
            ...
            if tags:
                results = [r for r in results if r.matches_tags(tags)]

            if search:
                results = [r for r in results if r.matches_search(search)]
