...
def run_vitest(path: Path) -> dict:
    """Run vitest with strict fork limits to prevent OOM spirals."""
    npx = shutil.which("npx")
    if not npx:
        return {"type": "vitest", "status": "fail", "duration_s": 0, "output": "npx not found"}
    # 2026-03-17: maxForks:8 with no guards caused 47-fork OOM spiral that killed KDE.
    # ALWAYS enforce single-fork, no file parallelism.
