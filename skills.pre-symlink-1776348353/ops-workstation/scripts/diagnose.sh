...
output_markdown() {
  ...
  if [[ -x "$script_dir/limits.sh" ]]; then
    # Extract just the table from limits output
    "$script_dir/limits.sh" 2>/dev/null | grep -A 10 "^| Limit" | head -7
    echo ""
  else
    echo "Limits check not available."
    echo ""
  fi
